...
> **「live データパイプラインが *購読 churn の最中* に壊れないか」を pytest で gate するとき（live 運用中の add/remove・mid-stream
> 設定変更）＝3 つの archetype 技法**（#141-145 live universe churn × kabu mock 実例 2026-06-26 / findings 0118）。実 venue
> capture（findings 0117 の codec-replayable mock）を **subscribe-gated な `MockVenueAdapter` に decode して流し**、実 `LiveRunner`＋
> controller＋戦略 cell を駆動しつつ membership を編集する。(1) **subscribe-gated mock は「記録された market-data の存在」を
> *購読の構造的証明*にできる**＝add→subscribe の非空虚 floor が**タイムスタンプ無し**で立つ（未購読なら adapter が drop＝0 件）。
> (2) **defense-in-depth（adapter gate ＋ runner の `_is_subscribed` guard の二重防衛）系では、片層を壊しても GREEN のまま**
> ＝それは vacuity ではなく **resilience**。「片層 break で GREEN だから空虚」と早合点せず、非空虚 litmus は**両層の上流**
> （feature の enqueue＝`bt.universe.add/remove` の bridge）を壊して取る（remove enqueue 削除→該当 id が流れ続け per-id RED）。
> (3) **async/threaded gate の convergence wait は *soft*（`_poll`→bool・hard raise しない）**＝壊れた production を「fixture error」
> ではなく **granular な per-id RED** に翻訳でき litmus が鋭くなる（hard `_wait` は 1 つの壊れで全 id を error 化）。さらに
> code-review が突く定番 vacuity: **fresh reducer に観測 id をそのまま食わせて keys⊆ids を assert＝tautology**（実 OHLC を捕捉して
> *routing 一致*＝primary 誤配線で per_id keys が崩れる、を assert／非空 guard／≥2 distinct id を足す）。**`controller.detach` は
> `_driver=None` を無条件代入し join 例外を握る**ので「detached＝driver is None」も空虚＝worker thread の `is_alive()` を観測しろ。
> **fire-and-forget の loop task（pump/recorder）の future を捨てると例外が silent**＝try/except で `bg_errors` に拾い assert する。
> Unity 描画半は別 gate（`KabuLiveChartRenderE2ERunner` を *4 銘柄 state JSON* へ拡張＝per_instrument locator の銘柄別曖昧性解消を
> 実密度差で floor）。

> **「Replay と Auto が同じ決定を出す（seamless parity）」を pytest で gate するとき＝*同一の合成入力を両経路に与えて
> 決定列を比較する*（#151-154 合成データ生成器 実例 2026-06-26 / findings 0120）**。Replay は bar を直供給（KernelStepper /
> 監視 sink）、Auto は **trade→TickBarAggregator→KlineUpdate→Bar** の production 経路（`MockVenueAdapter.inject_tick`）。
> 3 つの archetype 技法: (1) **同じ `{iid:[Bar]}` を両経路へ**与えれば aggregator が Replay と同一 OHLCV を復元する＝
> 入力を二重生成せず単一供給源から分岐（`auto_trades` は flat 足=1tick / 一般足=O,H,L,C を**同一 bucket 内 4tick**で
> 復元・末尾に**次 bucket の flush tick**を足さないと最終 bar が never-closed で欠落＝flush off で RED）。(2) **parity 比較は
> ts ラベルでなく決定列 `(instrument, signed_qty)` で**＝Replay の bar は END ラベル（15:30 / :59.999999）、Auto の
> aggregator bar は bucket 開始ラベル（`bucket*interval`）で **ts は一致しないが time-of-day は保存**される。ts 一致を
> 期待すると偽 RED。(3) 非空虚 litmus は **production logic（ranking の close 依存・aggregator の OHLC 復元・flush）を壊して**
> 取る（close 非依存ランキング→flip と dominant の picks が同一化で RED）。**backcast 固有の 2 gotcha**: ① MARKET 注文は
> `price=None` で venue に行き、production `MockVenueAdapter` 既定 fill の `avg_price=None` を broker の fill-guard が
> **REJECTED 化**する＝parity（決定列比較・約定価格非依存）では `submit_order` を override した filling mock subclass で
> 確定 FILLED にする（v19_cell_auto_parity は `set_next_order_outcome(avg_price=…)` を都度仕込むが、複数約定が 1 注入
> ループに混在するときは subclass の方が決定的）。② **live cell 経路の FILLED `OrderEventData` は静的属性 `symbol` が空**
> （EC-stream 由来でない place でも live は空）＝発注銘柄は `driver._orders[ev.client_order_id].instrument_id` で引く
> （order_events は提出＝発火順）。marimo worker は finally で `controller.detach`（worker join）→`runner.aclose`→loop stop→
> thread join まで畳む（孤児 session が pytest FdCapture を壊す・#112 既知の罠）。

> **既存テストのアドホック合成バー（`_synthetic_bars`/`_series`/手書き `Bar(...)`）を生成器へ集約する移行は、移行前の
> 出力を *スナップショットして byte-identical を機械比較してから* 着地する**（#153 実例 2026-06-26）。移行は「parity を維持」が
> AC なので、(1) 編集前に旧関数の出力を `[(iid,ts,o,h,l,c,v),…]` で JSON dump、(2) 生成器版に置換、(3) 再 dump して
> `before==after` を assert＝価格式・ts 規約・スロット・OHLC 既定・volume 定数のどれか 1 つでも取り違えれば即検出。
> flat 足（open=close）を生成器の gap 既定（open=前足終値）で再現するときは **open を close に明示 pin**（既定のままだと
> 上昇バーで open≠close になり byte-identity が割れる）。移行で**未使用化した import**（旧 `merge_bars_by_ts` 直呼び等）も
> code-review が拾うので一緒に掃く。
...
>
> **「実 producer から採取した committed fixture を実 renderer/decoder に流す」と、合成 payload gate が *通っていた* 別 leg（dead code を含む）を露出させる——「実データは合成より強い」の具体形**（#165 marimo リッチ output サンプル 実例 2026-06-26 / findings 0123）。既存 AFK（Section19）は **合成 raw-markdown**（`# Title\n**bold**`・タグ無し）を `SetOutput` に流して「markdown→`<b>`」を gate していたが、実 `mo.md` は **rendered HTML**（`<strong>…`）を出すので C# `RichToUnity` が**別分岐**を取る（生 markdown→`MarkdownToUnity`／HTML→`HtmlToUnity`）。`HtmlToUnity` は `<strong>`→`<b>` 変換**直後**に汎用タグ strip（`<[^>]+>`）で `<b>` も剥がす**デッドコード**で、実サンプルを流して初めて RED（合成は別 leg ゆえ素通り）。教訓: (1) **renderer/decoder の gate は「実 producer が実際に出す形」で流せ**——同じ型・mimetype でも markup が違えば *別分岐*を通る（合成入力が production と同じ leg を通る保証は無い）。(2) committed fixture を実サンプルから採取（`python -m tests.capture_*`）し、Python gate（payload 産出＋freshness）と AFK gate（routing/描画）で**同一 fixture を pin**＝1 capture で両レッグ整合（md/table は byte 一致・matplotlib image は version 依存ゆえ構造的 valid-PNG のみ・mo.ui は `random-id` UUID を正規化後 byte 一致）。(3) **fixture-drift で gate が vacuous 化しないよう、その fix が *実際に走る前提条件*（例「fixture data に `<strong>` がある＝HtmlToUnity leg を通る」）を AFK で明示 assert**（code-review 指摘: 前提を pin しないと将来 raw-markdown 再 capture で別 leg に落ち「古い壊れた regex でも GREEN」へ戻る）。image の GPU 画素 decode は HITL 降格・routing は `[image/png]` ラベルで pin（Section19 と同型）。
>
...
>
> **そして #149（2026-06-26・caret 不可視）: 「issue が挙げた不変条件が *既定値で全部満たされている* なら、その不変条件を額面で
> assert する gate は vacuous——真因は別の所にあるので RED→GREEN で *実際に変わる物* を探せ」。** issue 本文の AC は caret の
> width>0・色 alpha>0・textViewport 割当を不変条件に挙げたが、コードで裏取りすると **3 つとも TMP/builder の既定で既に true**
> だった（caretWidth 既定 1>0・customCaretColor=false→caretColor は textComponent.color に追従で alpha 1・textViewport は
> builder が配線済み）＝それらを assert する gate は **既に GREEN ＝不具合を検出できない**。真因は別物＝**TMP_InputField が
> caret CanvasRenderer を OnEnable でしか・かつ textComponent!=null のときしか生成しない**（`TMP_InputField.cs:1172`）のに、
> builder が `new GameObject(typeof(StrategyInputField))` コンストラクタで field を足す＝**最初の OnEnable が textComponent 未配線で
> 走り**（次行で配線）caret renderer が永久に未生成＝どの zoom でも caret 不可視（文字入力は renderer 不要なので効く＝症状一致）。
> **症状から真因 class を当てるな**——owner に AskUserQuestion 1 問（「いつも見えない／縮小時だけ」）で「zoom で細い」説（width）を
> 棄却し、construction-order に切れた（owner スクショは 1.3× で text 可視・caret 不在＝width 説と矛盾）。**fix の altitude**＝
> 事後の re-cycle（`enabled=false→true`・余分な OnEnable(null) を残す）より、**サブツリーを inactive で組み textComponent を
> 配線してから SetActive(true)**＝最初の OnEnable で依存を満たす（Unity prefab 流儀・simplify レビューの Altitude/Efficiency 両
> エージェントが指摘）。**gate の作り方（効果が headless 不可なとき前提条件を pin）**＝caret renderer 生成は
> `if(Application.isPlaying)`-gated（`:1170`）で `-batchmode` EditMode では生成されない（TMP caret/mesh trap）が、**OnEnable
> 自体は EditMode でも走る**ので、`StrategyInputField` に observability seam（`OnEnableCount`/`TextComponentReadyAtLastEnable`・
> SubmitConsumedCount と同型）を足し OnEnable override で「最新 enable 時に textComponent を見たか」を記録＝**caret 生成が依存する
> 正確な前提条件**を直接 assert する。`OnEnableCount>0` で「OnEnable が走らなかったから false」の偽陰性も排除。実 caret の点滅
> 可視（0.2–5× zoom）は HITL。RED→GREEN litmus＝field を active 構築（配線前 enable）に戻すと「latest OnEnable saw a NULL
> textComponent」で RED。memory [[tmp-inputfield-caret-needs-textcomponent-before-enable]] が正本。
...
3. **台本（.md）に Action 行を足す/更新**。カバー状態を 5値で付け、HITL/対象外 は理由併記。
   - **決定を反転/退役する slice では、その挙動を *参照する他 runner の .md 台本* と *PASS タグの要約文言* も棚卸しして flip する**。`.cs` の assert を反転しても、(a) 同じ挙動を spec正本として記述する別 runner の `.md`、(b) runner の最終 `Debug.Log("[E2E … PASS] …")` 要約文字列、(c) `E2E-INDEX.md` の issue/findings 参照——の 3 つは別ファイルで漏れやすく、放置すると **spec正本が gate と逆の旧挙動を語り続ける**（maintainer が .md を信じて反転を巻き戻すリスク）。実例 #113 2026-06-24: Open 層の自動 wrap 退役で `FileOpenNonMarimoE2ERunner.cs` を error 期待へ flip したが、`FileNavGuardE2ERunner.md`（FILEGUARD-06 行＋discardDirty seam 節）が #86 wrap/`WrapMode`/`discardDirty:true` を spec正本として記述したまま残り、`StrategyEditorNotebookE2ERunner` の PASS 要約も "non-marimo Open wraps as 1 cell" のままだった——いずれも code-review(simplify) が検出。grep 起点: 退役する型名/フラグ名（`WrapMode`/`discardDirty` 等）を **`.md` 込みで** 全 grep し、コメント・台本・要約も同時に直す。
     **さらに、退役するのが *共有 catalog の kind / enum value*（`FloatingWindowCatalog.KIND_*` / `OpenMenu.Venue` 等）なら、それを *他 runner の `.cs` フィクスチャ* が generic な stand-in に使っていないか必ず全 grep する**——「assert を反転」では済まず、**フィクスチャを生き残る kind へ移行して契約を保つ**のが正（削除すると回帰網が穴あく）。同時に **「catalog が全 kind を resolve する」系の assertion は『この kind を forward-compat で skip する』AC と *直接衝突* する**ので、退役 kind をその列挙から外す。実例 #126 2026-06-24（ADR-0026）: `KIND_STARTUP` を catalog `Default()` から外す（＝saved layout の `"startup"` を `TryGet=false` で skip させる唯一の手）と、`FloatingWindowE2ERunner` が startup を **core/dock の汎用 fixture** に使う 11 箇所が壊れ、`Section12a`（catalog が全 dock kind を resolve）が forward-compat-skip の AC と矛盾した。fixture を生き残る core（`KIND_RUN_RESULT`）/ plain dock（`KIND_BUYING_POWER`）へ移行し、`Section12a` から startup を除去、`Section32`（factory base group）を 5→4 に書換えて GROUP-14 を再固定。ADR-0019 §2「core kinds=FIXED」は *配置 supersession の帰結* として {run_result} に縮小（ADR は無改変・findings に帰結記録）。**fixture 移行先は geometry を割らない kind を選ぶ**——退役 kind の minSize ≤ 既存 spawn サイズの生存 kind に寄せれば座標 assert を再計算せず blanket textual replace で済む（実例 #178: orders/positions/buying_power〔minSize 240×140〕を `chart`〔280×200・280×180 spawn を clamp-up して座標が割れる〕でなく `run_result`〔240×140〕へ。**ただし sibling 並行退役で移行先 kind 自体が消えると二重退役**——#178 の `run_result` は同日 #172 が popup 化で退役したので merge 後は kind-agnostic fixture を front-plane `KIND_ORDER`/`KIND_STRATEGY_EDITOR`〔280×180・両退役を生存〕、dock-plane 限定 fixture を唯一の生存 dock kind `chart`〔サイズ bump〕へ再寄せ）。**退役する *field / private member* を消すときは `Assets/Scripts` の compile 参照だけでなく `Assets/Tests` の *string-name reflection 参照*（`ty.GetField("_buyingPowerView")` 等）まで grep する**——reflection は文字列引数ゆえ field が消えても compile を素通りし runtime で初めて null→RED（compile-only gate も `error CS` 0 で GREEN を返し見逃す）。実例 #178: 退役した `_buyingPowerView/_ordersView/_positionsView` を `NotebookToHakoniwaJourneyE2ERunner`（NBHAKO・12 PASS の登録 gate）が `ReadSeams` で reflection 参照、`grep Assets/Scripts` では出ず compile も通ったが runtime で全 section RED 寸前。退役 field/method は **`grep -rn '"<fieldName>"' Assets/Tests`（クォート込み）も必ず打つ**。退役 surface の data 検証 gate は data の*新しい行き先*（バーのホバー card・byte 一致の同 formatter）へ assert を付け替える。**続報 #172/#173（ADR-0037・run_result を dock→popup 退役）: kind の grep だけでは穴が残る——退役で *base cluster の個数* が変わるとき `KIND_*` 文字列を一切含まない *数値 base-count assertion* が sibling runner に潜む（`BaseDockWindowIds.Length != N`＝`ScenarioStartupE2ERunner` S13・`baseIds = {…}` の長さ＝`ChartPlacementJourneyE2ERunner` CP-S4-01・PASS-tag 要約の「4 base singletons」「dock 5→4」）。`grep KIND_RUN_RESULT` に出ない（本数を数えるだけで kind 名を書かない）ので、kind grep に加えて `grep -nE "expected [0-9]|!= [0-9]|Length [!=]= [0-9]|[0-9] base"` を全 runner/probe＋PASS-tag 要約に走らせ N→N-1 を先に掃く。base を 1 枚未満に減らす退役（#172+#178 合算で base=0）なら `FormGroup` の ≥2 閾値割れで factory base group 自体が形成されなくなる＝group 形成テスト（S32）を「production base 数から decouple した FormGroup *力学* テスト」へ書き換える。**
...
- **終了時クラッシュ系ゲート（verdict=プロセス exit code）は macOS `ReportCrash` の非同期/throttle で `.ips` が偽陰性になる**（#170 実例 2026-06-27 / findings 0126）: `exit()→__cxa_finalize` の C 拡張 dtor crash（duckdb 等）を gate するとき、合否の正本は **プロセス exit code（139=SIGSEGV / 0=clean）**。`.ips`（DiagnosticReports）の有無を verdict にすると壊れる——`ReportCrash` は**非同期で >2s 遅延**（`sleep 2` で「新規 .ips 無し」＝偽 GREEN を踏んだ実例）、かつ**連続クラッシュで throttle**して以降 `.ips` を書かない。`.ips` 署名（`Unity-*.ips` を JSON パースして triggered thread のスタック）は**最初の数回だけ**確認用に使い、回帰判定は exit code を正本にする。control（残留を作らない経路）が exit 0、RED 経路が 139、で「その residual が原因」を pin する。
- **abort/crash *そのものが非決定論*（exit code が flaky）のときは exit code を verdict にせず、決定論で 100% 観測できる *真因の上流不変条件* を gate にする**（#181 macOS Tachibana(Demo) crash 実例 2026-06-28 / findings 0130）: クリック→Unity 即死の真因は venue prompt-login が `tkinter.Tk()` を**非メインスレッド（`venue-login-dialog` executor）で生成**し macOS Cocoa の NSWindow main-thread 要件違反で `NSException`→`libc++abi` abort（SIGABRT）すること。だが Cocoa main-thread 違反は **UB なので同一コードで exit 134 と exit 0 を両方観測**＝「落ちる」を verdict にした gate は flaky で使えない。代わりに **`tkinter.Tk` をスレッド記録 fake に差替えて実 `_handle_prompt_login` を駆動し「`Tk()` が非メインスレッドで生成されたか」を assert**＝Cocoa を一切触らず（abort せず）決定論 100% で真因を固定（findings 0126 の「exit code を正本に」を、exit code 自体が信用できない場合へ一段一般化）。fix 方式非依存に書く（main-thread dispatch でも off-main 拒否でも `Tk()` が非メインで作られなくなれば GREEN）＝`xfail(strict=True)` で fix 前に land（#58 パターン）、platform 固有なので `skipif(sys.platform!="darwin")`。**典型 death-angle: 既存テストが全て `monkeypatch.setattr(..., "_try_create_tk", lambda: True)` で実 probe を潰しており、誰も実 GUI-lib 呼び出しを実スレッドで踏まない**——「GUI ライブラリ（tkinter/Cocoa/Win32）を *どのスレッドで* 触るか」が真因のクラスでは、その fake が真因経路を丸ごと隠す。**fix スライス（#181/ADR-0040 2026-06-28・findings 0131）でこの flaky-abort gate を GREEN にする最良手は「GUI ライブラリ自体を撤去」のことがある**（owner 判断で tkinter ダイアログ→Unity uGUI モーダル＋Python は headless 認証だけにオーケストレーション反転）。そのとき: (1) **真因不変条件 gate を「症状観測」から「真因経路が構造的に存在しない」へ書き換える**——`xfail(strict)`「Tk() が非メインで生成されたか」→ enforcing「login 経路のどのモジュールも `import tkinter`/`from tkinter` しない」（`inspect.getsource` を source-scan・import を戻すと RED で再び macOS crash 経路）。**source-scan は素の `"tkinter"` でなく `"import tkinter"`/`"from tkinter"` で当てる**——コメント/docstring の "tkinter" 語に誤反応する（1st run で全モジュール offender 化→修正の実害あり）。(2) **crash-class の回帰スイート（#133-135 TKTEARDOWN = Tcl_AsyncDelete）は「移送」でなく「廃止」**——GUI ライブラリが消えれば crash クラスは構造的に発生し得ず、契約は move-target でなく eliminate-target（[[grill-with-docs]] 削除スライス discipline の「契約が sibling に移送されるか／消滅するか」を見分ける・findings に「crash クラス消滅で回帰廃止」と明記）。(3) C# 半（モーダル入力→送信→結果）は Python-FREE fake executor で AFK、headless 認証半（submit RPC→実 auth）は pytest＝2 ゲート分割。**code-review の盲点に注意: 新規ファイルを *追加* するスライスで finder agent が `git diff HEAD` を使うと untracked 新ファイルが diff に出ず「クラス/モジュールが存在しない（CS0246/ImportError）」の偽 finding を量産する**——レビュー時は `git status --short` で untracked を列挙して読ませる（compile PASS＋pytest＋AFK が緑なら「存在しない」系 finding は即 REFUTED）。
- **RED は再現するが clean GREEN が存在しない（提案 fix が実機で全滅 / 上流バグ blocked）ときは、無理に passing gate を著さず「committed repro probe + findings + issue を blocked」に着地する**（#170 実例 2026-06-27）: 本スキルの既定フロー（RED→GREEN・rollup に PASS タグ）は GREEN 到達可能を前提にするが、原因が**上流ライブラリの未修正バグ**（例 duckdb/duckdb#13904 の C++-static dtor が finalize で GIL を踏む）だと、issue の「本命 fix」を含め全候補が RED のまま落ちることがある。そのときの正しい成果物は (1) 再現と「どの mitigation が効く/効かない」を実機実測で固めた **repro probe（`Assets/Editor/*Probe.cs`・passing gate ではないと明記）**、(2) 上流 issue リンク＋マトリクスを記した **findings**、(3) 効かない fix の **revert**（dead fix を出荷しない）、(4) issue を **blocked 化**＋本文の誤った「本命」前提を訂正。`E2E-INDEX` に PASS gate として載せない（GREEN が無いので嘘になる）。「verify したら本命 fix が無効と判明」は**根本原因の確認が正しくても fix 方針が誤っている**ケース＝owner に方針判断を返す（AskUserQuestion）。
