...
>
> **「集合 SoT を反復元にして per-element に副作用を発火する helper」を *早期 return の内側* に置くと、_new addition_ のテストだけでは通るが _restored / pre-existing element_ のテストで silent regression する**（#129 ChartSpawnPreview 実例 2026-06-25）。production の `SyncChartWindowsToUniverse` は `missing.Count == 0 → return` の early-return を持ち、当初その後に `RequestChartPreviewsForAllLiveCharts()` を置いた＝Universe.Add 経路（CHARTPREVIEW-01）は GREEN だが、RestoreFloating 由来で `_chartViews` に居る restored chart（CHARTPREVIEW-03）は missing 0 で early-return に飲まれ helper が発火せず RED。**SoT 反復元 helper は early-return の *外* に置き、ゲートは必ず「新規追加」と「既存集合の再反復」の両ケースを別 Section で踏む**——前者だけだと「helper が呼ばれた」と「helper が新規 element だけ拾った」を区別できず vacuous pass する。AFK runner は (a) Universe.Add → 新規 iid が calls にある (b) Universe に既にある iid を含む全体反復 → 既存 iid も calls にある、を別 Section で固定。litmus: helper を `if (missing.Count > 0) { … helper(); }` の中に置く → (b) RED・(a) GREEN（位置で differential）。[[delete-the-production-logic]] litmus の特殊形＝**「production の controlflow ブランチを 1 つ動かすと特定 Section だけが落ちる」位置感度**を gate に組み込む。
>
> **ApplyLayout / Restore 系の sequencing-sensitive Section では「dirty-guard を上流で立てると下流の reseed が silently no-op する」**（#129 Section3 実例 2026-06-25）。`ScenarioStartupController.Params` の dirty-guard（`if (Params.Dirty) return;` in `PopulateFrom`）は production の意図された不変条件（in-flight edit を外部 reseed で潰さない）だが、AFK Section が「ReseedFromEditor 末尾の sync で restored chart の preview が発火するか」を gate するときに、Section の冒頭で `scenario.SetStart/SetEnd` 等を呼ぶと **Dirty=true → reseed の `PopulateFrom` no-op → Universe 空 → SyncChartWindowsToUniverse が restored chart を orphan despawn**、で偽 RED になる。診断: 「positive control（ApplyLayout 直後）は GREEN なのに reseed 後の最終 assert が RED」は dirty-guard / lifecycle-guard の踏み抜き候補。対処は (a) Section 冒頭の Params 事前設定を撤去して sidecar 由来の seed に委ねる、または (b) reseed を回避して `SyncChartWindowsToUniverse` を直接駆動し contract（helper iteration）だけを pin する。**positive control を「ApplyLayout 直後」「reseed 直後」の 2 点で取る**と、どの段で消えたか即座に切り分けられる（#129 では 1 点だけ取って 1 ラウンド余計に消費）。
>
