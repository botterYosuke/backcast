...
description: >-
  ...
  **この preflight 義務は GitHub Actions の YAML に限らない**——**workflow をローカル build/release スクリプト（PowerShell 等）へ移植する / 新規に build・配布・smoke スクリプトを書く**ときも同型のトリガー。「ローカルで build して release にアップロードするスクリプトを作る」「workflow をスクリプトに置き換える」「CI を退役してローカル script に一本化」と言われたら、実装に直行せず先に grill で **runtime 契約（プロセスの終了挙動・出力パス・パス長・tar/zip ツールの実体・log マーカー）を docs と過去 commit から洗い出す**こと（#180 実例 2026-06-27: `deploy-gh-release.yml` を `scripts/build-and-release.ps1` へ移植する際 grill を発動せず実装に直行し、Unity shutdown hang・GNU tar vs System32 bsdtar の `C:` 誤認・bundled venv の MAX_PATH/Move-Item lock を **6 回の build 実走で逐次発見**する羽目に。いずれも da794cf や移植元 YAML のコメントに既出で、先に docs を読めば preflight 一発で拾えた）。
---
...
**「Phase 1 docs（ADR + findings + CONTEXT）を実装した。設計通りか見て」という実装レビュー依頼では、ADR を額面で読まず *同じ Phase の companion findings と git のコミット順* に突き合わせる。** Phase 1 の ADR コミットは *issue 本文ベース* で書かれ、その後の grill コミット（codebase 裏取り F1–F6・spike 結果）が下位決定を *訂正* していても、ADR がその訂正に追いつかず **issue 本文の旧設計を「方針」として残す**ことがある（＝ADR が自分の companion findings と矛盾する内部不整合）。owner はこれを「方針と違う気がする」と曖昧にしか言えない。やること: (1) `git log --oneline <issue-range>` で「ADR コミット → findings/spike コミット」の順序を確認し、後発 findings が ADR の D-項を supersede していないか 1 項ずつ突合（特に *速度・GIL・スレッド・seam の reuse 可否* など spike で覆りやすい load-bearing 決定）、(2) **`ls docs/findings/ | sort` で finding 番号の重複を確認**（2 コミットが同番号を別ファイルで採番する事故が起きる——同一設計の木の前半・後半なら 1 ファイルへ統合）、(3) 食い違いは「ADR の文言を binding 決定（最新 findings）に合わせて *直接修正*」が正——未マージ枝の intra-Phase-1 不整合は decision の逆転ではなく文書バグなので新 superseding ADR は要らない（**自己保護条項は surface した上で**、owner HITL で in-place 修正を確認）。CONTEXT glossary の用語名そのものが旧設計を体現している場合（例「速度レジスタ」←実体は call-time キャプチャで register ではない）は用語ごと改名し backlink も追従（#95 実例 2026-06-20: ADR-0016 D8/D9 が issue 本文の「live-mutable 速度レジスタ・毎 bar sleep」のまま、後発 grill F6 の binding「sleep 撤去・auto-switch・速度は開始時キャプチャ・stop だけ cross-thread」と逆。D2 の「HeadlessKernel 流用」も F1 訂正「marimo Runner net-new 配線」と不整合。findings 0070 が 2 ファイルで番号重複。3 件とも ADR/CONTEXT を F1/F6 に in-place 整合＋findings 統合で決着、code-review が残存 2 箇所の「速度レジスタ」と class 名 `DataEngineCore`→`DataEngine` を検出）。
...
**設計が *先行する grill セッション群* で既に完全凍結されている「実装引き継ぎ」依頼では、Q1–… を再導出しない。** issue のコメントスレッド＋ADR＋findings に設計の木が `[x]` で出揃い、あるコメントが「実装に入る作業者はこのコメント＋正本（ADR/findings）を読めば足りる」と明言していることがある。このとき grill の価値は「新たな質問の往復」ではなく **(1) 正本（ADR=immutable / findings の設計の木）を読み込んで凍結済み下位決定を頭に入れ、(2) `git status` / `git log` / 該当シンボル grep で *working tree の現状が凍結プランのどの Step か*（例: まだ前任の SUPERSEDED 実装が残っているか）を裏取りし、(3) 実装スコープ（どの Step まで今セッションでやるか）を AskUserQuestion で 1 回だけ再アンカーして実装に入る** ことにある。先行セッションが費やした grill を Q1–Q7 の再インタビューで無駄にしない。複数コメントがあるときは **最新コメントが最終設計**で、古いコメント（特に「SUPERSEDED」明記のもの）は履歴として扱う。実装着地は slice の `docs/findings/` に追記して締める（`code-review(simplify)`＋`post-impl-skill-update` も併発）（#81 実例 2026-06-18: findings 0049=text-append が SUPERSEDED → HITL 訂正 → ADR-0013+findings 0050=cell-as-floating-window へ再設計 → 第4コメントに S1 実装分解。引き継ぎセッションは再 grill せず 2 正本を読み込み、working tree が Step 0 の旧 text-append のままと git 確認し、Step 0+1 をやるか AskUserQuestion で確定して実装・golden gate を GREEN 化した）。**さらに owner が「実装コストは度外視」「理想的な完成形を目指せ」「手を抜くなよ」「全 slice」「全部やって」のような all-in 指示を併発しているときは、(3) のスコープ AskUserQuestion すら省略可**——指示自体が「spec の全 slice を本セッションで」を意味するので、scope 確定の round-trip は無駄になる。pre-frozen 設計 + all-in 指示 = 即 `behavior-to-e2e` formal invoke → 実装、で良い（#104 実例 2026-06-21: 8 vertical slice の Hakoniwa window group / drag semantics を ADR-0019 + findings 0082 即読 → owner の「実装コストは度外視して理想的な完成形を目指せ。手を抜くなよ」で scope = 全 slice 確定 → behavior-to-e2e で S20–S31 + GROUP-01..13 を固定 → 8 slice 連続実装 → code-review(simplify high) → pair-relay で F1–F10 fix loop。AskUserQuestion 1 回も挟まず完走）。**ただし「issue 本文が pre-frozen を主張している」=「正本 docs が repo に揃っている」とは限らない**——issue 本文が参照している `docs/findings/NNNN-*.md` や `CONTEXT.md` のグロッサリー項が **repo に存在しないことがある**（issue オープン時に予約された finding 番号を別 slice が消費した・CONTEXT 項の追加が省略されたまま設計合意が完了した、等）。pre-frozen 即実装に進む前に **`Glob` で参照 finding を引き・`Grep` で CONTEXT 項を引いて実在確認**し、欠落していれば grill の deliverable は「設計インタビューの再実行」ではなく「**owner と既に合意済みの設計を docs に固定する＝欠落 finding/CONTEXT 項を作成**」になる。新規 finding 番号は `ls docs/findings/ | sort` で次空き番号を取り、issue 本文の番号参照が stale だった場合は finding 内で「番号注記: issue 本文は 0NNN を名指していたが本 finding は 0NNN+k で採番」を明記して読み替えを示す。**all-in 指示があっても docs が欠落していれば即実装には進まない**——欠落 docs を grill の成果物として先に landし、その後 behavior-to-e2e へ pivot する（#140 実例 2026-06-25: issue 本文は `docs/findings/0111-chart-window-instrument-label-in-title.md` を名指したが 0111 は別 finding が消費済 → 0112 で採番・CONTEXT「銘柄表示ラベル」項も無かったので並行で新設・owner の all-in 指示 (実装コストは度外視 / 理想的な完成形 / 手を抜くな) を受けて 2 件の docs を先に書き、その後 behavior-to-e2e → 9 section AFK → code-review high → 8 finding fix → 2nd-pass で 1 Medium 残・fix まで完走）。
...
**AskUserQuestion を `(P1)/(P2)/(P3)` のような多択ラベル＋技術ジャーゴンで盛り過ぎると owner に「意味わからない」で差し戻される。** grill 中盤で下位決定が枝分かれしたとき、設計の木の全枝を 1 つの multi-select に詰めると、option 文言が「`bound + not dirty` `WYSIWYR` `Save As 強制` `destructive 上書き`」のような repo 内部語の連鎖になり owner が読み下せない。owner が中央の問い（核）を先に固定したいフェーズでは、**最小の yes/no 1 問へ畳む**のが正解。「Save したとき、ファイルを marimo 形式に書き換えてよい？／はい・いいえ」のように、一語で答えられる核を 1 つだけ立て、派生（dirty 扱い・provider 供給条件・resume 経路）は yes/no 取った後で個別の小問にする。AskUserQuestion 1 問あたり「decision の 1 軸 + 平易な日本語の 2 択」が上限の目安（#86 実例 2026-06-19: 3 択 `(P1)/(P2)/(P3)` で「bound + dirty 状態」「destructive 上書き」「provider supplyable」「Save As 強制」を一度に聞こうとして「意味わからない。もっとわかりやすく質問して」で reject、Save 上書き OK か否かの yes/no 1 問に畳んだら即着地）。設計の木の全枝を見せる衝動は grill agent 側の事情で、owner にとっては「決めて欲しい一点を一度に問われた方が早い」。**さらに、option を 2 択へ畳んでも *前提（何が起きていて・どの語が何を指し・なぜ今それを決めるのか）を説明せずに* いきなり AskUserQuestion を出すと、owner は質問の意図自体を読めず「意味がわからない。前提を説明してから聞いて」で差し戻す**——畳む（structure）と前提を噛み砕く（context）は別の作業で、両方要る。質問を出す前に、症状→真因→選択肢が効く理由を**平易な言葉（必要なら例え話）で 1 段落**先に置き、技術語（`basis_ms`・頂点予算・クラス名）には日常語の言い換え（「バーの間隔＝ものさし」）を添える。それから「決めてほしい一点」を 2 択で問う（#133 実例 2026-06-30: Replay 分足 collapse の修正方針を、前提説明なしに `basis_ms` の正本／vertex クランプの 2 問で聞いて「意味と意図がわからない。前提をわかりやすく説明してから質問して」で reject。「チャートは『1 マス＝何分か』のものさしを持つ／今それが日足に化けている」と例え話で前提を噛み砕いてから同じ 2 択を聞き直したら即着地）。
