...
| markdown cell (`mo.md`) detect/seed | NO cell type — a normal cell `mo.md(r"""…""")`. Detect: mirror `_convert/common/format.py::get_markdown_from_cell` (refs=={mo}, no defs). Canonical seed: `_ast/codegen.py::construct_markdown_call` (bare `mo`, prefix `r`). backcast [m] button + shared-import-cell: findings 0126 |
