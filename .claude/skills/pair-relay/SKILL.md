...
2. **authoritative なゲートは司令塔が自分で回す**。Navigator が「compile exit=0」と言っても、司令塔が改めて Unity batchmode（`-executeMethod <...>Probe.Run` 等）／`pytest` を実行し raw 出力で確認する。これは「Medium 以上なし確定前に必ずテストを回す」原則の延長で、Navigator の自走 compile はあくまで Navigator の作業ログであり最終ゲートではない。
   ⚠️ **backcast Unity batchmode の wrapper exit 1 は benign のことが多い（#8 viz-spike 実例で司令塔が2回引っかかった 2026-06）**: ラッパースクリプト末尾の `grep -c "error CS"` は **0 件マッチで exit 1** を返す（＝コンパイル成功なのにタスクが「failed exit 1」になる）。加えて、**owner が Editor を開いたまま／Unity Hub・`Unity.Licensing.Client` daemon が常駐**していると、batchmode が license channel / Mono debugger port を奪い合い（`Failed to handshake to channel`・`debugger-agent: Unable to listen on <port>`）非ゼロ終了することがある。判定は wrapper の exit code ではなく **`UNITY_EXIT=0` ＋ ログ内の `Exiting batchmode successfully now!` ＋ `MonoPostProcessAllAssets` 完走 ＋ `grep -c "error CS"` の値（0 を期待）**で行う。確実な再走が要るなら `tasklist | grep -i Unity` で Editor/Hub の常駐を確認し、`grep -c` を `... ; echo "CS=$(grep -cE 'error CS' log || true)"` のように exit code から切り離す。
