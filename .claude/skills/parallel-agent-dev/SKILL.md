...
description: 実装計画をタスクグラフに分解し、依存関係の無いタスクを複数エージェントに並行させて消化するオーケストレーション手法。大型フェーズ（5タスク以上）を単一エージェントで順次実装するより 3〜5倍速で完遂できる。同じ「分解→並行→集約」パターンは複数観点の並行レビューにも使える（ドメイン別レビューエージェント＝例: nautilus/並行性・venue forward-compat・コード品質 を同時に走らせ、所見を集約して Medium 以上を修正）。トリガー: 「/parallel-agent-dev」「大型フェーズ」「フェーズを実装して」「計画書を実装して」「複数言語にまたがる実装」「Python + Rust 実装」「**Python + C# (Unity) 実装**」「プランを並列で」「並行実装」「複数エージェント」「並行レビュー」「複数観点でレビュー」「複数エージェントでレビューして修正」「Medium 以上が無くなるまでレビュー」。ユーザーが /pair-relay を指定しても対象が 10 タスク以上 / 多言語混在（Python+Rust **または backcast の Python+C#(Unity)**）の場合はこちらを優先する。
---
...
13. **検証ゲートの実行自体が working tree を汚す（生成 artifact の再生成）／レビュー・監査オーケストレーションにも本スキルは使える** — 本スキルは「実装」前提で書かれているが、**完了報告のレビュー/E2E カバレッジ監査を専門 agent に分担並行させる用途**にもそのまま使える（read-only review agent を 4 本＝source correctness / E2E bypass / dead-code / docs 整合 の次元別に並列起動 → オーケストレーターが裏取り＋consolidate → Medium 以上を直列 fix）。実例（#125–#128 ADR-0026 レビュー 2026-06-25）: 4 review agents が cross-corroborate して真の RED 1 件（`ChartPlacementJourneyE2ERunner` が startup 退役の棚卸し漏れ＝5 再走ゲート外で GREEN すり抜け）と litmus 虚偽（`IsCoreKind` は呼び出し元ゼロの dead で litmus にならない）を発見。**ただし agent の主張は #6 どおり必ず自分で grep/read 裏取り**（CascadiaMono asset +2560 行の指摘は tracked diff に無く誤報だった）。**Unity 固有の落とし穴**: `scripts/run-live-e2e.ps1` で batchmode E2E を回すと副作用で `Assets/TextMesh Pro/.../CascadiaMono SDF.asset` が再生成され `git status` に M（~+1600 行）で出る＝#10 の cargo-fmt 全ツリー汚染の Unity 版。**検証後に `git checkout -- "<その asset>"` で revert**してからコミット範囲を決める（[[unity-batchmode-regenerates-font-atlas]] memory）。また Unity C# プロジェクトの検証ゲートは `cargo test`/`pytest` ではなく **batchmode AFK runner（`-Method <X>.Run`・`[E2E <id> PASS/FAIL]` タグ rollup・exit 0）と CompileOnly**＝本 SKILL の cargo/clippy/pytest 文言は読み替える。さらにユーザーが**レビュー中に並行で commit/rebase する**ことがある（findings の番号衝突で 0101→0102 へ rename される等）——「ファイルが消えた」と早合点せず `git reflog`/`git log --all -- <path>`/ls で実地確認する（#10b の延長）。
