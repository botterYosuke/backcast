...
- **公式マニュアル（必読の一次資料）**
  ...
  - 同梱 PDF / Excel（`manual_files/` 配下に実ファイルあり。v4r9 で旧 `api_request_if_v4r7.pdf` / `api_request_if_master_v4r5.pdf` / `api_web_access.xlsx` は下記 v4r9 ファイルに置き換えられ、同梱されなくなった）:
    - [e_api_interface_v4r9.pdf](manual_files/e_api_interface_v4r9.pdf) — **v4r9 インタフェース仕様（REQUEST I/F・データ仕様・マスタを含む最新の一次資料）**
    - [e_api_web_access_v4r9.pdf](manual_files/e_api_web_access_v4r9.pdf) — v4r9 ブラウザからの動作確認例
  ...
  - 外部参照のみ（`manual_files/` には同梱されていない）:
    - EVENT I/F 利用方法・データ仕様は HTML リファレンス `ComT6` と [references/event_protocol.md](references/event_protocol.md) を参照。同内容の旧 `api_event_if_v4r7.pdf` / `api_event_if.xlsx` は e-shiten.jp の公開 URL でのみ確認できる
    - ローカルでは Python サンプルに抜粋コメントがあるのでそれを補助資料にする
- **バージョン表記**: 現行は **v4r9**（`e_api_v4r9`、マニュアル v4.9-000 @2026.05.16）。最大の変更は**公開鍵認証の導入**（[references/pubkey_auth.md](references/pubkey_auth.md) 参照）。同梱資料は v4r9 に置き換え済み（旧 v4r7/v4r5 PDF は撤去）。ただし**本番 URL は移行期間中 旧 `e_api_v4r8` も引き続き有効**（URL レベルの互換。R1）
- **Python サンプル（1 サンプル = 1 サブディレクトリ）**: `samples/e_api_*_{tel,pubkey}.py/`（v4r9 で主要サンプルが `_tel`→`_pubkey`＝公開鍵認証版にリネーム。一部の派生版は `_tel` のまま残存）
  ...
  - 総合例（スタンドアロン、直下に配置）: `samples/e_api_sample_v4r9.py` / `samples/e_api_sample_v4r9.txt`（v4r9 で旧 `e_api_sample_v4r8.py` / `.txt` を置き換え）
...
- マルチバイト（日本語）は Shift-JIS エンコード後に `%xx` 化が公式流儀だが、現状マルチバイト送信は発生していないため未検証。拡張時は [`e_api_web_access_v4r9.pdf`](manual_files/e_api_web_access_v4r9.pdf) の事例に従う
...
- `src/screen/login.rs` の立花用ログイン UI — Python tkinter ヘルパーを in-process（専用スレッド・#122）で開く（フィールド名・ラベル・順序を Rust 側に書かない）
