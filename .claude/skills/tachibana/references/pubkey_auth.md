...
from Cryptodome.PublicKey import RSA   # pycryptodomex（import 名 Cryptodome。plain pycryptodome は Crypto）
...
- 依存は **pycryptodomex**（import 名 `Cryptodome`）。⚠️ plain `pycryptodome` は import 名が `Crypto` で
  `from Cryptodome...` だと `ModuleNotFoundError`（#92 実装で踏んだ）。`Cryptodome` namespace を使うなら
  pyproject の dep は **`pycryptodomex`**。秘密鍵は `RSA.import_key(pem_text)` でオブジェクト化する。
...
- **pycryptodomex**（import 名 `Cryptodome`）依存を公開鍵復号の実装時点で `python/` の依存に加える。
  資格情報保護に Fernet を使うなら **cryptography** も。backcast 実装の正本は `tachibana_pubkey.py`
  （`decrypt_s_url` / `load_private_key` / `load_private_key_from_file`）と `tachibana_credentials.py`
  （`resolve_credentials`: Fernet → dev env の 2 系統）。エラー型 `PubkeyCryptoError` / `CredentialsError`
  は `TachibanaError` 配下（#92 / ADR-0023 / findings 0087）。
