---
name: 3Dモードのカメラ視点を永続化
overview: 3Dモードのカメラ位置とOrbitControlsのtargetをLocalStorageに保存し、3Dモードを切り替えても視点が保持されるようにする
todos:
  - id: create-view-atom
    content: カメラ視点情報を管理するatomを作成（src/core/three/cell-3d-view.ts）。LocalStorageキー名は"marimo:3d:cameraView:v1"とする
    status: pending
  - id: add-set-camera-view
    content: SceneManagerにsetCameraViewメソッドを追加して、カメラ位置とtargetを設定できるようにする
    status: pending
  - id: restore-camera-view
    content: edit-app.tsxで、SceneManagerのinitialize後にatomから視点情報を復元
    status: pending
    dependencies:
      - create-view-atom
      - add-set-camera-view
  - id: save-camera-view
    content: edit-app.tsxで、OrbitControlsのendイベント（操作終了時）で視点情報をatomに保存。イベントリスナーのクリーンアップも実装
    status: pending
    dependencies:
      - create-view-atom
      - add-set-camera-view
