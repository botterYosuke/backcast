---
name: 3Dモード専用コントロールUI作成
overview: GridControlsコンポーネントを複製して3Dモード専用のコントロールUIを作成します。新しい3d-layoutディレクトリを作成し、既存のGridControlsの設定項目を全て含み、さらに3Dモード専用の設定項目（Grid Spacing X/Z/Y、Grid Plane Opacity、Snap to Grid）を追加したコントロールコンポーネントを実装します。
todos:
  - id: create-directory
    content: src/components/editor/renderers/3d-layout/ディレクトリを作成
    status: pending
  - id: create-types
    content: src/components/editor/renderers/3d-layout/types.tsを作成し、Grid3DConfig型を定義
    status: pending
    dependencies:
      - create-directory
  - id: create-controls
    content: src/components/editor/renderers/3d-layout/grid-3d-controls.tsxを作成し、GridControlsを複製してGrid3DControlsコンポーネントを実装
    status: pending
    dependencies:
      - create-types
---
