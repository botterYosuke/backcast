...
- [x] ✅ Electron環境の検出機能
  - `window.electronAPI` の存在確認で検出
  - IPC経由でサーバーURLを取得
- [x] ✅ `src/core/runtime/config.ts` の確認
  - `config.ts`は変更不要（`mount.tsx`で動的に設定）
  - 通常のWeb環境では従来通りの動作を維持

#### 3.2 mount.tsx の修正
- [x] ✅ Electron環境での初期化処理を追加
  - `initElectronRuntime()`関数を実装
  - `initStore()`関数内でElectron環境を検出して非同期初期化を開始
- [x] ✅ サーバーURLを `runtimeConfig` に設定
  - IPC経由でサーバーURLを取得して`runtimeConfigAtom`に設定
  - サーバーステータス変更を監視して動的に更新
- [x] ✅ サーバー接続失敗時のエラーハンドリング
  - 最大10回の再試行（500ms間隔）
  - エラー時もアプリを続行（RuntimeManagerが接続エラーを処理）

#### 3.3 エラーハンドリングの強化
- [x] ✅ サーバー未起動時の処理
  - サーバーステータスを監視して起動を待機
  - ローディング中はデフォルト設定でアプリを起動
- [x] ✅ 接続エラー時の再試行機能
  - 最大10回の再試行を実装（指数バックオフではなく固定間隔500ms）
  - サーバーステータス変更を監視して自動更新
- [x] ✅ ログ出力によるエラー情報の記録
  - Loggerを使用してデバッグ情報とエラーを記録
...
**完了項目**:
- ✅ Pythonランタイム検出機能（埋め込み/システムPython）
- ✅ 動的ポート割り当て（2718-2728）
- ✅ marimoサーバー起動・終了管理
- ✅ ヘルスチェック機能（`/healthz`エンドポイント）
- ✅ 自動再起動機能（最大3回、指数バックオフ）
- ✅ IPC通信実装（サーバーURL、ステータス、再起動、ログ）
- ✅ ログ管理機能（メモリ内保持）

**設計変更**:
- システムPythonをフォールバックとして使用（Phase 4で埋め込みPython対応予定）
- ログはメモリ内に保持（ファイル出力は既存のlogger.jsで対応）
- Windowsでのプロセス終了処理を特別対応（`SIGTERM`が使えないため）

**コードレビュー結果**:
- ✅ エラーハンドリング: 適切に実装されている
- ✅ ポート管理: 動的割り当てと可用性チェックが実装されている
- ✅ プロセス管理: 正常終了と強制終了の両方をサポート
- ✅ IPC通信: セキュアな実装（contextIsolation使用）
- ⚠️ 軽微な修正: 未使用変数`processRef`を削除

**動作確認**:
- Electronアプリ起動: ✅ 成功
- サーバー起動: 確認中（marimoのインストールが必要な可能性）

**次のステップ**:
- Phase 3: フロントエンド統合（runtimeConfigの動的設定）
...
**完了項目**:
- ✅ Electron環境の検出機能（`window.electronAPI`の存在確認）
- ✅ TypeScript型定義の追加（`src/custom.d.ts`に`ElectronAPI`インターフェースを定義）
- ✅ `initElectronRuntime()`関数の実装
  - IPC経由でサーバーURLを取得
  - サーバーステータスを監視して起動を待機（最大10回、500ms間隔）
  - サーバーステータス変更を監視して動的に`runtimeConfigAtom`を更新
- ✅ `mount.tsx`の`initStore()`関数内でElectron環境の初期化処理を追加
  - Electron環境を検出
  - 非同期で`initElectronRuntime()`を呼び出し
  - 初期値として`DEFAULT_RUNTIME_CONFIG`を設定（後で更新される）
- ✅ エラーハンドリングの実装
  - サーバーURL取得失敗時の再試行（最大10回）
  - エラー時もアプリを続行（RuntimeManagerが接続エラーを処理）

**設計決定**:
- `initStore()`は同期的に実行されるため、Electron環境の初期化は非同期でバックグラウンド実行
- サーバーURLが取得できない場合でも、デフォルト設定でアプリを起動し、RuntimeManagerが接続エラーを処理
- サーバーステータス変更リスナーは、アプリが終了するまで実行され続ける（明示的なクリーンアップは不要）

**実装のポイント**:
- TypeScriptの型安全性を確保するため、`window.electronAPI`の型定義を追加
- サーバー起動待機中もアプリを起動できるよう、初期値を設定してから非同期で更新
- エラーハンドリングはLoggerを使用して記録し、アプリのクラッシュを防止

**次のステップ**:
- Phase 4: Python環境のセットアップ
