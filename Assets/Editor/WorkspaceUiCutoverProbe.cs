...
public static class WorkspaceUiCutoverProbe
{
    ...
    static string Section3_BootCanonical()
    {
        ...
        if (editors == null || !editors.TryGetValue(WINDOW_ID, out var editor) || editor == null) return "U3: adopted editor missing";
        string path = editor.Document.CurrentPath;
        if (string.IsNullOrEmpty(path)) return "U3: boot did not bind the adopted editor to the canonical v19 marimo";
