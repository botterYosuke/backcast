...
public sealed class SettingsVenueSectionView
{
    ...
    // (button, enabled-predicate) — interactable refreshed each frame to follow the async venue poll.
    readonly List<(Button btn, Func<bool> enabled)> _items = new List<(Button, Func<bool>)>();
    ...
    public void Refresh()
    {
        foreach (var (btn, enabled) in _items)
            if (btn != null) btn.interactable = enabled();
    }

    Button MakeButton(RectTransform parent, string text, ref float y, float h, float gap, Action onClick)
    {
