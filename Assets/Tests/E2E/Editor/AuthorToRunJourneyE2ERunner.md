...
| JOURNEY-AUTHOR-02 | File→New（**観察セル種付き**・トヨタ universe・untitled） | `BackcastWorkspaceRoot.DoFileNew`→`_coordinator.New(ObserveSeedBody)`＋`SeedFreshDocumentDefaults()` | notebook 1 セルに観察 body 種付け・clean・region_002+ despawn・universe==[7203.TSE]＋トヨタ chart spawn・`_currentLayoutPath=""`・untitled（非 supplyable） | New を invoke し cell0.Body==ObserveSeedBody＋universe[7203.TSE]＋chart:7203.TSE＋untitled を assert | 自動(E2E済・Section1) | #169 [NewSeedsObserveNotebook](./NewSeedsObserveNotebookE2ERunner.md) NEWSEED-03 / [MenuBar](./MenuBarE2ERunner.md) MENU-02 |
