...
| pytest `test_replay_chart_watchable_playback.py` ✅ | `REPLAY-PLAY-01..06` | 6 | 6 | #182（Replay 実行中チャートが `bars_per_second` ペースで1本ずつ前進＝watchable playback・[findings 0131](../../../../docs/findings/0131-replay-watchable-playback-cursor.md)）。本番経路（`load_replay_data` cold-seed→`start_engine`→`apply_replay_event`）で `get_current_state` の per-instrument 系列を replay cursor（`rs.timestamp_ms`）にクリップ（`_mode=="replay"` gate）。01=RUNNING 中の単調追従、02=LOADED 全期間（#156 回帰なし）、03=完走全本＋observation break 据え置き、04=multi-instrument、05=Live/static は clip しない（mode guard pin）、06=post-run preview 再シードは全期間（D4 `_full_preview_iids` 免除・review H1 回帰ガード）。pure-Python・決定論 |
| [ChartNoDataE2ERunner](./ChartNoDataE2ERunner.md) ✅ | `CHART-NODATA-01..02` | 2 | 2 | #182 副次（空チャートの「データなし」表示＋1970 エポック軸抑制・[findings 0131](../../../../docs/findings/0131-replay-watchable-playback-cursor.md)）。01=新規 ChartView は `NoDataShown`＋active 時間ラベル 0、02=系列→空 Render で stale 軸を 0 にクリア＋marker 復帰（precondition 床で非空虚）。実画素/グリフ目視は HITL |
