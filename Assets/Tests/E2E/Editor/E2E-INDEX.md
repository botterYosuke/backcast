...
| pytest `test_kabu_live_universe_churn.py` ✅ | `LIVEUNIV-01..05` | 5 | 5 | #141-145 + findings 0117（live 運用中の universe add/remove を **実 prod kabu キャプチャ**で回帰・[findings 0118](../../../../docs/findings/0118-live-universe-churn-kabu-mock.md) / ADR-0031 S4/S5）。実 marimo cell（`universe_churn_cell.py`）を LiveAuto 経路（controller→driver→LiveCellBridge）で駆動し、`KabuPushFrameProcessor` で decode した実 kabu フレームを subscribe-gated `MockVenueAdapter` に流す最中に `bt.universe.add/remove`。01=mid-stream ADD が live data を産む（subscribe-gate で構造的非空虚）、02=REMOVE が venue 購読を畳む（churn 後 probe で削除 id drop・survivor 流通）、03=最終 membership 一致（runner ∧ engine mirror）、04=no crash/clean teardown、05=reducer per_id 一貫。pure-Python・決定論・RED litmus=bridge enqueue 削除で per-id RED |
