...
> Abort / clean 素通し / valid-marimo も確認必須）を観測する。集約 `Open` の #86 F1 dirty-refuse とその discard 緩和
> シームは [StrategyEditorNotebookE2ERunner](./StrategyEditorNotebookE2ERunner.md) `S10` が正本。
...
6. dirty で File→Open → Discard すると選択 `.py` を開く（discardDirty:true が #86 F1 を緩和）。
...
- **discardDirty seam**（slice 2・findings 0069）: `NotebookCellCoordinator.Open(path, positions, discardDirty)`→
  `MarimoNotebookDocument.Open(path, discardDirty)`。`discardDirty:true` は SaveGuard「Discard」判定を得た
  `DoFileOpen` だけが渡し、#86 F1 の dirty-refuse を緩めて非 marimo `.py` の 1-cell wrap で `_cells` を置換する。
- 本 Runner は **全行 Python-FREE**（File 操作も SaveGuard も host 非依存・modeReq は disconnected で null）。
  `FakeMarimoSynthesizer` を注入（`FailDecompose=true` で非 marimo 経路＝#86 F1 を踏む）。
...
| FILEGUARD-06 | dirty File→Open→Discard（非marimo） | `BackcastWorkspaceRoot.cs:1660` `DoFileOpen`→`1679 _coordinator.Open(..., discardDirty:true)` | discardDirty:true が #86 F1 を緩和・1-cell wrap で buffer 置換 | Discard 後 bound==選択 `.py`∧WrapMode∧本文==file を assert（discardDirty 無しなら F1 refuse で FAIL） | 自動(E2E済) | [StrategyEditorNotebook](./StrategyEditorNotebookE2ERunner.md) S10, [FileOpenNonMarimo](./FileOpenNonMarimoE2ERunner.md) |
...
- **discardDirty:true が #86 F1 を緩和**して非 marimo `.py` の dirty 置換を可能にする（FILEGUARD-06・slice 2 シームの上位配線）。
