...
| DOCK-01 | dock kind（chart + 4 base singleton）が catalog に存在 | `FloatingWindowCatalog.Default`（#99 / findings 0075 §2・**ADR-0026 で KIND_STARTUP 退役**） | chart は multi-instance、buying_power/orders/positions/run_result は singleton、各 accent・defaultSize≥minSize、unknown-kind tolerance 不変 | 5 dock kind の resolve・size 健全性・既存 kind 非退行・unknown tolerance を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S12） |
...
| PLANE-01 | パンで奥（dock 5 種＝chart+4 base・ADR-0026 で startup 退役）と手前（エディタ/発注）に速度差が出る＝奥行き | `BackcastWorkspaceRoot.BuildWorkspace`（`_dockLayer` 1.0× ／ `_floatingLayer` 1.2×）→`CanvasViewMath.ParallaxLayerOffset`（#103 / ADR-0018 / findings 0075 §10） | DockLayer は背面 sibling（Content の早い子）、factor 1.0=offset 0（Content を 1× で乗る）、floating は factor 1.2=offset (1-1.2)·pan、同一論理 top-left の 2 窓がパン後に異なる viewport 位置へ（engine==math 合成） | 背面 sibling・`ParallaxLayerOffset(1.0)=0`/`(1.2)≠0`・dock 1× 追従・floating 1.2× 追従・速度差≠0 を assert（合成スタック=S16）＋**実シーン**で DockLayer が FloatingWindowLayer の背面 sibling・`_dockLayer`/`_floatingLayer` serialized 参照を検証（S19） | 自動(E2E済) | `FloatingWindowE2ERunner`（S16／S19） |
...
| PLANE-03 | 保存/復元が両プレーンのウィンドウを kind でルーティングして round-trip | `BackcastWorkspaceRoot.CaptureLayout`（両 controller union）／`RestoreFloating`（`DockShape.IsDockKind` で plane 振り分け）（#103 / ADR-0018 / findings 0075 §10） | 両 controller を 1 つの `floatingWindows` に union → disk → 復元時に dock 5 種→奥 layer・order→手前 layer へ振り分け、hidden run_result は hidden のまま（ADR-0026 で startup 退役・S18 は run_result で round-trip）、cross-plane leak なし、スキーマ追加 0 | capture union（4 窓）→disk→kind ルーティング restore で各窓が正しい layer に親付け・hidden 保持・leak なしを assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S18） |
...
| DRAG-01..04 | gesture-channel 判定＋ドロップ先 outcome 分類（pure 算術・**ADR-0029 で cursor-per-frame 3 mode 退役**） | `FloatingWindowMath.ResolveChannel` / `ResolveDropOutcome`（ADR-0029 §3/§4 / findings 0106 §1, §3） | **ResolveChannel**: eject/Alt ⇒ SingleWindowPickup・plain ⇒ IslandMove（gesture 開始で確定）。**ResolveDropOutcome**: cursor が同 island メンバー rect 内 ⇒ Swap（**距離不問**・距離トリガ撤廃）／picked rect が別 island に磁石 flush ⇒ MergeToIsland／空き地 ⇒ Detach。swap が flush に優先 | ResolveChannel 真理値表・swap 距離不問・flush merge・空き地 detach・swap 優先・center-in-rect 出入りを assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S24） |
...
| DRAG-17 | eject つまみ アフォーダンス（常時可視 "⤴"・第 2 raycast target・drag handler 無し） | `FloatingWindowEjectHandle.Attach` ／ `FloatingWindowTitleInput.Awake`（ADR-0029 §3 / findings 0106 §1） | 各 title-bar に常時可視の "⤴" つまみ（chip Image=raycast target・glyph Text=非 raycast・最後 sibling）、drag handler を持たないので press は title input へ bubble、`ResolveChannel` で eject/Alt ⇒ SingleWindowPickup・plain ⇒ IslandMove | bare title-bar に `Attach` → 名前/active/raycast target/no-drag-handler/last-sibling/glyph 非 block・ResolveChannel reachability を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S41） |
...
| DRAG-20-HITL | 掴む対象でモード固定ドラッグの体感（owner 不満①②の解消確認・#136 S6） | 実 playmode（ADR-0029 / findings 0106 §HITL） | 島の遠距離移動が引っかからずできる（不満①）、ドラッグ中に mode が化けない（不満②）、eject つまみの発見性・押しやすさ・Alt-drag の操作感、swap の `ぷるん` 局所 reflow の収まり（spring 200ms / 8%）と残隙間の許容、ESC が各チャンネルで効く | — | HITL専用（実ピクセル＋実マウス／実キー。owner playmode・findings 0106 §HITL） | — |
