...
| GROUP-14 | 初回起動（保存レイアウト無し）で base dock cluster を 1 つの Hakoniwa group に束ね、flush（くっついた/docked）に配置する（工場出荷値） | `FloatingWindowController.FormGroup` ／ `BackcastWorkspaceRoot.FormFactoryBaseGroup` ＋ `DockDefaultPlacement.ComputeFlushRects`（no-resume 分岐のみ）（#105 / ADR-0019 D8 amendment / ADR-0020 / findings 0082 §12 / findings 0083） | FormGroup が member 全員に 1 つの非 null groupId を stamp、programmatic Spawn は依然 mint しない（S20 不変）、cluster は startup + run_result core を含むので Hakoniwa group（core drag = HakoniwaCoreLock で freeze・全体 translate 禁止）、live member < 2 なら group 化しない、初回配置は gap=0 で隣接タイルが flush（隙間なし）、saved layout 復元時は本経路を通らず RestoreFloating が doc の groupId を尊重 | bare-stack controller に 5 base 窓を spawn → FormGroup → 共有 groupId・Hakoniwa drag mode・<2 no-group・ComputeFlushRects の隣接 flush（gapped は非 flush）を assert（S32） | 自動(E2E済) | `FloatingWindowE2ERunner`（S32） |
