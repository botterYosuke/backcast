...
floating window ではない（Hakoniwa tile）~~は pre-ADR-0017 の旧記述で退役**——`chart:<id>` は ADR-0017 以降 **multi-instance な
floating window kind**（DOCK-01／DOCK-04／DRAG-13 が正本・他 window と同等のドラッグルール）。z-order = layer 内 sibling index（後の sibling ほど前面）、persist は
...
実感は HITL。resize（右下つまみ・島内 flush 追従押し出し）は **#139・ADR-0030 で実装**＝別 runner
[`FloatingWindowResizeE2ERunner`](./FloatingWindowResizeE2ERunner.md)（RESIZE-01..12 自動・RESIZE-13 HITL）が正本（WINDOW-12 行参照）。
...
| WINDOW-12 | window 右下つまみを drag してリサイズ（島内 flush 追従押し出し） | `FloatingWindowResizeHandle.Attach`／`FloatingWindowResizeGrip`／`FloatingWindowController.ResizeApply`／`FloatingWindowMath.ResizeIslandPush`（#139・ADR-0030） | 左上固定・右下伸縮、本文自動伸縮、島内 flush 追従押し出し（対称・連鎖・island scope）、min clamp、ESC revert、persist round-trip | **別 runner `FloatingWindowResizeE2ERunner`（RESIZE-01..12）で自動化済**（実つまみジェスチャ発火は RESIZE-13 HITL） | 自動(E2E済・FloatingWindowResizeE2ERunner) | `FloatingWindowResizeE2ERunner` |
...
| DOCK-01 | dock kind（chart + run_result）が catalog に存在し、**退役 3 kind は解決されない** | `FloatingWindowCatalog.Default`（#99 / findings 0075 §2・**ADR-0026 で KIND_STARTUP 退役**・**ADR-0038 で buying_power/orders/positions 退役**） | chart は multi-instance、run_result は singleton、各 accent・defaultSize≥minSize、unknown-kind tolerance 不変、退役 buying_power/orders/positions は `TryGet=false`（forward-compat skip） | 2 dock kind の resolve・size 健全性・既存 kind 非退行・unknown tolerance・**退役 3 kind の非解決（S12d）**を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S12） |
| DOCK-02 | base+chart の初回（リセット）グリッド配置（pure 算術） | `DockDefaultPlacement.ComputeRects`（#99 / findings 0075 §4） | `ceil(√n)` グリッドの絶対 canvas 論理 rect、row-major slot 0=top-left、y up-positive、重なり無し、n≤0→空（**任意 n の純算術**＝base クラスタは ADR-0026 後 4 窓だが ComputeRects は kind 非依存） | n=0/5/4/1 の slot 位置・cell サイズ・非重複を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S13） |
...
| PLANE-01 | パンで奥（dock＝chart+run_result・ADR-0026 で startup・ADR-0038 で 3 base 退役）と手前（エディタ/発注）に速度差が出る＝奥行き | `BackcastWorkspaceRoot.BuildWorkspace`（`_dockLayer` 1.0× ／ `_floatingLayer` 1.2×）→`CanvasViewMath.ParallaxLayerOffset`（#103 / ADR-0018 / findings 0075 §10） | DockLayer は背面 sibling（Content の早い子）、factor 1.0=offset 0（Content を 1× で乗る）、floating は factor 1.2=offset (1-1.2)·pan、同一論理 top-left の 2 窓がパン後に異なる viewport 位置へ（engine==math 合成） | 背面 sibling・`ParallaxLayerOffset(1.0)=0`/`(1.2)≠0`・dock 1× 追従・floating 1.2× 追従・速度差≠0 を assert（合成スタック=S16）＋**実シーン**で DockLayer が FloatingWindowLayer の背面 sibling・`_dockLayer`/`_floatingLayer` serialized 参照を検証（S19） | 自動(E2E済) | `FloatingWindowE2ERunner`（S16／S19） |
...
| PLANE-03 | 保存/復元が両プレーンのウィンドウを kind でルーティングして round-trip | `BackcastWorkspaceRoot.CaptureLayout`（両 controller union）／`RestoreFloating`（`DockShape.IsDockKind` で plane 振り分け）（#103 / ADR-0018 / findings 0075 §10） | 両 controller を 1 つの `floatingWindows` に union → disk → 復元時に dock kind→奥 layer・order→手前 layer へ振り分け、hidden 窓は hidden のまま（ADR-0026 で startup・ADR-0038 で 3 base 退役・S18 は run_result kind の dock 窓で round-trip）、cross-plane leak なし、スキーマ追加 0 | capture union（4 窓）→disk→kind ルーティング restore で各窓が正しい layer に親付け・hidden 保持・leak なしを assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S18） |
| PLANE-04 | パンで奥行きが見える・またぎ吸着しない・mode 切替・persist round-trip の実感（目視） | `BackcastWorkspaceRoot`（実 playmode・シーン再ビルド後） | パンで奥 1.0× と手前 1.2× の速度差が視認できる、奥パネルと手前エディタは近づけてもくっつかない、保存→再起動で両プレーンが復元（ADR-0026 で startup dock window 退役＝Replay/Live の base 差分なし・旧 startup show/hide は消滅） | — | HITL専用（実ピクセル＋実パン＋実レイアウト保存。ADR-0018 / findings 0075 §10） | — |
...
| DRAG-01..04 | gesture-channel 判定＋ドロップ先 outcome 分類（pure 算術・**ADR-0029 で cursor-per-frame 3 mode 退役・ADR-0032 で eject つまみ起動退役**） | `FloatingWindowMath.ResolveChannel` / `ResolveDropOutcome`（ADR-0029 §3/§4 / ADR-0032 / findings 0106 §1, §3 / findings 0113 §1） | **ResolveChannel**: Alt ⇒ SingleWindowPickup・plain ⇒ IslandMove（gesture 開始で確定・**ADR-0032 で eject つまみ退役＝Alt 単独起動**）。**ResolveDropOutcome**: cursor が同 island メンバー rect 内 ⇒ Swap（**距離不問**・距離トリガ撤廃）／picked rect が別 island に磁石 flush ⇒ MergeToIsland／空き地 ⇒ Detach。swap が flush に優先 | ResolveChannel 真理値表（Alt 単軸 2 行）・swap 距離不問・flush merge・空き地 detach・swap 優先・center-in-rect 出入りを assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S24） |
| DRAG-02/03/16 | IslandMove（**無制限距離・detach なし**）vs SingleWindowPickup carry 実描画（DragApplyDelta 絶対オフセット） | `FloatingWindowController.DragApplyDelta`（ADR-0029 §2/§3 / findings 0106 §2, §3） | IslandMove=島全員が rest+offset で実描画シフト・**256px 超でも detach せず一体移動**、SingleWindowPickup=picked のみ実描画・siblings rest、channel は BeginDrag で固定し drag 中化けない、groupId は drag 中不変、singleton | IslandMove 移動・**無制限距離**・pickup carry・channel 不変・singleton を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S25） |
| DRAG-15 | gesture-channel lock（mode は OnBeginDrag で確定し drag 中化けない・ADR-0029 不満②の根治） | `FloatingWindowController.BeginDrag(id,start,channel)` / `DragSession.channel`（ADR-0029 §1 / findings 0106 §1） | channel は BeginDrag で 1 度確定し DragApplyDelta/ReleaseDrag が drag 中再評価しない（cursor 位置/距離で化けない）、IslandMove は 256px 超でも IslandMove のまま・SingleWindowPickup は cursor がどこでも pickup のまま | DragApplyDelta が返す channel が begin の channel と一致し続ける（S24 ResolveChannel + S25 unlimited/carry で固定）を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S24/S25） |
| DRAG-08 | **pickup** detach release commit（空き地 drop）+ 連鎖 dissolve helper + Close cascade + Hide 温存 | `FloatingWindowController.ReleaseDrag` / `CommitPickup` / `DissolveIfShrunkTo` / `Close` / `Hide`（ADR-0029 §4 / findings 0106 §3 / findings 0082 §5, §10） | SingleWindowPickup で空き地 drop ⇒ `dragged.groupId=null` + 残 visible ≥2 → 維持・残 1 → 連鎖 dissolve、core 含み island も同じ（detach は pickup 限定・IslandMove では起きない）、Close も同 helper、Hide は groupId 温存、helper 直接呼びは threshold で dissolve | 7 ケースを assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S26） |
| DRAG-11 | Hakoniwa special **退役**（core-bearing island も island-move / pickup-detach 可能） | `FloatingWindowController.DragApplyDelta` / `ReleaseDrag`（ADR-0024 §1 / ADR-0029 §1 / findings 0106 §1） | startup + run_result の island で startup を plain drag → island-move（一体移動・無制限・旧 translate-ban 退役）、core member を pickup で空き地 drop → Detached（旧 core-lock 退役）→ groupId=null + remnant dissolve | 2 ケース（core island-move / core pickup-detach）を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S27） |
...
| DRAG-01/18 | swap = **サイズ維持＋島内局所 reflow**（**ADR-0029 で 4 値交換退役**） | `FloatingWindowMath.ReflowIslandAfterSwap` / `FloatingWindowController.CommitSwapReflow`（ADR-0029 §6 / findings 0106 §4） | pickup で sibling 上に drop: A/B は各自の `(w,h)` を保持し **anchor だけ交換**（4 値交換しない）、同じ island のメンバーだけが flush へ re-snap（`ぷるん`）、reflow scope は island に厳密限定、残隙間は許容、kind/id/groupId 不変 | pure reflow（size 維持・anchor 交換・re-snap・island scope・残隙間）＋ wiring（picked が OWN size 保持）を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S29） |
...
| DRAG-14/19 | drag ghost は **post-swap+reflow 後の島レイアウト予告**（pickup の swap 候補専用） | `DragGhostLayer.Render` ／ `FloatingWindowController.ComposeReflowGhosts`（ADR-0029 §5 / findings 0106 §3, §5） | pickup で sibling 上: picked SOLID at swapped anchor（**OWN size 維持**）+ reflow で動く隣窓 DASHED、IslandMove=0 枚、pickup 空き地=0 枚、container は最後 sibling（前面）、Clear で 0、ReleaseDrag で commit-on-release clear、ALPHA=0.45 | bare-stack DragGhostLayer で reflow preview（picked solid size 維持 + neighbour dashed）/ islandmove 0 / pickup-detach 0 / sibling / Clear / commit-on-release を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S31） |
| DRAG-14 | FormGroup の **MECHANICS**（form / core-detach / flush 配置）— ADR-0020 工場グループの土台 | `FloatingWindowController.FormGroup` ＋ `DockDefaultPlacement.ComputeFlushRects`（#105 / ADR-0020 / ADR-0024 §1 / findings 0083） | FormGroup が member 全員に 1 つの非 null groupId を stamp、programmatic Spawn は依然 mint しない、cluster は **plain island**（core 含むが特別扱い無し＝core も pickup で自由に detach）、live member < 2 なら group 化しない、初回配置は gap=0 で flush。**ADR-0038 注**: production の base は run_result のみ（1 窓）に縮退（FormGroup は live<2 で no-op＝工場グループは自然に形成されない）。S32 は base 数から **decouple** され、代表 4 窓（run_result kind）で FormGroup 力学を gate する | 代表 4 窓 spawn → FormGroup → 共有 groupId・core **pickup-detach** 可能・<2 no-group・ComputeFlushRects(4) flush を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S32） |
...
| DRAG-05/06 | in-drag 磁石吸着の実描画（controller wiring・stickiness） | `FloatingWindowController.DragApplyDelta`（RenderTranslate / RenderDetach）（ADR-0029 §2/§3 / findings 0106 §2） | IslandMove: island outer edge が外部 window と R_SNAP 内 → 実窓が flush へ snap・cursor が圏内なら貼り付く（stickiness）・圏外で解放、Pickup: picked の edge が R_SNAP 内 → A 単独 flush snap | IslandMove snap+stickiness+解放・Pickup snap を実 rect で assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S36） |
| DRAG-07 | release-position merge（controller wiring・IslandMove Q3 & Pickup） | `FloatingWindowController.CommitTranslate` / `CommitPickup`（ADR-0029 §4/§5 / findings 0106 §3） | IslandMove release で別 island Y に overlap/flush → ResolveNearestFlush で snap → merge（cascade・owner Q3）、Pickup release で別 island Y に flush → A を flush snap → singleton 経由で Y に編入、empty + 偶発 flush は ADR-0019 D4 で IslandMove attach | IslandMove-overlap-merge・Pickup-flush-merge・D4 incidental attach を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S37） |
...
| DRAG-13 | chart:<id> が他 window と同等のドラッグルール | `FloatingWindowController`（chart kind は特別扱いなし）（ADR-0024 §8 / ADR-0029 / findings 0106） | chart spawn は groupId=null、base 窓（run_result kind・ADR-0026 で startup・ADR-0038 で buying_power/orders/positions 退役）と flush attach で island 化、island-move（一体移動・無制限）、pickup で空き地 drop で detach（remnant dissolve）＝swap/island-move/pickup-detach/merge 全部適用 | spawn=null・flush attach・island-move・pickup-detach を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S40） |
| ~~DRAG-17~~ | ~~eject つまみ アフォーダンス（常時可視 "⤴"）~~ **退役（[[ADR-0032]] / findings 0113）** | ~~`FloatingWindowEjectHandle`~~（削除済） | **廃止**＝owner HITL で「見た目の雑音・Alt+drag で十分」と判断。SingleWindowPickup の起動は `Alt`+drag 単独になり、見える起動アフォーダンスは無い（Section41 は退役） | — | 対象外（ADR-0032 で機能退役） | — |
| DRAG-10-HITL | "プルン" spring の体感（実 200ms / overshoot 8% / ESC revert / 磁石吸着の felt） | `RectSpringDriver`（実 Update tween）／実 playmode（ADR-0024 §3 / ADR-0029 §8 / findings 0088 §3） | 実 spring の見た目（200ms・8% overshoot・1 回弾む）、ESC で rest へ戻る felt、磁石吸着の "くっつく" 体感、swap reflow ghost の視認性 | — | HITL専用（実フレーム tween ＋実マウス／実キー。findings 0088 §3） | — |
| DRAG-20-HITL | 掴む対象でモード固定ドラッグの体感（owner 不満①②の解消確認・#136 S6 / ADR-0032 eject 廃止確認） | 実 playmode（ADR-0029 / ADR-0032 / findings 0106 §HITL / findings 0113 §HITL） | 島の遠距離移動が引っかからずできる（不満①）、ドラッグ中に mode が化けない（不満②）、**title-bar から "⤴" チップが消え whiteboard が清潔**（ADR-0032）・`Alt`-drag で単窓ピックアップが起動する操作感、swap の `ぷるん` 局所 reflow の収まり（spring 200ms / 8%）と残隙間の許容、ESC が各チャンネルで効く | — | HITL専用（実ピクセル＋実マウス／実キー。owner playmode・findings 0106/0113 §HITL） | — |

> ~~**chart を floating window と呼ばない**（Hakoniwa tile が正・`dispatcher.rs` が `PanelKind::Chart` spawn を拒否）~~
> **退役（pre-ADR-0017）**: `chart:<id>` は今や multi-instance floating window kind（DOCK-01／DRAG-13）。
