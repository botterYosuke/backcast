...
| DOCK-01 | dock kind が `chart` のみ・退役 base kind は非解決 | `FloatingWindowCatalog.Default`（**ADR-0026 startup・ADR-0037 run_result・ADR-0038 buying_power/orders/positions を全退役**） | dock kind は `chart`（multi-instance）のみ、defaultSize≥minSize、unknown-kind tolerance 不変、退役 5 kind は `TryGet=false`（forward-compat skip） | chart の resolve・size 健全性・既存 editor/order 非退行・unknown tolerance・**退役 kind の非解決（S12d）**を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S12） |
| DOCK-02 | base+chart の初回（リセット）グリッド配置（pure 算術） | `DockDefaultPlacement.ComputeRects`（#99 / findings 0075 §4） | `ceil(√n)` グリッドの絶対 canvas 論理 rect、row-major slot 0=top-left、y up-positive、重なり無し、n≤0→空（**任意 n の純算術**＝base クラスタは ADR-0026/ADR-0037 後 3 窓だが ComputeRects は kind 非依存） | n=0/5/4/1 の slot 位置・cell サイズ・非重複を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S13） |
...
| PLANE-01 | パンで奥（dock=chart のみ（startup ADR-0026・run_result ADR-0037・buying_power/orders/positions ADR-0038 を全退役））と手前（エディタ/発注）に速度差が出る＝奥行き | `BackcastWorkspaceRoot.BuildWorkspace`（`_dockLayer` 1.0× ／ `_floatingLayer` 1.2×）→`CanvasViewMath.ParallaxLayerOffset`（#103 / ADR-0018 / findings 0075 §10） | DockLayer は背面 sibling（Content の早い子）、factor 1.0=offset 0（Content を 1× で乗る）、floating は factor 1.2=offset (1-1.2)·pan、同一論理 top-left の 2 窓がパン後に異なる viewport 位置へ（engine==math 合成） | 背面 sibling・`ParallaxLayerOffset(1.0)=0`/`(1.2)≠0`・dock 1× 追従・floating 1.2× 追従・速度差≠0 を assert（合成スタック=S16）＋**実シーン**で DockLayer が FloatingWindowLayer の背面 sibling・`_dockLayer`/`_floatingLayer` serialized 参照を検証（S19） | 自動(E2E済) | `FloatingWindowE2ERunner`（S16／S19） |
...
| PLANE-03 | 保存/復元が両プレーンのウィンドウを kind でルーティングして round-trip | `BackcastWorkspaceRoot.CaptureLayout`（両 controller union）／`RestoreFloating`（`DockShape.IsDockKind` で plane 振り分け）（#103 / ADR-0018 / findings 0075 §10） | 両 controller を 1 つの `floatingWindows` に union → disk → 復元時に dock kind（=chart のみ）→奥 layer・order→手前 layer へ振り分け、hidden な dock 窓は hidden のまま（startup ADR-0026・run_result ADR-0037・buying_power/orders/positions ADR-0038 を全退役＝S18 は 3 chart 窓で round-trip）、cross-plane leak なし、スキーマ追加 0 | capture union→disk→kind ルーティング restore で各窓が正しい layer に親付け・hidden 保持・leak なしを assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S18） |
...
| DRAG-14 | 名前付き id 群を 1 つの **plain island** に束ねる group 機構＋flush 配置（ADR-0020・現在は saved-layout group 復元が利用） | `FloatingWindowController.FormGroup` ＋ `DockDefaultPlacement.ComputeFlushRects`（#105 / ADR-0020 / ADR-0024 §1 / findings 0083。初回起動の factory caller `FormFactoryBaseGroup` は ADR-0038 で退役＝base 集合が空のため削除） | FormGroup が member 全員に 1 つの非 null groupId を stamp、programmatic Spawn は依然 mint しない、island は **plain**（core 含むが特別扱い無し＝core も pickup で自由に detach）、live member < 2 なら group 化しない、配置は gap=0 で flush、saved layout は RestoreFloating の groupId 尊重 | 代表 3 窓 spawn → FormGroup → 共有 groupId・pickup-detach 可能・<2 no-group・ComputeFlushRects(3) flush を assert | 自動(E2E済) | `FloatingWindowE2ERunner`（S32） |
