...
`FooterModeE2ERunner.cs`（第二波 2本目・実装済み・37 Check AFK GREEN＝#127/ADR-0026 で view section を `SettingsModeSegmentView` へ retarget・FOOTER-13 反転後）が自動検証する **ワークスペース footer（実行モード）サーフェス**の台本。
...
実行モードセグメント（頭脳 `FooterModeViewModel` 不変、findings 0025 §3・#76 S6b-β-clean U4）。**#127/ADR-0026 で
セグメントは `WorkspaceFooterView` から Settings モーダルの Mode セクション（`SettingsModeSegmentView`）へ移設**——footer は
mode **ステータス表示専用**に縮退（現在モード / `switching…` / `LiveAuto:<runId>`・ボタン 0）。Replay / Manual / Auto の
3 セグメント。ExecutionMode は engine 正準の
...
| FOOTER-06 | lock 中のセグメント再クリック抑止（**#127: セグメントは Settings の Mode セクション**） | `SettingsModeSegmentView.Refresh`（`btn.interactable=!locked`） | `Locked` 中は全セグメント `interactable=false`＋alpha 0.35（2nd クリックが engine 答えと race しない）。`Locked` 判定は VM 不変 | `SettingsModeSegmentView` を build し VM の `Locked`→`interactable` を assert | 自動(E2E済) | `FooterModeE2ERunner`（view section retarget） |
| FOOTER-07 | Manual/Auto セグメントの venue-gated 可視性（**Settings の Mode セクション**） | `SettingsModeSegmentView.Refresh`→`FooterModeViewModel.ShowManualAutoSegments` | `ShowReplaySegment` 常時 true、`ShowManualAutoSegments=VenueLive`。venue down で Manual/Auto を `SetActive(false)` | `SettingsModeSegmentView` を build し可視性を assert | 自動(E2E済) | `FooterModeE2ERunner`（view section retarget） |
...
| FOOTER-13 | footer は mode ステータス表示専用＝**ボタン 0**（#127/ADR-0026: mode セグメントは Settings へ移設・transport は #76 で退役） | `WorkspaceFooterView.Build`（status `Text` のみ） | footer `bar` 配下に Button が一切無いこと（旧 transport `btn:▶..` も mode `btn:Replay/Manual/Auto` も無い）。非空虚化: `SettingsModeSegmentView` がセグメントを持つことを先に assert | footer の `GetComponentsInChildren<Button>` が空・`SettingsModeSegmentView` にセグメント在を assert（`WorkspaceFooterView.Build` に AddModeSeg を戻すと RED） | 自動(E2E済) | `FooterModeE2ERunner`（findings 0102 で ADR-0026 re-home） |
...
- **FOOTER-06/07 の view 反映を薄い uGUI section で追加**（#127/ADR-0026 で `SettingsModeSegmentView` へ retarget）:
  `SettingsModeSegmentView` を build し、segment の `Button.interactable`（lock 中 false）と Manual/Auto の
  `gameObject.activeSelf`（venue-gated）を反射確認。footer の `_modeSegs` は退役済みなので footer 側では assert しない。
  teardown は `DestroyImmediate`。root 合成も Python も不要。
