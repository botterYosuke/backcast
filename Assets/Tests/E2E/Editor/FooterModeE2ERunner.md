...
| FOOTER-13 | footer に退役した replay-transport コントロールが無いこと（#76 S6b-β-clean／Phase 6 global ▶ Run sunset の cutover 負 invariant・RunButton retire から U4 re-home） | `WorkspaceFooterView.Build`（MakeButton が `btn:`+label 命名） | footer `bar` 配下に `btn:▶/⏸/⏭/⏹/1x..50x` が一切無いこと（3 mode セグメント存在を non-vacuity pin した上で） | re-homed U4 section が transport ボタン不在を assert（`WorkspaceFooterView.Build` に `▶` を再追加で RED） | 自動(E2E済) | `FooterModeE2ERunner`（findings 0063→0075 §3c re-home） |
