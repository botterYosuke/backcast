...
| HAKONIWA-15 | hakoniwa の入力ディスパッチャ（RawImage への screen-press → RT pixel → slot → header=swap / body=pan の実配線） | `HakoniwaStageInputSurface`（`IBeginDrag`/`IDrag`/`IEndDrag`）←`_hakoniwaRawImage` ／ `HakoniwaController.RouteNormalized` | header-band press で source slot 捕捉、別 cell へ drag END で `Swap` 発火・`_order` 入替、body press は pan へ fall-through（Swap 非発火）、screen→RawImage-local→RT pixel 変換が決定的に正しい。EventSystem 経路（実 screen-press）は HITL（HAKONIWA-11b） | `HakoniwaStageInputProbe` §①②③④（合成 `PointerEventData` を handler へ直接注入・EventSystem bypass）RED→GREEN（findings 0068 §17） | 自動(Probe有・要昇格)（issue #93 入力実配線 gate。E2ERunner 昇格は `HakoniwaInputRoutingE2ERunner`） | `HakoniwaStageInputProbe` §1-4 / findings 0068 §17 |
| HAKONIWA-16 | runtime spawn される chart:<id> / BuyingPower タイルが Hakoniwa layer に乗る（perspective camera の RT に描画される・CANVAS-CULL 前提を保つ） | `BackcastWorkspaceRoot.BuildTileShell`（`go.layer = _hakoniwaRoot.gameObject.layer`） | spawn 後の tile root layer == `_hakoniwaRoot.layer`、Stage canvas root layer == Hakoniwa、tile subtree に nested Canvas 不在（別 cull root を作らない・findings 0068 §18 CANVAS-CULL） | `HakoniwaStageInputProbe` §⑤（実 root を reflection 合成し `Universe.ReplaceAll` で chart tile spawn 後 layer / nested-Canvas を assert）RED→GREEN | 自動(Probe有) RED→GREEN | `HakoniwaStageInputProbe` §5 / findings 0068 §18/§19 |
