...
// （台本: 同ディレクトリ KabuLiveProdE2ERunner.md / 方針: ADR-0027 / findings 0106）。
//
...
// （findings 0106）。本番口座で発注/購読を走らせないため、検証は login→CONNECTED の確立までに限定する
...
public static class KabuLiveProdE2ERunner
{
    ...
    const string ENV_HINT = "prod";              // 本番 18080。意図的に prod を叩く唯一の runner（findings 0106）
    const string ENV_PASSWORD = "PROD_KABU_API_PASSWORD";  // findings 0106 / D2: prod は verify と別パスワード
    ...
    static string Execute(out bool skipped)
    {
        ...
        // ── step 0: prod creds（API パスワードのみ・findings 0106 / D2）。 ──
