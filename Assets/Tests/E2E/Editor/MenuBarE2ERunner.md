...
グローバルメニューバー（`MenuBarView` ＋ 頭脳 `MenuBarViewModel`）。File / Edit / Venue / Help の 4 トップ
レベル（TTWR 1:1 parity・ADR-0005）。Venue サブメニューは `VenueMenuViewModel`（#21）を **再利用**（再実装しない）。
File は **レイアウト/ドキュメントの I/O**（戦略 `.py` 本体の編集は Strategy Editor サーフェス）。
...
| MENU-10 | Venue メニューを開閉 | `MenuBarView.cs:115`→`Toggle` | `_open` が `Venue`⇄`None`、各 Connect 項目の `interactable` が接続状態に追従 | 反射で `_open`＋`_venueItems` の enabled を assert | 要新規自動化 | — |
| MENU-11 | Venue → Connect MOCK（dev・Editor 限定） | `MenuBarView.cs:208,216` | `_onConnect("MOCK","")` 発火、Python 起動・接続 | mock venue で接続 ACK を assert（実 venue 不要） | 自動(Probe有・要昇格) | `VenueMenuM3Probe` `VenueLoginSecretProbe` |
| MENU-12 | Venue → Connect（4 parity variant・prod grey-out） | `MenuBarView.cs:209,220` | prod は `*_ALLOW_PROD` 未設定で `interactable=false`、demo/verify は接続可 | `VenueMenuViewModel` の gate を assert（接続自体は HITL） | 自動(Probe有・要昇格) | `MenuBarVerify` |
| MENU-13 | Venue → Connect（実 venue へ接続） | `MenuBarView.cs:216` | 実 kabu/立花 へログイン、secret modal 連動、badge が `CONNECTED` | — | HITL専用（実 venue 接続・外部認証/秘密情報依存） | `VenueLoginSecretHitlMenu` |
| MENU-14 | Venue → Disconnect | `MenuBarView.cs:222` | `venue.CanDisconnect` のとき `_onDisconnect` 発火、badge が `DISCONNECTED` へ収束 | gate は自動、切断 RPC は mock で assert | 自動(Probe有・要昇格) | `VenueMenuM3Probe` |
