...
| JOURNEY-NBHAKO-05 | 走行中スナップショット bar1 → Hakoniwa の 4 base tile が実数表示（D6/D8） | `WorkspaceEngineHost.cs:132/139`（override）→`BackcastWorkspaceRoot.cs:970/1012`→`PushReplayTiles`→`ReplayPanelDecoder.DecodePortfolio`→`FormatReplay*`→`LivePanelTileView.ShowText` | `TestPortfolioJsonOverride`=snap1 → `RefreshLiveTiles` → positions tile が建玉行・orders tile が FILLED 行＋count1・buying_power tile が bp/equity・run_result tile が `running o:1 f:1` | Section1: 各 tile の `_content.text` が snap1 の実数を含む（"(no data — Replay)" でない） | 自動(E2E済) | — |
| JOURNEY-NBHAKO-06 | bar2 スナップショット → 同じ tile が**逐次更新**（one-shot でなく追従） | 同上（payload-change gate `cs:1020` が新 string を通す） | override=snap2（position 増・equity 変）→ `RefreshLiveTiles` → tile text が snap2 の実数へ**変化**（snap1 と異なる） | Section1: positions/buying_power tile text が snap1≠snap2 で更新 | 自動(E2E済) | — |
...
| JOURNEY-NBHAKO-11 | step のスナップショットも Hakoniwa に届く（bar-by-bar→結果 tile） | 同 NBHAKO-05 の経路（step も同じ poll/override→tile 鎖） | step press 後 override=stepSnap → `RefreshLiveTiles` → positions tile が stepSnap の建玉を反映 | Section2: positions tile text が stepSnap の実数 | 自動(E2E済) | — |
