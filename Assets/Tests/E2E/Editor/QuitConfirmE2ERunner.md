...
> 後処理）は純ロジック（`QuitConfirmController` は `isDirty`/`isBound`/`pickerReturnedPath` を真理値注入する pure logic）
> なので**自動可**。batchmode 抑制（headless で確認しない）は配線不変条件、実 OS ウィンドウ close ＋実 native picker は
> *HITL*。

## 対象サーフェス

アプリ終了確認（頭脳 `QuitConfirmController` ＋ chrome `QuitConfirmOverlay` ＋ root 連携 `BackcastWorkspaceRoot` の
...
|---|---|---|---|---|---|---|
| QUIT-01 | clean（非 dirty）で終了 → ダイアログ出さず即終了 | `QuitConfirmController.RequestQuit`／`BackcastWorkspaceRoot`(wantsToQuit) | `RequestQuit(false)==QuitNow`・`!IsOpen` | clean→`RequestQuit`→`QuitNow` を assert | 自動(E2E済) | `QuitConfirmE2ERunner`(RequestQuitGate) |
| QUIT-02 | dirty（bound）で終了 → 確認ダイアログ表示 | `QuitConfirmController.RequestQuit` | `RequestQuit(true)==Confirm`・`IsOpen==true` | dirty→`RequestQuit`→`Confirm`＋`IsOpen` を assert | 自動(E2E済) | `QuitConfirmE2ERunner`(RequestQuitGate) |
| QUIT-03 | ダイアログで「保存」（bound） → 保存して終了 | `QuitConfirmController.ChooseSave` | `ChooseSave(true)==SaveThenQuit`・`!IsOpen`（配線で .py 書込＋dirty 解除） | open→`ChooseSave(true)`→`SaveThenQuit`＋`!IsOpen` を assert | 自動(E2E済) | `QuitConfirmE2ERunner`(ChooseOutcomes) |
| QUIT-04 | ダイアログで「保存しない」 → 保存せず終了 | `QuitConfirmController.ChooseDiscard` | `ChooseDiscard()==QuitWithoutSave`・`!IsOpen`（on-disk 不変・dirty 据え置き・終了続行） | open→`ChooseDiscard`→`QuitWithoutSave` を assert（選択前 `IsOpen` を liveness 確認） | 自動(E2E済) | `QuitConfirmE2ERunner`(ChooseOutcomes) |
| QUIT-05 | ダイアログで「キャンセル」 → 終了中断 | `QuitConfirmController.ChooseCancel` | `ChooseCancel()==AbortQuit`・`!IsOpen`（終了中断・残留） | open→`ChooseCancel`→`AbortQuit` を assert（選択前 `IsOpen` を liveness 確認） | 自動(E2E済) | `QuitConfirmE2ERunner`(ChooseOutcomes) |
| QUIT-06 | dirty（untitled）で「保存」 → Save As；path→終了 / cancel→中断（案A） | `QuitConfirmController.ChooseSave`／`ResolveSaveAs` | `ChooseSave(false)==SaveAsThenQuit`；`ResolveSaveAs(true)==SaveAsThenQuit`(commit) / `ResolveSaveAs(false)==AbortQuit` | untitled で `ChooseSave(false)`→`SaveAsThenQuit`、続けて `ResolveSaveAs(true/false)` を両方 assert | 自動(E2E済) | `QuitConfirmE2ERunner`(SaveAsResolve) |
...
- **QUIT-01/02（終了判定ゲート）**: `RequestQuit(isDirty)` は dirty 判定だけで分岐する——非 dirty は `QuitNow`（ダイアログ
  なし・`IsOpen` 据え置き false）、dirty は `Confirm`（`IsOpen=true`）。notebook 参照は持たず、dirty は引数注入。
- **QUIT-03/04/05/07（ダイアログ選択 outcome）**: `ChooseSave(isBound)` は bound→`SaveThenQuit`・untitled→`SaveAsThenQuit`、
  `ChooseDiscard()`→`QuitWithoutSave`（isBound 非依存）、`ChooseCancel()`→`AbortQuit`。いずれも `IsOpen=false` にし
  `LastOutcome` を更新。QUIT-04/05 の負側 outcome は選択前に `IsOpen==true` を先に確認して vacuous を回避。
- **QUIT-06（Save As 後処理・案A）**: untitled で `ChooseSave(false)` は `SaveAsThenQuit` を返してダイアログを閉じ、配線が
  native picker を開く。`ResolveSaveAs(pickerReturnedPath)` は path あり→`SaveAsThenQuit`（終了続行）、cancel→`AbortQuit`
  （終了取りやめ＝ダイアログも再表示しない）。
...
- delete-the-production-logic litmus: `RequestQuit` の dirty 分岐を消す → QUIT-02..07 RED。`ChooseSave` の `isBound`
  分岐を消す → QUIT-03/06 RED。`ResolveSaveAs` の cancel→`AbortQuit` を消す → QUIT-06 の picker-cancel leg RED。
...
| Probe | 種別 | 本台本での扱い |
|---|---|---|
| `QuitConfirmE2ERunner` | EditMode・pure C#（真理値注入） | **QUIT-01..07 の正本**。`QuitConfirmController` を直接 new し `isDirty`/`isBound`/`pickerReturnedPath` を注入する pure-logic gate（findings 0068） |
...
- controller 単体の不変条件（QUIT-01..07）は `QuitConfirmController` を直接 new する pure-logic セクションで assert
