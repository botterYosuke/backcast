...
| RRT-06 | **#138 第2スライス** mode→run_result 可視性（Replay/LiveAuto=表示・LiveManual=非表示）＋巻き込み非表示なし | `BackcastWorkspaceRoot.cs` `DriveRunResult` (`_dockWindows.RectOf(WINDOW_ID_RUN_RESULT)` を直接 `SetActive`＝絶対トグル・単一窓) | run_result 窓 `activeSelf` ＋ orders/positions/buying_power の `activeSelf` ＋ `CaptureLayout` premise (RRT-08) | Replay/LiveAuto で `run_result.activeSelf==true`・LiveManual で `false`・3 sibling は LiveManual でも `true` | 自動(E2E済) |
