...
| SCENARIO-16 | scenario tile を Settings へ移設・dock base window 5→4（#126/ADR-0026） | `BackcastWorkspaceRoot.BaseDockWindowIds`（startup 除去）/ `SettingsModalOverlay.ScenarioSection` に tile を build | base dock の id list が **4 個**で `startup` を含まない（buying_power/orders/positions/run_result）。tile/controller は不変＝Settings の Scenario セクションが宿す | Section13 が `BaseDockWindowIds.Length==4` ＆ no "startup" を反射 assert（startup を戻すと RED） | 自動(E2E済) | `ScenarioStartupE2ERunner`(S13) |
