...
| SCENARIO-16 | dock base window が run_result のみに縮退（#126/ADR-0026 で startup・#174-178/ADR-0038 で 3 panel 退役） | `BackcastWorkspaceRoot.BaseDockWindowIds`（startup＋buying_power/orders/positions 除去）/ `SettingsModalOverlay.ScenarioSection` に tile を build | base dock の id list が **1 個（run_result のみ）**で `startup`・退役 3 kind を含まない。tile/controller は不変＝Settings の Scenario セクションが宿す | Section13 が `BaseDockWindowIds.Length==1` ＆ run_result 在・退役 id 不在を反射 assert（退役 id を戻すと RED） | 自動(E2E済) | `ScenarioStartupE2ERunner`(S13) |
