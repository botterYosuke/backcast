...
| STRATEGY-18 | InputField 直接タイプ / IME / caret 同期 / **実 Return で可視改行＋フォーカス保持** | `StrategyEditorView.cs:102,124`（uGUI InputField boundary）／Enter 経路は `StrategyInputField.OnSubmit`（#148）・Escape 経路は `StrategyInputField.OnCancel`（findings 0117） | キー入力→InputField.text→onValueChanged 同期、IME 合成、`_suppress` 中の自己 write 無視。**実キーボードの Return が見た目に改行を入れフォーカスを保つ**（OnSubmit 消費の決定論半分は STRATEGY-59=S27 が AFK で pin） | — | HITL専用（uGUI InputField・IME・実キーボードの編集フィール・実 IMGUI key pump＝findings 0010 §9 / 0116） | `Strategy Editor HITL` |
