...
| STRATEGY-39 | escape regression: `&` リテラルが entity escape されない | `StrategyEditorView.EscapeForUguiRichText` が `<` のみ escape（UGUI は entity を decode しないので `&`→`&amp;` は字面のまま画面に出る pure regression・findings 0076 §6 D6） | `print("a & b")` 相当の stdout segment で `CurrentConsoleText` がリテラル `a & b` を含み `&amp;` を含まない | Section21: `exec.SetConsole({stdout,"a & b\n"})`→`view.RunCell→DrainAndRoute`→`CurrentConsoleText` を assert | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-40 | multi-cell routing: pressed cell + reactive descendant が**それぞれの**窓に paint・互いに滲まない | `NotebookRunController.ApplyResult` 200/206 行が `cells[co.Index]` → `RegionOf` で per-ran index→region routing | exec が `(idx=0, "from-c0")` と `(idx=1, "from-c1")` を返す press で、R1 console=from-c0／R2 console=from-c1、互いに leak しない | Section21: `exec.SetMulti((0,"from-cell-0"),(1,"from-cell-1"))`→`run.RunCell(R1)`→`view1.CurrentConsoleText`/`view2.CurrentConsoleText` を assert | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-41 | re-press REPLACE: 同 cell 再 press で**前回 text が混ざらない** | `StrategyEditorView.SetConsole` が `_consoleText.text = BuildConsoleRichText(...)` で**置換**（append ではない） | "round1" press→"round2" press 後、`CurrentConsoleText` が `round1` を**含まず** `round2` を含む | Section21: 2 連 press で 1st payload が 2nd 後の text から消失 | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-42 | re-press EMPTY: 同 cell の 2nd press が空 segment 列なら**ブロックが隠れる** | `SetConsole(null\|empty)` の早期 return が `_consoleBlock.gameObject.SetActive(false)` | 非空 press→ConsoleBlockVisible=true、空 press→ConsoleBlockVisible=false | Section21: non-empty→empty 連 press 後 `ConsoleBlockVisible==false` | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-43 | overflow → 本物の ScrollRect（findings 0076 §6 D5） | `StrategyEditorContentBuilder.BuildOutputBlock` が ScrollRect + Viewport + Content + 縦 Scrollbar を組み、`StrategyEditorView.ApplyBlockSize` が Block.LE.preferredHeight を min(Content.preferredHeight, body*0.45) で cap | 多行 payload（40 行）で `scroll.content.rect.height > scroll.viewport.rect.height + 1px`・`verticalNormalizedPosition = 0f / 1f` 設定が往復で stick | Section21: 40-line payload→content>viewport を assert＋`scroll.verticalNormalizedPosition`設定の round-trip を assert | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-44 | bodyH==0 first-frame race: 初回描画でも block が見える | `StrategyEditorView.ApplyBlockSize` が `bodyH==0` ブランチで natural drives（cap collapse 回避） | adopted window のサイズ未確定状態（`ForceRebuildLayoutImmediate` priming なし）で press→`ConsoleBlockVisible==true`・`preferredHeight > 0` | Section21: adoptRoot を resize する**前に** press して `ConsoleBlockVisible` ＋ `LE.preferredHeight > 0` を assert | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-45 | `</color>` 注入耐性: amber wrapper を user payload が閉じない | `EscapeForUguiRichText` が `<` → `&lt;` 変換で全 stderr segment を escape してから `<color=#ffa01c>...</color>` で囲む | stderr segment に `</color>` を仕込んでも `CurrentConsoleText` に `&lt;/color>` が出現・OUR `<color=...>...</color>` wrapper が破られない（escaped が wrapper の内側） | Section21: stderr {"start","</color>middle","end"} payload で `amberOpen < escapedLeak < amberClose` の順序を assert | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
| STRATEGY-46 | dormant-reuse race: AddCell/DeleteCell が in-flight run を drop（findings 0076 §6 D7） | `NotebookCellCoordinator.ListMutated` event を `NotebookRunController.Invalidate` に wiring。AddCell / DeleteCell / SyncWindowsToNotebook で generation bump → 古い request の result は drain 時に dropped | press R1（cellA）→DeleteCell(R1)→AddCell(cellC 再 bind R1)→DrainAndRoute で `ConsoleBlockVisible==false`・stale stdout が cellC の view に leak しない | Section21: synchronous lane で press→DeleteCell→AddCell→DrainAndRoute の sequence で view の console 不在を assert | 自動(E2E済) | `StrategyEditorNotebookE2ERunner`(S21) |
...
- **STRATEGY-39（`&` 二重エスケープ regression）litmus**: `EscapeForUguiRichText` に `Replace("&","&amp;")` を**復活**させると、`a & b` payload が `a &amp; b` で
  paint され S21 の `Contains("&amp;")` 否定 assert が落ちる（UGUI は entity を decode しないため pure regression・findings 0076 §6 D6）。
- **STRATEGY-40（multi-cell routing）litmus**: `NotebookRunController.ApplyResult` の `cells[co.Index]` を `cells[0]` に collapse すると、R2 への
  descendant 出力が R1 へ寄り S21 の `view2 missing 'from-cell-1'` assert が落ちる（STRATEGY-20 の親戚だが multi-ran 経路を pin）。
- **STRATEGY-41（re-press replace）litmus**: `StrategyEditorView.SetConsole` を `_consoleText.text += BuildConsoleRichText(...)`（append）に変えると S21 の
  `Contains("round1")` 否定 assert が落ちる。
- **STRATEGY-42（re-press empty hides）litmus**: `SetConsole` の `null\|empty` 早期 return から `_consoleBlock.gameObject.SetActive(false)` を消すと S21 の
  post-empty `ConsoleBlockVisible==false` assert が落ちる。
- **STRATEGY-43（overflow scroll）litmus**: `BuildOutputBlock` から ScrollRect を `RectMask2D`-only 構成に**戻す**と S21 の `scroll==null` ／
  `content > viewport` ／ `verticalNormalizedPosition` round-trip assert のいずれかが落ちる（findings 0076 §6 D5 を逆退させない pin）。
- **STRATEGY-44（first-frame paint）litmus**: `ApplyBlockSize` の `bodyH==0` ブランチ（`bodyH > 0f ? min(...) : natural`）を `bodyH * cap` 一本に戻すと
  S21 の `preferredHeight > 0` assert が落ちる（cap が 0 へ collapse する）。
- **STRATEGY-45（`</color>` 注入耐性）litmus**: `EscapeForUguiRichText` の `<` → `&lt;` 置換を消すと、user の `</color>` が**実際の閉じタグ**として paint され
  S21 の `Contains("&lt;/color>")` assert が落ちる（amber wrapper も実際に破られる）。
- **STRATEGY-46（dormant-reuse race）litmus**: `BackcastWorkspaceRoot.cs:388` の `_coordinator.ListMutated += () => _notebookRun.Invalidate();` を消す
  か、`NotebookCellCoordinator` の AddCell/DeleteCell から `ListMutated?.Invoke();` を抜くと、stale stdout が cellC の view へ leak し S21 の
  `Contains("stale-from-A")` 否定 assert が落ちる（generation bump が DeleteCell+AddCell で発火することを構造的に pin・findings 0076 §6 D7）。
