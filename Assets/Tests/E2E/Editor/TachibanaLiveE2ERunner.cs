...
public static class TachibanaLiveE2ERunner
{
    ...
    static string Execute(out char[] second)
    {
        ...
        // ── step 2.4: 本番トリガ経由で market-data 購読 → per-ticker FD WS を開く（#107） ──
        // 以前はこの runner が host.Lanes.SubmitSubscribeMarketData を *自分で* 叩いていたが、それは
        // 「本番 UI が購読を起動する配線」をゲートできない死角だった（production-binding の盲点）。#107 で
        // 撤去し、本番と同じ LiveSubscriptionCoordinator + LaneSubscribeSink を universe 投入 → LiveManual 突入の
        // 一括購読として駆動する（テスト自身は SubmitSubscribeMarketData を呼ばない）。購読が走った観測は
        // step 2.5 の EC WS gate（実 demo の WS が立ち上がる）で行う。
        // #85 follow-up: account-level standalone EC stream は ST p_errno=2 で届かないため、per-ticker FD
        // subscription の p_evt_cmd に EC,SS,US を相乗りさせる方式（event_protocol.md §EC）。7203.TSE を
        // carrier に購読し、その WS で EC を待つ（EC は口座単位で全件配信されるため carrier=発注 ticker でよい）。
        ...
        subCoord.OnModePoll(FooterModeViewModel.LiveManual);   // Replay→LiveManual 立ち上がり → universe 一括購読
