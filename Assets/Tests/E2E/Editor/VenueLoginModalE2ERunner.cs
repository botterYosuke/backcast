...
//
// 据え置き（HITL 専用）: 実 venue 認証（kabu verify / tachibana demo）・実キーボード drain・実モーダル表示は
// display/venue-bound（findings 0131 §HITL）。本ゲートは controller のロジック契約（secret 規律・OK gate・
// fields JSON・mode 再導出・probe 連動・PEM ピッカー seam・submit→result）を決定論で固定する。
...
public static class VenueLoginModalE2ERunner
{
    ...
    static void Type(VenueLoginModalController m, string s) { foreach (char c in s) m.AppendSecretChar(c); }
    ...
    public static void Run()
    {
        try
        {
            KabuSecretDrainAndMask();          // VLOGIN-MODAL-01
            SubmitGating();                    // VLOGIN-MODAL-02
            FieldsJsonShape();                 // VLOGIN-MODAL-03
            SecretTransientAndZeroize();       // VLOGIN-MODAL-04
        ...
        if (_fail.Count == 0)
        {
            Debug.Log("[E2E VENUE LOGIN MODAL PASS] kabu char[] secret drain + masked + zeroize (VLOGIN-MODAL-01) / " +
                      "OK gate kabu+tachibana (VLOGIN-MODAL-02) / fields JSON (VLOGIN-MODAL-03) / secret transient + " +
                      "zeroize (VLOGIN-MODAL-04) / result render + retry/allow_retry (VLOGIN-MODAL-05) / mode re-derive " +
                      "(VLOGIN-MODAL-06) / station probe gates OK (VLOGIN-MODAL-07) / PEM picker seam (VLOGIN-MODAL-08) / " +
                      "submit→result roundtrip via Python-FREE fake executor (VLOGIN-MODAL-09) — " +
                      "pure VenueLoginModalController, #181/ADR-0040/findings 0131, under Unity Mono");
            EmitPerIdTags();
    ...
    // Covers: VLOGIN-MODAL-01 — kabu API パスワードは char[] 1 文字ずつ・masked dot・backspace・close で zeroize。
    static void KabuSecretDrainAndMask()
    {
        ...
        m.AppendSecretChar('p'); m.AppendSecretChar('w'); m.AppendSecretChar('9'); m.AppendSecretChar('x');
        m.BackspaceSecret();                    // drop the 'x'
        m.AppendSecretChar('Z');
        Check(m.SecretLength == 4, "kabu secret length mismatch: " + m.SecretLength);  // p,w,9,Z
        Check(m.MaskedPassword == "••••", "kabu mask mismatch: " + m.MaskedPassword);
        m.Close();
        Check(m.SecretIsZeroed(), "kabu secret buffer NOT zeroized after close (secret 規律 違反)");
        // litmus: if Close() skipped ZeroizeSecret the buffer would survive → RED here.
    }
    ...
    // Covers: VLOGIN-MODAL-04 — TakeSecretTransient は入力 PW のコピー・controller バッファは submit 成功(Close)で zeroize。
    static void SecretTransientAndZeroize()
    {
        var m = new VenueLoginModalController();
        m.Open("KABU", KabuInit("verify", 18081));
        Type(m, "9753");
        ...
        Check(m.SecretLength == 4, "controller secret cleared too early (before result)");  // still held until result
        Array.Clear(transient, 0, transient.Length);   // host zeroizes the transient after the RPC
        m.ApplyResult(new VenueLoginSubmitResult { Success = true });
        Check(!m.IsOpen, "modal not closed on success");
        Check(m.SecretIsZeroed(), "controller secret NOT zeroized after success close");
    }
    ...
    static void ModeRefreshRederives()
    {
        ...
        Check(k.SecretLength == 4, "kabu prefill not seeded");
        ...
        Check(k.SecretIsZeroed(), "kabu secret not zeroized on mode switch");
    ...
    static void SubmitRoundtripViaFakeExecutor()
    {
        ...
        // retype the correct password → submit → success closes the modal + zeroizes.
        // (a real failure path keeps the previously typed chars; the user fixes them — here we clear+retype.)
        while (m.SecretLength > 0) m.BackspaceSecret();
        Type(m, "good-pw");
        ...
        m.ApplyResult(r2);
        Check(!m.IsOpen, "modal not closed on success");
        Check(m.SecretIsZeroed(), "secret not zeroized after successful submit");
