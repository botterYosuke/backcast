...
_Avoid_: バーを [[floating window / FloatingWindowLayer / z-order]] や `DockLayer` の window として扱うこと（ADR-0038 で dock/floating 集合から外れた・`Content` 配下に置かない）／バーの位置/可視を永続化すること（非永続・固定 anchor）／data 未取得でバーを非表示にすること（常時表示・未取得は「—」）／Live 純資産を venue account の equity フィールドから読もうとすること（**存在しない**・C# 導出が正）／退役 3 kind を spawn し直すこと（catalog TryGet=false で skip・バー＋ホバーが唯一の面）
