...
**非 marimo `.py` の Open は 1-cell wrap（#86・findings 0052）**: File→Open が選んだ `.py` が marimo notebook でない（`load_app` が None・broken / imperative 戦略など）場合、集約は abort せず**生本文をそのまま 1 cell（name=`_`・config=既定）**に詰めて bound する。Open 直後は dirty=false で Run 解禁（on-disk は元の `.py` のままなので imperative 経路で走る）。一旦 Save すると `generate_filecontents` が marimo 形式に書き換え＝**一方向のマイグレーション補助**（destructive 変換は owner 公認）。fail-soft の null は **path/IO エラーのみ**（missing file / read failure / wrong extension）。
