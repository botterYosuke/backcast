...
- **`bt.replay()` / `bt.step()` を呼ばない cell（=「土台」層・engine 非接続）**: marimo native の reactive 再計算（`thin_drain.HeadlessKernel` を流用する interactive 経路）で per-cell RUN 押下時に押した cell＋下流が DAG 順で再評価される。純粋計算。副作用なし。`bt.bar()` / `bt.portfolio()` を読むことは可能（active bt state の参照は runner を進めない）。
...
**速度レジスタ**: 旧 `_REPLAY_BAR_INTERVAL_SEC = 0.01`（`_backend_impl.py:189` の hardcoded constant・#76 S6b-β で footer transport (#30) と共に固定された値）を、**host 所有・thread-safe・live-mutable な速度レジスタ**へ昇格する（Phase 4）。配置は **`DataEngineCore` 上・`_replay_stop_event`（`core.py:43`）の隣**で loop が毎 bar 読む（`sleep(base / speed)`）。**user-facing API は `bt.replay(speed=N)` の引数のみ**＝call 時にレジスタへ書き込み、loop が次の bar から尊重する。**UI コントロール（slider / pacing button）は作らない**（#30/#68 footer transport の退役判断を維持・findings 0046）。`bt.step()` には速度は無意味で `speed` 引数を持たない（ユーザー自身が pace する＝ボタンを押すごとに 1 bar）。退役した transport を意図的に復活させる理由（B2 の visual playback ニーズが「reactive drain は瞬時に終わる」前提を変えた）は findings 0070 に明示記録（silent な復活でない）。
_Avoid_: per-cell RUN を「`bt` 使用 cell に限る」と解釈すること（**全 cell 窓に在る**・`bt` 非使用 cell は純粋計算の reactive 再評価）／global ▶ Run / title-bar Run / footer transport を「将来復活」前提で温存すること（ADR-0016 で formal supersede・復活は新規 ADR が要る）／命令型 `.py` の UI 実行を「sidecar 必須で開けない」と解釈すること（1-cell wrap で **Open できる**・実行入口だけが per-cell RUN に一本化）／速度レジスタを UI slider に bind すること（API は `bt.replay(speed=N)` 引数のみ・UI ボタンは作らない）／`_REPLAY_BAR_INTERVAL_SEC` を constant のまま新規参照を増やすこと（Phase 4 でレジスタへ昇格・thread-safe getter 経由が正）
