...
Replay 実行**中**に [[アカウントサマリーバー]]（旧 BuyingPower/Positions/Orders 3 base パネル＝ADR-0038 で退役）＋
[[run-result popup]] へ出す実数値。**post-run の
...
findings 0046・方針: ADR-0007/0006）。※2026-06-27 [[ADR-0037]]: RunResult は dock base panel を退役し **[[run-result popup]]**
（screen-anchored 右上・content があるときだけ）へ。content drive（format 関数）は同じものを再利用、ターゲットだけ付け替え。
※2026-06-27 [[ADR-0038]]: BuyingPower/Positions/Orders の 3 base パネルも dock を退役し **[[アカウントサマリーバー]]**（ヘッダー
直下・全幅・常時）へ。ホバー詳細は同じ format 関数（`FormatBuyingPower`/`FormatPositions`/`FormatOrders`）を再利用。両 ADR
合算で **base dock パネルは全滅＝dock=chart のみ**。
...
infinite canvas の Content 上で、**独立した floating window**（`chart` / `positions` / `orders` /
`buying_power`）が **同一 [[window group / groupId]] を共有してくっついた集合**を指す概念ラベル。
専用の bounded サーフェス GameObject（HakoniwaRoot）は持たず、全 window は奥プレーン `DockLayer`（ADR-0018・1.0倍）の子。
※2026-06-27 [[ADR-0037]]: `run_result` は dock window を退役（[[run-result popup]] へ）。※2026-06-27 [[ADR-0038]]:
`buying_power` / `orders` / `positions` も dock window を退役（[[アカウントサマリーバー]] へ）＝**dock 集合は `chart` のみ**
（base dock singleton 全滅・Hakoniwa group / factory base group 概念は退役）。
くっつき方は drag 中の **in-drag 磁石吸着（[[R_SNAP]] = 96px）+ release-position commit（最寄り flush へ snap して merge）**（ADR-0024）。
**~~[[Hakoniwa group]] 概念は ADR-0024 で退役~~**（startup / run_result の特別扱い無し・全 group が同じ挙動）。drag mode は **[[gesture channel]] で gesture 開始時に固定**（[[ADR-0029]]）: plain title-bar drag → **島移動**（距離無制限・別島に flush で merge・detach は起きない）／ **`Alt`+drag → 単窓ピックアップ**（島から 1 枚抜き出し・ドロップ先で swap / merge / detach 決定・[[ADR-0032]] で eject つまみ起動を退役）。~~ADR-0024 の cursor 位置動的 3 mode 判定（< [[D_DETACH]] で translate / ≥ で detach）~~は **ADR-0029 で supersede**（距離トリガ撤廃＝owner「detach 感度が高すぎ・島の遠距離移動不可」「内外で化けて分かりにくい」）。
...
~~[[Hakoniwa group]] の判定に使う kind 集合 = **`startup` / `run_result`**~~。**ADR-0024 で概念退役**——startup / run_result は他の window と同等のドラッグ挙動。detach 不可 invariant も廃止（全 window が D_DETACH 超えで detach 可）。~~`DockShape.IsCoreKind` は ADR-0020 first-launch factory grouping の base 窓 ID 列挙には引き続き存在しうる~~。※2026-06-27 [[ADR-0037]]: `run_result` が dock window を退役したため **`IsCoreKind` は空集合**（全 kind に false）へ畳む＝dead-code simplify（ADR-0024 以降 production 挙動経路から未参照だったので挙動影響なし）。factory base group は 3 窓 `{buying_power, orders, positions}` で ≥2 閾値により成立。※2026-06-27 [[ADR-0038]]: その 3 窓も
dock 退役（[[アカウントサマリーバー]] へ）したため **base dock 窓は 0＝factory base group 概念ごと退役**（`FormFactoryBaseGroup`
/ base singleton 列挙を退役・first-launch dock = `chart` のみ）。`IsCoreKind` は空のまま。
_Avoid_: core / non-core で挙動を分けるコードを新規に書くこと（ADR-0024 で全 window 同一挙動・ADR-0037 で core 集合は空）

**~~D_DETACH（detach 閾値）~~** ※2026-06-21 新設・2026-06-22 値再較正・**2026-06-25 SUPERSEDED（退役）by [[ADR-0029]]**（findings 0082／0088 §11／**0106 §6**）:
~~drag 中の detach 判定閾値 = `256f` canvas-logical px。判定 = `|cursor - drag_start| ≥ D_DETACH` で detach~~。**ADR-0029 で距離トリガごと退役**——detach はもはや距離ではなく「**[[単窓ピックアップ]]（`Alt`+drag・[[ADR-0032]] で eject つまみ起動を退役）を兄弟でも別島 flush でもない空き地にドロップした結果**」（owner「detach の感度が高すぎてすぐ引き剥がれる・島の遠距離移動も不可」＝距離 1 軸では島移動と detach を分離できない）。`FloatingWindowMath.D_DETACH_PX` 定数は**削除**。新たに distance を判定に使わない。
...
in-drag 磁石吸着の発動距離 = **`96f` canvas-logical px**（owner Q15=C）。`FloatingWindowMath.R_SNAP_PX` 定数。**[[IslandMove]] / [[単窓ピックアップ]] の運搬**で毎フレーム発動: dragged の **外周辺**（IslandMove なら island の外 4 辺・pickup なら運ぶ A の 4 辺）と他 window の対辺との最短距離が `≤ R_SNAP` ∧ 直交軸 overlap > 0 のとき、実描画位置を flush 位置へ snap（実窓ごと寄せる）。**[[island-scoped reflow]]（swap 直後）でも同じ `ComputeMagneticSnap`（R_SNAP）を島内隣窓に適用**する（ADR-0029 D6）。cursor が R_SNAP 圏外に出るまで貼り付く（stickiness）。発動の瞬間は [[spring animation]] 200ms で補間（"プルン"）。release で snap 位置が確定したら最寄り flush merge へ統合（[[window group / groupId]] release-position rule）。離散 snap で物理シミュレーションではない（owner Q6=A・連続吸引は AFK の "snap-on-release" 権威モデルを崩すため不採用）。
_Avoid_: R_SNAP を screen pixel にすること（zoom 非依存性）／毎フレームの座標変動を AFK で再現しようとすること（channel と最終 rect の assert で十分）／swap **target 検出**に R_SNAP を使うこと（target 検出は center-in-rect・edge attraction 無し。※swap **後の [[island-scoped reflow]]** は別フェーズで R_SNAP を島内隣窓に適用する＝混同しない）
...
**gesture channel** ※2026-06-25 新設・方針 **[[ADR-0029]]**（findings 0106 §1）・**2026-06-25 起動経路更新 [[ADR-0032]]**（findings 0113）:
floating window のドラッグ mode を決める **2 チャンネル**。`OnBeginDrag` 時点で「何をどう掴んだか」で 1 度だけ確定し、その drag の間は**化けない**（~~ADR-0024 の cursor 位置 per-frame 判定~~を supersede＝owner「内外で swap/translate に化けて分かりにくい」の根治）。`DragChannel { IslandMove, SingleWindowPickup }`。**IslandMove**＝plain title-bar drag（island 全体を平行移動・距離無制限・merge あり・detach なし）。**SingleWindowPickup**＝`Alt`+title-bar drag（島から 1 枚抜き出し・ドロップ先で swap/merge/detach 決定）。判定の唯一の入力は Alt（`ResolveChannel(altHeld)`・ADR-0032 で eject つまみ起動を退役）。
_Avoid_: drag 途中で channel を切り替えること（gesture 開始で固定）／distance や cursor 内外で mode を再判定すること（ADR-0029 で退役）

**単窓ピックアップ（SingleWindowPickup）** ※2026-06-25 新設・方針 **[[ADR-0029]]**（findings 0106 §3）・**2026-06-25 起動経路更新 [[ADR-0032]]**（findings 0113）:
[[gesture channel]] の一方。`Alt`+title-bar drag で起動し（~~eject つまみ "⤴"~~は ADR-0032 で廃止）、island から 1 枚（A）を抜き出して実描画で運ぶ。release のドロップ先で結果確定: 同 island の兄弟 rect 上 → **swap**（[[swap drop target]]）／別 island に flush（磁石 engaged）→ **merge**（singleton 経由）／空き地 → **detach**（A.groupId=null・残 <2 で連鎖 dissolve）。membership を減らす操作（detach）は本チャンネル限定＝[[IslandMove]] では起きない（owner「すぐ detach する」の根治）。
_Avoid_: detach を本チャンネル以外で起こすこと／距離で detach を判定すること（ドロップ先で決まる）

**eject つまみ** ※2026-06-25 新設 [[ADR-0029]]・**2026-06-25 退役 [[ADR-0032]]**（findings 0113）:
~~各 floating window の title-bar に常時表示する単窓ピックアップ起動アフォーダンス（"⤴" アイコン・第 2 raycast target）~~。**廃止**＝owner が HITL で「見た目の雑音／UI が忙しい・Alt+drag で十分」と判断（ADR-0032）。[[単窓ピックアップ]]の起動は `Alt`+drag 単独になり、見える起動アフォーダンスは無い（invisible mode を owner が再受容＝既知のコスト）。`FloatingWindowEjectHandle` は削除済み。
...
各 floating window の root 右下角に常時表示する**リサイズ起動アフォーダンス**（"◢" チップ・raycast target・独自 IDragHandler）。「ここを掴めばサイズを変えられる」を可視化。title-bar drag 入力（[[gesture channel]] を `ResolveChannel` で確定する経路）と違い**独自 IDragHandler を持つので `ResolveChannel` に入らない**（ADR-0029 の 2 チャンネル不変条件を壊さない別系統）。左上 pivot 固定で右下へ伸縮・全 window 一律（per-spec フラグなし）。
...
**銘柄表示ラベル（instrument label・`<id> <name>`）** ※2026-06-25 新設（findings 0111）:
...
_v1 制約_: `BackcastWorkspaceRoot._chartViews` Dictionary は `chart:<iid>` keyed で **1 chart-window per iid**。同 iid を別 window で開く v2 拡張は `chart:<iid>#<n>` 命名で別 slice として扱う（sidecar schema は per-FW-entry で対応済）。
...
**観られる再生（watchable playback・Replay 実行中の可視カーソル）／[[ReplayWindow]]** ※2026-06-28 新設・[[#182]]（findings 0131）／**2026-06-30 リファクタ**で `engine.replay_window.ReplayWindow` に集約:
`bt.replay(bars_per_second=N)` の再生は **RUN 開始後、これまでにストリーミングされた本数ぶんだけ**チャートに描かれ、ペースに合わせて1本ずつ前進する（＝観られる）。RUN 前（LOADED / preview）は **chart 全期間 supply** の cold seed をそのまま全期間表示（fit-all）。両者を分けるのが **可視カーソル**＝最新のストリーム済 primary bar の時刻（replay clock）。RUN 中・RUN 後（完走 or 途中 break）は per-instrument 系列をこのカーソルまでにクリップし、RUN 前と再 load 後は全期間に戻す。例外（D4）: RUN 後に `populate_replay_preview` で**再 preview した銘柄**は全期間 fit-all に戻す。**この概念（カーソル gate＋clip＋preview 免除集合）の正本は `ReplayWindow`**＝ `engine.core` は `visible_cursor_ms()`／`clip()`／`mark_preview()`／`forget()`／`rearm()` を呼ぶだけ。免除集合はかつての engine フィールド `_full_preview_iids` を `ReplayWindow._preview_iids` に内包（`mark_preview`=populate_replay_preview／`forget`=forget_instrument／`rearm`=load_replay_data）。interface 単体テストは `tests/test_replay_window.py`、本番経路の end-to-end witness は `tests/test_replay_chart_watchable_playback.py`（REPLAY-PLAY-01..06）。
_Avoid_: 可視カーソルを「ストリーム済件数」で持つこと（multi-instrument で非 primary を誤クリップ・正は replay timestamp）／クリップ gate を「RUNNING のときだけ」にすること（完走/break 後に host が IDLE へ戻すので、break 時点の本数が全期間へ snap back する）／run 完走後の **再 preview**（`populate_replay_preview` は IDLE で発火・全カタログ再シード）を古い run cursor でクリップすること（review H1・D4 で免除＝免除しないと完走後に追加した新銘柄チャートが run 終端までしか描かれない）／clip ロジックを `_build_trading_state_locked` へ inline に戻すこと（正本は [[ReplayWindow]]）
...
（`chart` / `positions` / `orders` / `buying_power`）も floating window**＝旧「chart は
floating window ではない」は**反転**（chart は multi-instance kind `chart:<id>`・universe 同期）。※2026-06-27 [[ADR-0037]]:
`run_result` は dock floating window を退役し **[[run-result popup]]**（screen-anchored・非永続）へ＝floating/dock 集合から外れる。
※2026-06-27 [[ADR-0038]]: `buying_power` / `orders` / `positions` も dock floating window を退役し **[[アカウントサマリーバー]]**
（screen-anchored・非永続）へ＝**dock 集合に残るのは `chart` のみ**（base singleton 全滅）。
**ドッキング = 磁石スナップ＋group 関係（ADR-0019 ＋ ADR-0024 ＋ ADR-0029）**: drag 中は **in-drag 磁石吸着**（[[R_SNAP]]=96px で flush 位置へ実描画 snap）、release で **flush 隣接判定**（ε=1px ∧ 直交軸 overlap > 0）／overlap なら最寄り flush へ snap して **[[window group / groupId]]** を付与/merge。drag mode は **[[gesture channel]] で gesture 開始時に固定**（[[ADR-0029]]・~~ADR-0024 の cursor 位置 per-frame 3 判定 / [[D_DETACH]] 距離 detach~~を supersede・drag 中化けない）: **①島移動**（plain title-bar drag）= island 全員を実描画で平行移動・**距離無制限**・magnetic snap・別島に flush で merge・**detach は起こさない**／**②[[単窓ピックアップ]]**（`Alt`+drag・[[ADR-0032]] で eject つまみ起動を退役）= 1 枚抜き出して運びドロップ先で **swap**（[[swap drop target]]・サイズ維持＋[[island-scoped reflow]]・ghost は reflow 後レイアウト予告）／ 別島 flush で **merge** ／ 空き地で **detach**（A 単独・残 <2 で dissolve）。**~~core（startup/run_result）含み時の Hakoniwa 昇格・移動禁止・core 不抜は ADR-0024 で退役~~**——全 island が同一挙動・core も pickup で自由に detach。**ESC** で drag 中 revert（spring 200ms・state 不変）。
**FloatingWindowLayer** = ADR-0018 §10 で「手前 1.2倍プレーン」用に縮退（`strategy_editor` cell ＋ `order` のみ）。元箱庭の dock 種（chart + buying_power/orders/positions・※[[ADR-0037]] で `run_result` 退役・※[[ADR-0038]] で `buying_power`/`orders`/`positions` も退役＝**`DockLayer` に残るのは `chart` のみ**）は **`DockLayer`（1.0倍・奥・背面 sibling）** に居る。各プレーンに独立 `FloatingWindowController` が居り、snap/group 母集合はプレーンに閉じる（cross-plane snap・cross-plane group とも禁止＝ADR-0018／ADR-0019）。**z-order** =
...
**run-result popup** ※2026-06-27 新設・方針 [[ADR-0037]]（findings 0125）:
run（Replay / LiveAuto）の結果を出す **screen-anchored な右上ポップアップ card**。`ScreenSpaceOverlay` Canvas 直下（[[infinite canvas]] の `Content` の子では **ない**）に置くので **パンしても動かない＝「3D空間（パララックス層）から除外」**。canvas に重ねて浮く（ガター予約なし）・固定サイズ・drag/resize 不可（title ＋ × close のみ）。表示は **content-derived ＋ manual-close latch**: run データがあるとき出現（Replay running→完了で full stats / LiveAuto telemetry）、× で閉じると **次の run まで再出現しない**（再 arm は新 run の rising edge＝Replay の portfolio 再投入 / LiveAuto の新 `run_id`）。**LiveManual は出ない**（live VM の telemetry/lifecycle フラグは process 内で sticky・決して reset されない＝findings 0125 F4 のため、content だけだと LiveAuto run 後に LiveManual へ flip すると stale telemetry が漏れる。よって live の hasContent は **`DisplayMode==LiveAuto` に scoping** して #138 の anti-stale 契約を content gate へ畳む）。content drive は旧 dock tile の format 関数（`FormatReplayRunResult*` / `FormatRunResult`）を再利用。**永続化ゼロ**（位置固定・可視は派生・latch は session 内）。旧 `run_result` dock base singleton（[[Replay portfolio projection]] の 4 つ目）を退役した行き先。
_Avoid_: run-result popup を [[floating window / FloatingWindowLayer / z-order]] や `DockLayer` の window として扱うこと（ADR-0037 で dock/floating 集合から外れた・`Content` 配下に置かない）／popup の位置/可視を `CaptureLayout`/`floatingWindows` に永続化すること（非永続）／旧 `DriveRunResult` 式の per-mode show/hide rule を足すこと（可視は content＋latch の派生で決まる・唯一の mode 参照は live content を `LiveAuto` に scoping する stale 防止だけ＝findings 0125 D3）

**アカウントサマリーバー** ※2026-06-27 新設・方針 [[ADR-0038]]（findings 0126）:
口座状態（純資産/含み損益・買付余力/現金・建玉件数・注文件数）を出す **ヘッダー（`MenuBarView`）直下の全幅サマリーバー**。`ScreenSpaceOverlay` Canvas に固定 anchor し（[[infinite canvas]] の `Content` の子では **ない**）**パンしても動かない**・ガター予約なしで canvas に重なる。**4 スロット**（①純資産＆含み損益＝主数値 Equity・含み損益の符号で緑/赤／②買付余力＆現金＝主数値 Buying Power／③建玉件数／④注文件数）。バー上は **アイコン枠＋主数値 1 つ**、ペア第 2 値（含み損益・現金）と銘柄別内訳は **ホバー詳細** card に回す。ホバー詳細は退役 dock tile の format 関数を再利用（②=`FormatBuyingPower`／③=`FormatPositions`／④=`FormatOrders`、①=新「口座サマリー」4 行）。アイコンは当面 **3D プリミティブ placeholder**（cube/sphere/capsule/cylinder を `RenderTexture`→`RawImage`・差し替え seam）。**両モード常時表示**（未取得は「—」）。Live 純資産は **`Cash + Σ(qty×avg+uPnL)`** を C# 導出（[[口座評価額(equity) / 現金(cash) / 買付余力(buying_power)]] の MTM 規約）、Replay は `PortfolioSnapshot.Equity`。**永続化ゼロ**。旧 `buying_power`/`orders`/`positions` dock base singleton（[[Replay portfolio projection]] の 3 つ）を退役した行き先。※2026-06-27 視覚リファインメント（findings 0126 §視覚リファインメント）: 帯（strip）背景は **透明＋クリック透過**（[[Universe sidebar]] と同型 `panel_background.a=0`・`raycastTarget=false`＝空白部は後ろの canvas へ click 透過）／スロットは **左詰め固定幅**（右は空白）／主数値は **アイコン下に縦積み**／金額は **k/M 短縮表記**（`298k`・ホバー card は従来 `Money` フル桁の byte 一致再利用＝精度維持）。
_Avoid_: バーを [[floating window / FloatingWindowLayer / z-order]] や `DockLayer` の window として扱うこと（ADR-0038 で dock/floating 集合から外れた・`Content` 配下に置かない）／バーの位置/可視を永続化すること（非永続・固定 anchor）／帯背景を不透明にし click を吸収させること（透明＋透過が正・sidebar 同型）／バー主数値の短縮 formatter を `Money` に統合しホバーのフル桁 byte 一致を壊すこと（別関数に分ける）／data 未取得でバーを非表示にすること（常時表示・未取得は「—」）／Live 純資産を venue account の equity フィールドから読もうとすること（**存在しない**・C# 導出が正）／退役 3 kind を spawn し直すこと（catalog TryGet=false で skip・バー＋ホバーが唯一の面）
...
**マークダウンセル / [m] Add Markdown ボタン**:
[[cell window / marimo notebook（cell-DAG authoring 表面）]] の **オーサリング affordance**（#179・findings 0128）。**マークダウンセルは専用の型ではない**——本体が `mo.md(r"""…""")` の**普通の cell**で、▶ per-cell RUN すると `text/markdown` 出力が窓に rich-text 描画される（[[rich output mimetype 契約（`{mimetype, data}`・#95 Phase 6）]] の既存経路を再利用＝net-new 描画なし）。[+] Add Cell の**真上**に置く 2 つ目のボタン [m] が、`mo.md` 種付きの cell を 1 枚足す（[+] が空 cell を足すのと同型）。**marimo 本家 parity で素の `mo` を使う**（`_mo` ローカル import ではない・owner 決定）ため、[m] は **共有 `import marimo as mo` cell を冪等に用意**してから種 cell を足す（既存に `import marimo as mo` を定義する cell があれば再利用・無ければ 1 枚追加）。素の `mo.md` が ▶ で `NameError` にならないのは、per-cell RUN が **押した cell＋stale な上流 ancestor（import cell）** を一緒に走らせるため（def/ref エッジ・globals persist の詳細は findings 0128 §D3）。作成直後は **手動 ▶**（[+] と同じ・自動 RUN しない）。[m] は [+] と同じ `_addCellOverlay` に同居し **`LiveManual` 非表示**を共有する（findings 0110 の mode-conditional 可視性）。
_Avoid_: マークダウンセルを別 cell 型／別ウィンドウ種と解釈すること（普通の `mo.md` cell・KIND_STRATEGY_EDITOR）／`_mo` ローカル import で自己完結させること（owner は本家 parity の素の `mo` を選択・[m] が共有 import cell を担保）／cell 内に `import marimo as mo` を同梱すること（[m] 2 回押下で `mo` 二重定義＝marimo エラー・共有 import cell が正）／作成時に自動 ▶ すること（手動 ▶・[+] と同型）／marimo の WYSIWYG マークダウン編集モードを移植すること（backcast は素の Python テキスト欄＋▶ 出力ペインのみ・本家 frontend の言語アダプタは無い）

**種付き New（default seeded notebook）/ untitled scratch run**:
New / 初期化 / 最初の Strategy Editor spawn の着地状態。**ADR-0036 で「空白 New＋placeholder」（findings 0050）と「Save まで Run ブロック」（#76）を supersede**。着地時、唯一のセルに **`bt.replay` 観察コード**（`for bar in bt.replay(bars_per_second=2): pass` ＝売買ゼロ）が**本文に種付け**され、universe SoT（[[universe registry（instruments SoT）vs scenario panel]]）はデフォルトで**トヨタ `7203.TSE`** で満たされる（→ `InstrumentRegistry.Changed → SyncChartWindowsToUniverse` でトヨタ chart も spawn）。placeholder（host-API hint）機構は**撤去**。種ノートは `_path=null` の **untitled** のまま **Run 可**——ゲートは「named なら [[WYSIWYR]] / untitled なら非空セルあり」で、untitled の Run 押下時にバッファ synth を **scratch `.py` へ遅延書き出し**して実行する（cwd=scratch dir＝[[ADR-0011]] 追従・§4 解消）。menu badge は **Untitled 維持**（[[document identity badge]]）。Save As で実パスへ束ね直すと以降は named WYSIWYR（dirty ゲート復活）。
_Avoid_: 種を「marimo 非忠実だから placeholder に戻す」と読むこと（ADR-0036 が忠実性より「即観察雛形」を意図優先・findings 0050 を明示 supersede）／種セルに `@app.cell` / `def _(bt)` wrapper を含めること（本体のみ・synth が付与）／untitled Run で scratch を **New 時点で eager 生成**すること（遅延＝Untitled badge とディスク非汚染維持）／デフォルト universe を saved layout 復元経路にも種付けすること（fresh 経路だけ・復元 doc は自前 universe を持つ）／トヨタ chart の自動 spawn を追加配線と見なすこと（既存 universe↔chart 同期の自然な帰結）
...
**per-cell RUN**: すべての [[cell window / marimo notebook（cell-DAG authoring 表面）]]（adopted `region_001` ＋ spawned `region_002+`）が個別の RUN ボタンを持つ（ADR-0013 が立てた `StrategyEditorWindowFrame` の idempotent find-or-create・X ボタンと同型）。クリックで押した cell＋reactive 下流が DAG 順で再実行され、`bt` 使用 cell は実 backtest を駆動する（[[notebook = backtest 一本化 / `bt` ハンドル / 土台 vs backtest 駆動 cell]] 参照）。**alias: Shift+Return 再生（Jupyter/marimo 流ショートカット・#164）**＝コードセル（`StrategyInputField`）にフォーカスがある状態で **Shift+Return / Ctrl+Return（Win）/ Cmd+Return（mac）/ 各キーのテンキー Enter** を押すと、その cell の ▶ ボタンの `onClick.Invoke()` を呼ぶ＝**▶ クリックと完全同一**（実行可否ゲート尊重・実行中（■）なら停止トグル・policy 重複なし）。**plain Return（修飾なし）は従来どおり改行**（trigger しない）。検知・改行抑止・発火・1-押下-1-発火 latch は `StrategyInputField.OnUpdateSelected` の IMGUI key-pump 1 箇所（Escape path 2＝findings 0117 §150 と同型）で行い `Update()` poll と二重化しない。`StrategyInputField.RunShortcutRequested`→`StrategyEditorView` relay→`BackcastWorkspaceRoot.WireCellRunButton` 購読で配線。記録: findings 0122（関連 0116 Enter=改行 / 0117 Escape 保持）。**専用の batch RUN ボタン（#76/#81 merge U1 の global ▶ Run）・title-bar Run（現行 `Assets/Scripts/StrategyEditor/StrategyEditorRunButton.cs`・ADR-0005 由来の 1:1 TTWR parity 単一 Run ボタン surface の現実装で、ADR-0012 partial supersede が strategy-authoring 表面の文脈で明示退役しなかった residual）・global transport（#30 既退役）は ADR-0016 で formal supersede**。**ADR-0005 由来の「アプリ全体で 1 つの再生ボタン」facet（ADR-0012 が strategy-authoring 表面を部分 supersede した時点で reactive cell-DAG モデルとの整合を将来 slice に委ねていた residual）は ADR-0016 で却下**（facet-scoped supersede・ADR-0005 / ADR-0012 本文は無改変＝両者の自己保護条項）。
...
「universe」は backcast で **第一義に [[universe registry（instruments SoT）vs scenario panel]]（`InstrumentRegistry`）が保持する銘柄集合＝ワークスペースが今扱っている銘柄の SoT** を指す。これと、SoT を**満たす（populate）/ 絞る（prune）入力**としての universe は**別物**で、語を裸で使わず必ず修飾する。5 つの sense と 2 つの軸を区別する:
...
- **軸の関係**: **SoT は populate 入力（sidecar / strategy / picker click / user 編集）で埋まり、prune allowlist ソース（venue live / Replay catalog）で縮む**。埋める軸（populate）と縮める軸（prune）は逆向きで、同じ「universe」でも役割が反対。
_Avoid_: 裸の「universe」で SoT と prune ソースを混同すること（venue live universe は**縮める境界**であって SoT でも populate 入力でもない）／strategy の銘柄集合（v19）を venue live universe と同一視すること（前者は populate 入力＝何を売買するか・後者は prune 境界＝今何が有効か・別軸）／prune ソースで SoT を「埋まる」と考えること（prune は縮小専用・populate しない）／#41 が dormant なのを「universe が壊れている」と読むこと（実 venue/catalog producer 待ちで SoT 自体は正常に動く)／**kabu Live picker が listed_info を引くことを「kabu の live universe が解禁された」と読むこと（picker browse のみ＝prune 軸の venue live universe producer は未実装のまま・#41 dormant・kabu store snapshot は依然 authoritative にならない＝#253 防御維持）**
...
**戦略 cell による動的 universe 編集（`bt.universe.*`・方針 ADR-0031）**:
戦略 cell（Replay / LiveAuto）が `bt` ハンドル経由で [[universe registry（instruments SoT）vs scenario panel]] を**走行中でも**編集できる seam。API は `bt.universe.add(id)` / `.remove(id)` / `.clear()` / `.list()`（**丸ごと置換 API は public に出さない**＝clear+add で表現）。**SoT は C# `InstrumentRegistry` のまま**で、`bt.universe.*` は Python を SoT に昇格させず「**プログラムによるユーザー編集**」として作用する（ユーザーが sidebar を手編集したのと同型の registry mutation）。
- これは **[[#41 populate 軸]]の "strategy" 入力の動的・双方向への拡張**であって、**禁止対象の autonomous なシステム prune（#253）ではない**——トリガは「ユーザーが自分の戦略を実行した結果」だから [[market-data 購読（subscribe）vs universe membership（#107）]] の「システムは絶対に銘柄を足さない/減らさない」**不変条件と両立する**（禁ずるのは autonomous なシステム操作・cell 編集は user-triggered populate）。
- **chart は `InstrumentRegistry.Changed` 既配線**なので add→spawn / remove→despawn は追加配線ゼロで反映。**永続化は既存 Save タイミングのみ**（編集が独自の sidecar 書き込みトリガを引かない＝手編集と同じ dirty→通常 Save 経路・ADR-0031 D4）。
- **Replay 走行中の add は「追加時点から」データ合流**（巻き戻し再生でも追加禁止でもない・現 replay 時刻〜`scenario.end` を DuckDB から読み `KernelStepper` の残ストリームへ ts 順 merge・ADR-0031 D5）。
- **LiveAuto は add→subscribe / remove→unsubscribe（対称）**。これを成立させるため購読起動を **UI クリック配線（`LiveSubscribeHook`）から `Registry.Changed` 起動へ拡張**し、unsubscribe-on-remove を新設する（ADR-0031 D6・現状は購読が UI クリック専用で remove→unsubscribe が無い）。
- **対象は Replay ＋ LiveAuto のみ**（LiveManual は人間所有モードで対象外）。ADR-0016 D6 の「universe は cell に書かない」facet を **ADR-0031 が supersede**（`start/end/cash/granularity` の startup-panel 所有は D6 のまま踏襲）。
_Avoid_: `bt.universe.*` を「SoT を Python へ移す」と読むこと（SoT は C# registry のまま・Python は双方向クライアント）／cell 編集を #253 の「システムが勝手に銘柄を」操作と同一視すること（user-triggered populate であって autonomous prune ではない）／編集ごとに即 sidecar 書き込みすること（既存 Save タイミングのみ・ADR-0031 D4）／LiveManual に持ち込むこと（人間所有モード・対象外）／`start/end/cash/granularity` まで cell から書けると読むこと（ADR-0031 が supersede したのは universe facet **のみ**）
...
**Appearance（Dark / Light）/ [[Whiteboard（light テーマ・Miro 風）]] / [[Card chrome ⇔ HUD chrome]]** ※2026-06-25 新設・方針 [[ADR-0028]]:
**Appearance** ＝ アクティブな配色変種のラベル（Dark / Light）。#44 時点では「どちらが有効か記録するだけ」の非挙動ラベルだったが、
ADR-0028 で **ユーザが設定ダイアログ（[[ADR-0026]] の集約口）で切替できる設定値**に格上げ。選択は **アプリ全体グローバル**（per-document
sidecar ではない・`PlayerPrefs` 等の単一キー）に永続化し、boot 時に最初のテーマ適用前へ読み込む。切替は **ライブ**（[[ThemeService]]
`SetTheme`→`Changed` で全サーフェスがその場で再適用）。Dark は現行の宇宙 HUD のまま温存し、Light を**追加**する（dark を退役しない）。
_Avoid_: Appearance を per-strategy / per-layout に紐づけること（アプリ全体設定が正）。

**Whiteboard（light テーマ・Miro 風）** ※2026-06-25 新設・[[ADR-0028]]:
明るい変種の総称。共有 chrome（エディタ／フッター／サイドバー／メニュー／モーダル）は **本物の Radix light scales**（#51 で予約・
`ColorScales.Light()` の本実装）で導出。キャンバス側のオーナー直値ロール（`workspace_background` ＝盤面・`hakoniwa_*` ＝タイル/
チャート/板）は **appearance 別の light 直値**を持つ（スケール非依存・隔離継ぎ目は維持）。Miro 的な「色じゃない要素」のうち
**ドット方眼の盤面背景・カードのドロップシャドウ・角丸カード**を light のときだけ適用（[[Card chrome ⇔ HUD chrome]]）。
**カラフルな付箋アクセントは不採用**（タイトル帯は accent 系のまま）。Dark との差は TTWR parity（dark 既定）からの **意図的な
divergence**＝ADR-0028 に理由を固定（移植元 TTWR は dark/light 両方を持つので light の存在自体は parity 沿い）。
_Avoid_: Light を「dark の色違い」とだけ捉えること（盤面の方眼・カードの影/角丸という構造差を含む）。

**Card chrome ⇔ HUD chrome** ※2026-06-25 新設・[[ADR-0028]]:
浮遊 window 1 枚あたりの装飾。**HUD chrome**（`HudFrameChrome`＝シアンのコーナー括弧＋縁グロー・dark 専用）⇔ **Card chrome**
（ドロップシャドウ＋角丸・light 専用）を **appearance で構造ごと切替**。ライブ切替なので、フレームは [[ThemeService]] `Changed` を
購読し（現状は未購読＝build 時焼き込み）、装飾サブツリーを再生成する必要がある＝ADR-0028 の load-bearing な配線。
_Avoid_: 装飾を「色の塗り替えだけ」で済むと見積もること（HUD↔Card は GameObject 構造が違うので再生成が要る）。
...
「操作して確定成功したら Settings を自動クローズ」する**（[[Settings 自動クローズ（単発アクション確定成功）]]・ADR-0039）。**フォーム系
...
その操作が**確定成功した時点で Settings モーダルを自動で閉じる**（ADR-0039・ADR-0026 の「Venue 送信後も開いたまま」を部分 supersede）。
...
**銘柄表示ラベル（`<id> <name>`）**:
ユーザ向け面で 1 つの instrument を呼ぶときの**共通表示文字列**。整形は `<id> <name>`（id は venue suffix
付きフル id・例 `7203.TSE`、name は listed_info `CompanyName` 由来・例 `トヨタ自動車`）。**name が null / 空 /
id と一致**の 3 ケースでは id 単独へ collapse（`7203.TSE 7203.TSE` 二重表示を防ぐ・picker 由来の de-dup
ルール）。name の唯一の source は **`engine.inproc_server.list_instruments(source, end)` RPC** の `Names`
並列配列（universe SoT＝`_scenario.Universe` にも poll state JSON `per_instrument` にも name は存在しない）。
代表的な consumer は (a) sidebar `+Add` [[picker]] の候補行（`PickerRow.Candidate`）、(b) **チャート窓
（id `chart:<iid>`）の窓枠 chrome タイトル**（#140 / findings 0112）。両 consumer は **共有 helper
`InstrumentLabel.Compose(id, name)` を経由**し整形 drift を構造的に断つ（drift＝picker と chart 窓で
collapse 条件が片方だけ更新されて表示が分岐する事故）。**Live 更新は持たない**：picker は open 時の
`replayEndDate` snapshot、chart 窓はその spawn 時の provider cache を読むだけ。spawn 時点で name 未取得
（Loading / Empty / Error / Unsupported / NotConnected / EndUnset）なら **id 単独で確定**（後追いの
タイトル差し替え seam を意図的に持たない＝owner 決定 2026-06-25）。常用パス（picker 経由 add 直後）は
cache が温いので名前が出る。layout 復元など picker 未経由 spawn では cache 未温で id 単独になり得る。
記録: findings 0024（picker shape）/ findings 0105（kabu listed_info supply）/ findings 0112（chart 窓）。
_Avoid_: chart 窓タイトルを固定文字列「Chart」のままにすること（複数チャート窓で銘柄判別不能・#140 の本旨）／
chart 窓タイトル用に listed_info 名前を別経路で fetch する **2 つ目の name source** を新設すること（
list_instruments RPC が唯一の source・SoT 二重化禁止）／spawn 後に name が遅延到着したらタイトルを差し替える
