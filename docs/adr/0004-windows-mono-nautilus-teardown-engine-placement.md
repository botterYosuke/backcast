...
- → **リコメンドは案 B（別プロセス）**。zero-copy/「Unity死=Python死」の能動保証という ADR-0001 の核を覆すが、
  decision 6 の安全要件を満たすにはこれが確実。**最終確定は owner**（zero-copy/レイテンシ要件 vs 安全要件の
  優先順位）。owner が案 A を選ぶ場合は、多重 CRT FLS teardown を回避する具体策（例: nautilus を Mono プロセス
  内で動かしつつ thread/process を一切 teardown させない別機構）の実現性を別途要検証。
