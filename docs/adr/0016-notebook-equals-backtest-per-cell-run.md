...
- 駆動 operation を**呼ばない** cell → **marimo native reactive 再計算**（`thin_drain.HeadlessKernel` を流用する interactive 経路。per-bar 用 frozen-list は使わない）。
...
`_REPLAY_BAR_INTERVAL_SEC = 0.01`（`_backend_impl.py:189`、hardcoded constant）を、**host 所有・thread-safe・live-mutable な速度レジスタ**へ置き換える。配置は **`DataEngine`（`python/engine/core.py:18`） 上、`_replay_stop_event`（`core.py:43`）の隣**。loop は **毎 bar 読む**（`sleep(base / speed)`）。
