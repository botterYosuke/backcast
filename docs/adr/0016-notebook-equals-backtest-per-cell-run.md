...
> **実装状態**: 本 ADR は #95 Phase 1 で **accepted**（docs + glossary のみ）。実 cell-facing API `bt` ／ `KernelRunner` の state-machine 化／速度（`bt.replay(bars_per_second=N)` pacing）／per-cell RUN ボタン配線は #95 Phase 2–6 の named slice で順番に landing する。`status: accepted` は **decision が確定**したことを指す（ADR-0012 と同じく「accepted だが実装は段階的」）。
...
- 既存実装の状況（2026-06-19 時点）:
  ...
  - `_replay_stop_event`（`DataEngine`・`core.py:43`、property `core.py:272`）= 唯一残る run-control event
...
「dry-run のように軽く動かしたい」ニーズは、`bt.replay()` / `bt.step()` を**呼ばない** cell（engine 非接続の純粋計算「**土台**」層 = ADR-0013 cell-DAG の reactive 再計算だけが走る・engine sink を一切触らない cell）で代替する。**駆動と参照の区別**は厳密に保つ:
...
- **`bt.submit_market(qty)`** = `bt.replay()` / `bt.step()` の per-bar context 内**でのみ有効**（KernelRunner の `ctx.submit_market → ctx.pending → fill@close` 経路を踏むため）。**context 外の呼び出しは Phase 3 で `ValueError` を raise（fail-closed）**＝silent no-op や denial 経由ではなく、明示的な exception で著者の typo を即時可視化する。
...
- 駆動 operation を**呼ばない** cell → **marimo native reactive 再計算**。実体は **`thin_drain.py:68` が既に import 済みの marimo `Runner` を、押した cell を root に in-proc 駆動し出力を捕捉する net-new 配線**（`HeadlessKernel` 単体の流用では届かない＝Kernel を組むだけで reactive single-cell 実行も per-cell 出力捕捉 seam も無い・findings 0070 F1）。per-bar 用 frozen-list は使わない。
- `bt.replay()` / `bt.step()` を**呼ぶ** cell → 実 backtest を駆動（`bt.replay()` で全 bar / `bt.step()` で 1 bar）。

**専用の batch RUN ボタン（#76/#81 merge U1 の global ▶ Run）・title-bar Run（現行 `Assets/Scripts/StrategyEditor/StrategyEditorRunButton.cs`・ADR-0005 由来の 1:1 TTWR parity 単一 Run ボタン surface の現実装で、ADR-0012 partial supersede が strategy-authoring 表面の文脈で明示退役しなかった residual）・global transport（#30 既退役）を本 ADR で formal に supersede** する。「アプリ全体で 1 つの再生ボタン」は ADR-0005 の 1:1 TTWR parity facet で、ADR-0012 が strategy-authoring 表面を部分 supersede した際に reactive cell-DAG モデルとの整合を将来 slice に委ねていた——本 ADR で**その residual を明示退役**する（facet-scoped supersede）。
...
- `bt.replay(bars_per_second=N)` は呼ばれるたび **pointer を 0 に reset** して end まで走り、完走時に pointer = end を残す。
...
- replay 実行中に同じ `bt` へ別の RUN が入った場合（駆動 cell の重複 RUN・別の駆動 cell の RUN・config 再 commit 起因の `bt` 作り直し競合）は **running guard でブロック**: 第二の RUN は exception を上げて即時拒否（queue や no-op ではない）、user には UI block popup（D11 Phase 6）で「`bt` is already running」を示す。実装と guard の正確な配置は Phase 4/6 で固定。
...
帰結: **#24 golden は byte-identical で保持**（**byte-identical の権威は ADR-0012 §2**「kernel per-bar 契約を不変の adaptation 境界とする」＝命令型経路の equity_curve は #24 golden に byte-identical。**ADR-0006 は #24 golden を凍結回帰 fixture として保持する不変条件**を持ち、本 ADR は両者を継承）。命令型経路は `KernelRunner` 無改修のため自動で保たれ、marimo 経路は wrap によって per-bar 順序が同一になる。
...
`bt.replay()` の per-bar 駆動は **既存の #65 sink 経路をそのまま push** する。`runner.py:237-303` の per-bar 順序を引用すると:

- 各 bar で `push_bar`（line 237）→ `on_bar`（243）→ denials は `on_order` のみ・sink へは push しない（246-248）→ accepted MARKET fill は `push_order` / `push_portfolio` を **fill 1 件ごと**に push（254-265、条件: `OrderStatus.ACCEPTED` ＋ fill 成立）→ **`on_equity` は無条件**で毎 bar push（282-284）
- これらの push を `ReplayKernelObserver` が受け、`engine.last_portfolio` を atomic swap で更新（#65）
- poll lane (`LiveRpcLanes.get_state_json`) が Hakoniwa を bar-by-bar に追従させる

**新 sink を作らない**。`push_order` / `push_portfolio` は fill が無い bar では何も流れない（既存契約・正常）。`bt.step()` も同一の sink 経路を踏むので、1 bar 分の状態（fill があれば fill も）が反映される。

### D8 — 速度（撤去機能の意図的復活・視覚機能として純化）

`_REPLAY_BAR_INTERVAL_SEC = 0.01`（`_backend_impl.py:189`、hardcoded constant）を**撤去**する。速度は **`bt.replay(bars_per_second=N)` の call-time 引数**で与え、**`replay()` 開始時にキャプチャする値**とする（**live-mutable な速度レジスタにはしない**＝走行中に変える手段が無く・RUN は順番待ちなので不要・findings 0070 F6）。**単位は名前に込める**: `bars_per_second` ＝「1 秒あたり N bar 描画」で **大きいほど速い**。bare な `speed=`（無次元の倍率に誤読される・旧 transport `[1,2,5,10,50]` 文脈と混線する）は **public API に出さない**。

- **`bars_per_second` 指定が無いとき**：明示 sleep を入れず **worker は全力で回す**（GIL ハンドオフは D9 の CPython auto-switch に任せる）。
- **`bars_per_second=N` 指定があるとき**：目で追うための**視覚機能**として per-bar に **概ね `1 / N` 秒**の pacing sleep を挿入しエンジンを意図的に遅くする（`N` は開始時キャプチャ値）。

ユーザー API は **`bt.replay(bars_per_second=N)` の引数のみ**。**UI コントロール（slider / pacing button）は作らない**。`bt.step()` には速度は無意味で、`bars_per_second` 引数を持たない（ユーザー自身が pace する＝ボタンを押すごとに 1 bar）。**cross-thread レジスタは stop（`_replay_stop_event`・`core.py:43`）だけ**で、速度はレジスタ化しない。

> **過去の文脈との関係**: #30/#68 で `▶/⏸/⏭/⏹ ＋ [1,2,5,10,50]` transport が削除された (`core.py:42` の在席コメント / `backend_service.py:387` / `WorkspaceFooterView.cs:5` / findings 0046) のは「reactive drain は瞬時に終わる前提」だった。B2 の visual playback ニーズが前提を変えたので、**pacing だけを `bt.replay(bars_per_second=N)` という形で意図的に復活**させる。UI ボタンは復活させない。**silent な復活でなく、findings 0070 に復活理由を明示記録**。

### D9 — run は worker thread・GIL は auto-switch に任せる（床は入れない）

`bt.replay()` の長走り loop は **worker thread** で実行（`BackcastWorkspaceRoot.OnRun → TryStartRun` と同型・Unity main thread を塞がない）。エンジンは Unity 本体とは別の **worker スレッドに 1 個だけ**置き、計算 cell も `bt` cell も同じ 1 個の marimo エンジンを通す（marimo Kernel/RuntimeContext はスレッドローカル＝1 Kernel を 2 スレッドから触らない・findings 0070 F5。replay 走行中の別 cell RUN は順番待ち）。

**GIL ハンドオフ**：`bars_per_second` 未指定時は明示 sleep を入れず、**CPython の自動切替（`sys.setswitchinterval` 既定 ~5ms）に任せる**。**「Hakoniwa 更新のためにエンジン速度を縛る GIL 床（per-bar 強制 sleep）」は入れない**——owner 優先順位は「最悪なのは Hakoniwa の都合でエンジン速度を拘束すること／見た目の遅れは許容」（findings 0070 F6・`python/spike/gil_handoff_spike.py` PASS）。`bars_per_second` 指定時の per-bar pacing sleep（概ね `1 / N` 秒）が GIL 解放点を兼ねる。**Phase 4 頭で Unity AFK で auto-switch を実機 reconfirm してから「明示 sleep 撤去」を最終 lock**（RED なら `sys.setswitchinterval` 調整へ pivot）。
...
| 4 | B2 `bt.replay()`（#97 を吸収） | running snapshot + 速度（`bars_per_second=N` pacing・開始時キャプチャ）+ stop |
...
- **速度の user-facing API**: 採用＝**`bt.replay(bars_per_second=N)` 引数のみ**（**開始時キャプチャ値**・live-mutable レジスタにはしない・将来 HITL slider は additive ADR で解禁可能）。**却下**＝UI コントロール復活（#30/#68 footer transport を再実装することになり退役判断を再 open）／reactive `mo.state` ベース（cell 配線複雑化・現スコープでは過剰）／**per-bar 強制 sleep の GIL 床**（Hakoniwa の都合でエンジン速度を縛る＝owner 優先順位に反する・findings 0070 F6）。
...
- **ADR-0005 由来の「アプリ全体で 1 つの再生ボタン」facet（ADR-0012 partial supersede が strategy-authoring 表面の文脈で明示退役しなかった residual）**: **本 ADR で却下・supersede**（D2 = per-cell RUN）。ADR-0005 / ADR-0012 本文は無改変（両者の自己保護条項）。
...
- **`_REPLAY_BAR_INTERVAL_SEC` 定数撤去**: Phase 4 で撤去し、pacing は `bt.replay(bars_per_second=N)` の開始時キャプチャ値へ置き換える（live-mutable レジスタは作らない・findings 0070 F6）。constant 参照（`_backend_impl.py:189`/`922`）は除去または `bars_per_second` 引数経路へ rewire。
...
- **走行中の GIL ハンドオフ**は `bars_per_second` 未指定時は CPython auto-switch（~5ms）に依存し、明示 sleep 床は持たない（findings 0070 F6・spike PASS）。`bars_per_second` 指定時のみ pacing sleep が解放点を兼ねる。poll thread（`LiveRpcLanes` 30s default）と Hakoniwa の bar-by-bar 追従の整合は #65 の既存契約のまま（書き手 back-pressure ゼロ）。
...
- **下位の実装事実**（`bt` 実装の正確な class／KernelRunner state-machine 化の seam／速度 pacing の正確な capture timing と per-bar sleep の挿入点／running guard の構造／per-cell RUN button の C# 配線詳細）は本 ADR に書き戻さず **Phase 2–6 の `docs/findings/` に記録**し、本 ADR を「方針: ADR-0016」として参照する。
