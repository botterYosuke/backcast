...
4. **デフォルト visible 範囲**: 起動時 / `ResetView` 時は **右端 anchor + `cell_width_px = DEFAULT_CELL_WIDTH_PX`**（plot 幅 540px なら ≈90 本表示）。Python が `max_history_len=1000` 本投げてきても**描画 vertex 数は ~180**（90 bar × 2 quad）に圧縮。Replay で scenario が数千本でも初期表示は ~90 本固定。
   - **追補（#156 follow-up・owner 2026-06-27）**: この「初期 ~90 本固定」は **opt-in の fit-to-all で上書き可**。owner 要望「初期化時にチャートに全期間の bar を見せたい」を受け、`ChartViewState.fit_all_on_autoscale`（既定 false）を `true` にすると `ResetView` が `cell_width = clamp(plot_width / span_slots, MIN, MAX)` で**全 series を 1 画面に fit**（**fit は bar 数でなく時間 span = `(last-first)/basis+1` で算出**——チャートは時間で配置/仮想化するので count で割ると gapped 日足で最古 bar が左端外に脱落する）。溢れる場合は **MIN クランプ＋右端寄せ**（owner 決定）。`BackcastWorkspaceRoot` が **Replay チャートのみ** true を立て、Live は false（recent ~90 本を 6px で維持）。下位事実・gate は findings 0119 「D-4 追補」。
