...
## 5. 検証ステータス（全 GREEN — Mac leg・#26 クローズ済み）

**Phase A（characterization）＋ Phase B（realtime 全置換）の AFK 権威ゲート、および owner HITL 目視すべて Mac leg で GREEN**。当初は Windows-Mono の live-path crash でブロックされていた（下記履歴）が、issue comment が示したクローズ条件「(a) Mac leg で実走」を満たして解消した。
...
- [x] `Tools > Backcast > Depth Ladder HITL` 目視（owner の手動 playmode leg）**GREEN（2026-06-14・Mac）**。実 Canvas 上で 5×5 ladder（ASK 999.9→995.9 赤 / spread / BID 994.9→990.9 シアン）が mid drift で `updates: 40`（40 tick 全置換）まで更新され、`stream complete — board holds last snapshot`・Console error 0 で終了。**前提**: `ReplayPanelsHarness.AutoBootstrapEnabled` が `RuntimeInitializeOnLoadMethod` で Play 毎に Python を先取りするため、menu-driven な本 HITL を回すときは一時的に `false`（Play を譲る）→ 実行後 `true` に戻す（findings 0011 #20 Live tracer HITL と同じ運用。コメントに明記済み）。`#34`（catalog precision mismatch・このマシンの nautilus が high-precision）は ReplayPanels 専用で本 HITL（mock venue / LiveAuto）には無関係。
