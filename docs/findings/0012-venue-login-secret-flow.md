...
- **Tachibana demo HITL**: ☐ 未実施。`Tools > Backcast > Venue Login Secret HITL (Tachibana demo)`。
  connect → place → SecretRequired → modal → submit → fill → logout。実施したら
  日時 / OS / Unity・Python 版 / 結果（秘密値は記録せず known-marker 不在で判定）を本欄に追記。
