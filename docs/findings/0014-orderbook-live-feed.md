...
issue 本文「不正段は当該段を skip し feed を止めない（既存 DepthCache の方針を踏襲）」に従い、**`_extract_depth` の段フィルタを「空欄」から「空欄・非数値・非有限（NaN/Inf）」へ拡張**（`_is_finite_quote`）。これで:

- `""`（欠損段）→ 従来どおり skip
- `"—"` 等の非数値 → 段ごと skip（`float()` を割らせない＝接続断を防ぐ）
- `"0"` / 指数表記 → 有限数なので段は残す（price≤0 の最終弾きは §2.1 の `DepthCache` gt=0 が担う二段防御）

`_is_finite_quote` は **後段 `_cb` と同じ `float(v)` でパースし `math.isfinite` で判定**する（`try: math.isfinite(float(v)) except ValueError: False`）。これで「guard が True を返した段は `float()` が必ず raise せず有限値」という不変条件が成立する。当初の `Decimal(v).is_finite()` は `"1_"`/`"1__0"` 等の underscore 区切りを Decimal が受理する一方 `float()` は `ValueError` を投げる乖離があり、その不正段が無防備な recv-loop float() を割り得た（code-review 2026-06-14 で指摘・float ベースへ是正）。`"1e400"` も `float→inf` を `isfinite` が codec 段で弾く。

`test_tachibana_depth.py` に RED→GREEN（`test_depth_skips_non_numeric_level`）＋境界（`test_depth_keeps_zero_and_scientific_notation`）＋不変条件（`test_is_finite_quote_implies_float_is_safe`: guard True ⟹ `float()` 安全、underscore も skip）を追加。
...
- [x] `test_tachibana_depth.py` 8 passed（非数値段 RED→GREEN ＋ `_is_finite_quote` 不変条件実証済）
