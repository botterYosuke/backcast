...
**owner-run HITL（2026-06-15・部分 pass）**：1画面同時表示（AC①）・無限空間フィールドの全画面化（chrome の裏も
背景・skybox 消失）・Python engine 1つ生成 を owner が実機 Play で確認（`pass!!`）。残る HITL 項目（zoom 下の
Hakoniwa swap・scenario 入力後の実 Replay streaming・File→Save→再 Play 復元・実 Play 停止 teardown・root 稼働中の
per-part Python HITL 拒否）は引き続き owner 確認。これら全 HITL 受入完了で ADR-0009 を `accepted` へ昇格する。
