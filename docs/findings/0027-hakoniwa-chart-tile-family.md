...
| **④** ~~#63~~ | box 位置/サイズ永続化 ＋ drag-handle 移動/リサイズ | ADR 0015/0016 | **ボツ**（#63 close 2026-06-24） |

> **stage④（#63）はボツ。** [ADR-0017](../adr/0017-hakoniwa-dockable-floating-windows.md)（2026-06-21）が split-grid 機構（`HakoniwaController`/`HakoniwaGridMath`/box-grow/`HakoniwaRoot` bounded box）を**退役**し、各パネルを独立 floating window 化した。box の移動/リサイズは floating window の自由配置＋磁石スナップ（[ADR-0024](../adr/0024-puzzle-feel-drag-magnetic-snap-swap-translate-detach-merge.md)）が担い、geometry（x/y/w/h）は `LayoutDocument.floatingWindows` で**既に永続化済み**。よって「単一 root box の drag-handle 移動/box geometry 永続化」という #63 の射程は **obsolete（root box が消えた）＝既存 floating window seam で代替済み** となり、別 slice として起こさない。stage① C（本 findings）の derived box-grow も ADR-0017 で `HakoniwaGridMath` ごと退役した（findings 0075）。
...
- **不採用（= C 外・後続 stage の additive 拡張）**:
  ...
  - **box 位置/サイズの永続化 ＋ drag-handle 移動/リサイズ**（ADR 0015/0016・findings 0007 §0/§9 deferred）→ ④ ~~#63~~ **ボツ**（ADR-0017 で split-grid 退役・floating window geometry が代替＝§0 冒頭注を参照）。C の box-grow は **derived（persist しない）**。
...
- **box 位置は固定・box サイズだけ universe 数から毎回再計算**（復元時も universe から導出するので安定）。TTWR が `HakoniwaSnapshot` に box_position/box_size を persist するのは **drag-handle で箱を動かせる（ADR 0016）から**であり、backcast はそれを ④（#63）に deferred した（その後 ④ はボツ・ADR-0017 で floating window へ移行）ので C では persist 不要。N が多くてもタイルは min size を保つ（固定 box だと 10 銘柄で ~175×100px となり実用性が落ちるため derived-grow を C に含める）。
...
- モード別 base tile ＋ base パネルのタイル化（② B+M）／per-mode profile（③ P）／box 位置・サイズの永続化＋drag-handle 移動/リサイズ（④ ~~#63~~ **ボツ**・ADR-0017 退役で代替）。
