...
- **不採用（= #62 外）**:
  ...
  - **box 位置/サイズ・cols/rows（divider）の永続化＋per-mode 化** → stage④ #63（ADR 0015/0016）。TTWR は box geometry を `HakoniwaSnapshot` 直下（**共有**）、cols/rows を `HakoniwaGridSnapshot`（per-mode）に持つが、backcast は #63 で同コンテナ（`HakoniwaProfile`）へ additive 拡張する。#62 の box は #60 の derived-grow（n_total 由来）を維持。
...
disk: LayoutDocument {
  ...
  hakoniwaProfiles: HakoniwaProfiles {                 # ← 新 SoT（additive・#63 拡張点）
    replay: HakoniwaProfile { panels }                 # #63: + cols/rows/box
...
- box 位置/サイズ・cols/rows 永続化＋drag-handle/divider resize（④ #63）／canvas/window/editor の per-mode 化／Replay パネル実データ（follow-up #65）。
