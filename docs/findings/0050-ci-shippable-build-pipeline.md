...
### P1. 成果物は owner-machine state から escape する

本 CI workflow は「shippable artifact が clean machine で再現可能」という thesis を成立させるために **GitHub-hosted runner** を選択する。これは ADR-0002（embedded runtime placement）の「verbatim copy で standalone が動く」要件の延長で、build host の env 汚染を artifact に乗せると検証不能な fail mode が生まれる。

この原則を覆す変更（hosted → self-hosted 等）は、本 findings の update では不十分であり、**新規 ADR（superseding 候補: ADR-0001 d3 / ADR-0002 §基準）の起案が必要**。
...
### 2.2 Runner choice（Q2）— Principle P1 適用

| 項目 | 値 | 根拠 |
|---|---|---|
| `runs-on` | **`windows-2022`**（pin） | reproducibility thesis を image にも適用。`windows-latest` は moving target（Microsoft が随時 bump） |
| Unity activation | **GameCI `game-ci/unity-builder@v4`** + Personal license (`UNITY_LICENSE` secret) | owner-machine activation を CI から escape |
| Image bump policy | `windows-2025` GA 後に explicit PR で bump、本 findings にも記録 | reproducibility |

#### 2.2.1 Self-hosted を採らない理由

- artifact thesis（"clean machine で再現可能"）を成立させるのは hosted のみ
- single point of failure（owner machine offline = CI 停止）
- `GITHUB_TOKEN` を personal machine に持たせる security trade-off
- Q2 grill で full analysis 済み

### 2.3 Cache strategy（Q3）— 3 層 + 4 guard rail

| Layer | 対象 | Raw size | Compressed (zstd) | Cache key |
|---|---|---|---|---|
| **Library** | Unity の asset import 後 cache | 6–10 GB | 5–7 GB | `${{ runner.os }}-Library-${{ hashFiles('Packages/packages-lock.json', 'ProjectSettings/ProjectVersion.txt', 'ProjectSettings/ProjectSettings.asset') }}` + `restore-keys: ${{ runner.os }}-Library-` |
| **Unity Editor** | `C:\Program Files\Unity\Hub\Editor\6000.4.11f1\` | 5 GB | 3–4 GB | `Unity-Editor-${{ env.UNITY_VERSION }}` |
| **uv wheel cache** | `$LOCALAPPDATA\uv\cache` | 0.5–1 GB | 0.3–0.6 GB | `uv-wheel-${{ hashFiles('python/uv.lock') }}` |

合計 worst case ~12 GB — repo 10 GB cap 超過リスクあり。以下 4 guard rail で対応:

#### 2.3.1 Guard rail A: ProjectSettings.asset を Library key に含める

`scriptingBackend` が Standalone で IL2CPP に変更されると Library は無効化されるが、key に `ProjectSettings.asset` が無いと stale Library で起動して native plugin load が壊れる。ProjectSettings.asset 全文を hash することで scripting backend 変更を確実に検知する。

**Mono2x の現状（latent observation）**: 2026-06-18 時点で `ProjectSettings.asset` の `scriptingBackend:` map は `Android: 1` のみで、Standalone は明示エントリ無し → engine default の Mono2x が適用される。ADR-0001 d4 の Mono2x 要件は **「明示 pin」ではなく「default に乗っている」**状態。`Standalone: 0` を ProjectSettings に明示するのは本 issue の scope 外だが、将来 GUI から触る人がいると silent flip するリスクは残る（本 findings の latent observation として記録）。

#### 2.3.2 Guard rail B: Library size を fail-fast 計測

```pwsh
- name: Measure Library size (cap=8GB raw / ~5-6GB compressed)
  id: lib-size
  run: |
    $size = (Get-ChildItem Library -Recurse | Measure-Object Length -Sum).Sum / 1GB
    "library_size=$size" | Out-File -Append $env:GITHUB_OUTPUT
    if ($size -gt 8) {
      Write-Error "Library bloat: $size GB (> 8GB raw cap)"
      exit 1
    }
```

Library が想定外に育つと cap を圧迫し LRU thrash を引き起こす。raw 8GB を超えたら fail-fast。fix path: PackageCache 清掃 / 不要 asset 削除 / Editor cache を切り捨て（下記 C）。

#### 2.3.3 Guard rail C: Editor cache の切り捨て fallback

Library cache が圧迫されたら Editor cache を犠牲にする方針:

- **理由**: Editor cold install は GameCI 経由で bounded ~7-10 min（変動小）/ Library cold は asset 量依存で 6-15 min（変動大）→ 予測可能なコストを毎回払い、予測不可能なコストを cache で吸収するのが正しい trade-off

実装: cache step を `if: ${{ vars.CACHE_EDITOR != 'false' }}` で gate し、`gh variable set CACHE_EDITOR false` で repo level に切れる。

#### 2.3.4 Guard rail D: `Unity_lic.ulf` は cache に乗せない

`%APPDATA%\Unity\Unity_lic.ulf` は GameCI が runtime activation するので毎回 fresh。Editor binary のみ cache する。

### 2.4 Secrets and license activation dance（Q5）
...
Unity license server が UI を要求するため、自動化は不可能。手順:

1. `request-activation.yml`（`workflow_dispatch` 専用 helper、永続保持）を repo で実行
2. Workflow が `game-ci/unity-request-activation-file@v2` で `Unity_v6000.x.alf` artifact を生成
3. Owner が `.alf` を download → https://license.unity3d.com/manual に手動 upload（Unity ID でログイン）
4. `.ulf` がブラウザに download される
5. PowerShell で base64 encode（**しない方が良い**: GameCI は `.ulf` 全文をそのまま secret に入れる方式が安定）:
   ```pwsh
   $ulf = Get-Content -Raw .\Unity_v6000.x.ulf
   ```
   この `$ulf` を GitHub repo secret `UNITY_LICENSE` にそのまま貼り付け
6. `request-activation.yml` 自体は **削除しない**（翌年の renewal で再利用）
...
| License activation helper | `.github/workflows/request-activation.yml` |
...
- **ADR-0001 は不変**。本 CI は executor in-proc parity を build leg literal-text で実証する probe（findings 0049 §2.6）を **hosted runner で再現可能化**するだけで、d3/d4 の文言は変えない。
- **ADR-0002 は不変**。本 CI は ADR-0002 の「verbatim copy で standalone が動く」要件を hosted で **再現可能化**するだけで、配置基準は触らない。
- **CONTEXT.md は不変**。CI workflow は domain glossary を持たないので CONTEXT.md には記載しない。

## 6. 初回 onboarding setup checklist

新マシン / new contributor / 年次 license refresh で再走する手順:

### Step 1: GitHub repo secret 投入

| Secret | 内容 |
|---|---|
| `UNITY_EMAIL` | `sasaicco@gmail.com` |
| `UNITY_PASSWORD` | Unity ID password |
| `UNITY_LICENSE` | (後段 step 3 で取得した `.ulf` 全文) |

### Step 2: License activation dance（§2.4.2）

1. `gh workflow run request-activation.yml` を実行
2. workflow artifact から `Unity_v6000.x.alf` を download
3. https://license.unity3d.com/manual に `.alf` を upload（Unity ID でログイン）
4. ブラウザに download される `.ulf` 全文を `UNITY_LICENSE` secret に貼り付け（Step 1 を update）
5. `request-activation.yml` は **削除しない**（翌年の renewal で再利用）
