...
## 実機 reconfirm 結果（P4-5）— **GREEN（sleep 撤去 lock 確定）**

`_REPLAY_BAR_INTERVAL_SEC` 撤去（imperative Replay path を `bar_interval_sec=0`＝全力）後、実機 Unity AFK `ReplayToHakoniwaE2ERunner` を再走:

```
[E2E REPLAY→HAKONIWA PASS] real kernel streamed 50 bars into the hakoniwa chart tile
via the production render path.   exit=0
```

明示 sleep 無しでも、worker thread の tight loop が GIL を握りっぱなしにせず、CPython auto-switch（~5ms）が C# poll lane（foreign thread `Py.GIL()`）へ GIL を渡し、50 bar が `get_state_json`→`InstrumentOhlcDecoder`→`ChartView.Render` を通って Hakoniwa に届いた（run 完走＋render 完了）。spike（`gil_handoff_spike.py`・Python thread reader）の結論を **foreign-thread の実機で reconfirm** ＝ **「明示 sleep 撤去」を最終 lock**（setswitchinterval floor pivot は不要）。

> 注: 全力 run は瞬時に終わるので「逐次 watch」は本質的に不要（owner F6 優先順位「見た目の遅れは許容」）。逐次 watch が要る visual playback は `bars_per_second=N` の pacing sleep が GIL 解放点を兼ねる（§P4-2）。reconfirm が証明したのは「**sleep 撤去で run/render 経路が壊れない**（deadlock/starvation 無し）」こと。

## 実装着地（`feat/#95-phase4`）

P4-1〜P4-7 の設計どおり実装し、done-gate 1–12 を GREEN 化した。

### landed したコード

| 物 | 場所 | 内容 |
|---|---|---|
| pacing | `kernel/stepper.py` `set_pacing` ＋ `backtester.py` `replay()` | `bars_per_second → 1/N` を replay 開始時にキャプチャ（未指定→0）。`_REPLAY_BAR_INTERVAL_SEC` 撤去（`_backend_impl.py`・imperative path も full-speed） |
| host hooks | `backtester.py` | `on_run_begin`（駆動初回に engine RUNNING）/ `was_driven`・`result`（host finalize）/ `arm`・`disarm`（cold-run guard）。`Backtester` は thin/marimo-free のまま |
| type fix | `kernel/orders.py` `signed_qty_to_side` | `abs(qty)` で int 型保存（fill record `"100"` ≠ `"100.0"`＝production parity の byte-identity） |
| injection | `notebook_session.py` `run_pressed(inject=)` | host free ref（bt）を kernel globals へ。cold-run は disarm・press は arm |
| lifecycle wrap | `_backend_impl.py` `run_cell(scenario_json)` ＋ `_build_notebook_bt` | bt 駆動 cell でだけ `load_replay_data`＋`ReplayKernelObserver`(#65)＋`stop_event` で bt 構築・注入・terminal で RunBuffer finalize→summary＋force_stop |
| RPC | `backend_service.py` / `inproc_server.py` | `scenario_json` を run_cell に通す |
| C# scenario/guard/stop | `NotebookRunLane`/`NotebookRunController`/`HostNotebookCellExecutor`/`WorkspaceEngineHost`/`BackcastWorkspaceRoot`/`StrategyEditorWindowFrame` | `ScenarioJson`＋`RunId`・running guard（RunId 相関 reject）・▶→■ stop トグル（`SetRunButtonGlyph`・`force_stop_replay`）・`BuildNotebookScenarioJson`・summary surfacing（`SetReplayRunSummary`） |

### done-gate 結果

1. **GIL reconfirm 実機 AFK GREEN**（§P4-5・`ReplayToHakoniwaE2ERunner` PASS・sleep 撤去 lock）
2. **既存 #24 / golden 系 green**（`_REPLAY_BAR_INTERVAL_SEC` 撤去後も byte-identical・215 kernel/golden/cell-api/v19 passed）
3. **production replay parity green**（`test_backtester_phase4.py` — observer 経由 byte-identical・float-qty leak を検出修正）
4. **pacing pytest**（`set_pacing` 1/N キャプチャ・full-speed default・per-bar sleep 実測・fail-closed）
5. **stop pytest**（mid-stream set → STOPPED・`test_notebook_replay_afk` cross-thread force_stop）
6. **running guard**（C# AFK STRATEGY-22・in-flight 中の第二 RUN reject）
7. **bt 注入・cold-run guard**（`test_notebook_interactive_run` の inject/arm/disarm・`test_notebook_replay_afk` 土台 cell が engine 非接続）
8. **offline import-purity 不変**（`test_strategy_runtime_offline` green・bt 注入経路は lazy）
9. **AFK 3 観点 GREEN**（GIL reconfirm／Python e2e `test_notebook_replay_afk`（replay→last_portfolio・pacing 実測差・stop）／C# `StrategyEditorNotebookE2ERunner` STRATEGY-21/22/23）
10. **CONTEXT 加筆**（per-cell RUN / replay 速度 entry に Phase 4 実装着地注記）
11. **findings 0073 landed**（本節）
12. **`code-review(simplify)` Medium+ 0**（後述）

### 実装中に確定した下位の下位決定

- **bt-driving 検出は `source.Contains("bt.replay" / "bt.step")`**（Python 側 run_cell ＋ C# 側 RunCell の両方）。純粋計算 cell は bt を一切構築せず Phase 2 経路（DuckDB load も engine 遷移も無し）。
- **replay は毎 press fresh bt**（Phase 3 stepper は forward-only single-use・reset は Phase 5）。ゆえに source-rebuild を跨ぐ state persist は **B2 では不要**（各 run が 0→end の atomic 操作）＝Phase 2 landmine（rebuild が live state を破棄）は **B3/step（Phase 5）の問題**。Phase 4 は arm/disarm で「cold-run が駆動しない」ことだけ保証すれば足りる。
- **float-qty leak（P4-6 が検出）**: `EventSink` は JSON 正規化で int/float 差を隠すが production `ReplayKernelObserver.push_order` は raw `str(fill.last_qty)`＝Hakoniwa Orders tile に `"100.0"` が出る。`signed_qty_to_side` が常に float 化していたのを int 型保存に修正＝imperative golden（`"100"`）と byte-identical。
- **summary surfacing**: bt run の finalize summary を `HostNotebookCellExecutor` が `run_summary` から抜き `WorkspaceEngineHost.SetReplayRunSummary` でセット＝title-bar Run と同じ run_result tile 経路。
- **running guard の RunId 相関**: 速い pure-compute run が先に drain して bt run の busy flag を誤クリアしないよう、busy clear は `result.RunId == _btRunId` でのみ（generation check より前＝notebook 差し替えでも flag は解放）。
