...
|---|---|---|---|
| S16 / STRATEGY-27,28 **per-cell stale** | 編集通知で該当窓に stale badge（amber）／press で idle へ戻り stale クリア／下流窓も badge | stale routing を「常に窓0」へ collapse → 別窓 badge で RED ／ press 後 clear を消すと badge 残留 RED | ✅ 着地（commit + AFK GREEN） |
| S17 / STRATEGY-29 **block popup** | bt running 中の第二 per-cell RUN → 通知ライン `bt is already running`／非ブロック press は通知無し（not-owner/server-not-ready も） | block guard を外すと通知無しで RED ／ 無条件通知にすると非ブロックで誤通知 RED | ✅ 着地（commit + AFK GREEN） |
| S18 / STRATEGY-30,31 **document badge（#90）** | Open→basename 常時可視／編集→`* name.py`／Save→`*` 消える／New→`Untitled` | badge 更新を消すと Open 後 stale title で RED（#90 AC4 文言非矛盾も） | ✅ 着地（commit + AFK GREEN） |
| S19 / STRATEGY-32,33 **rich output routing** | 4 text mimetype（plain／markdown→rich `<b>`／html `<table>`→pipe／未対応→`[mime]` labelled）を AFK で全経路 pin。`image/png`→RawImage は `Texture2D.LoadImage` の decode 能力を throwaway texture で probe し分岐: decode 可（HITL/GPU）＝full RawImage active assert／decode 不可（headless batch）＝mimetype 伝播のみ AFK 確認（`[image/png]` ラベルが routing 非 collapse を証明）＋RawImage active は **HITL-only**（S3/S8 と同じ LogWarning 降格） | AFK: controller の Mimetype passthrough を外す→`[image/png]` ラベル消失→RED ／ HITL: 全 mimetype を Text へ collapse→RawImage 非 active→RED | ✅ 着地（AFK GREEN・image decode は HITL） |

> **S19 image/png HITL 降格の裁定（2026-06-21 AFK 実走後）**: `-batchmode -nographics` でも `-batchmode`（GPU 許可）でも **image/png leaf のみ** RED で、S1–S18 と S19 前段 4 mimetype（plain/markdown/html/unsupported）は全 GREEN（`Found no leaked weakptrs` 後の grep＝flush-race でない）。`StrategyEditorContentBuilder` が RawImage を配線し（`view.Initialize(…, image)`）、controller `ApplyResult` が `SetOutput(Output, Mimetype, Data)` を渡し、view `SetOutput` は `mt=="image/png"` 分岐を最初に選ぶ——前段 4 text mimetype の GREEN が routing 健全を裏付ける。唯一の失敗点は view `TryDecodeImage` 内の `Texture2D.LoadImage` が headless graphics device 不在で PNG を decode/upload できない**環境制約**（routing バグでない）。よって production code は無改変、S19 の image leaf のみ「decode 能力 probe で分岐＝GPU 不在なら mimetype 伝播のみ AFK・RawImage active は HITL」へ降格（S3 の save-failure・S8 の glyph-count と同型の HITL 降格 precedent）。
...
> **着地（2026-06-21・Slice 9）**: U4→**FOOTER-13**（`FooterModeE2ERunner`・footer transport-retired）／ U5→**SCENARIO-15**（`ScenarioStartupE2ERunner`・startup-no-Run）へ re-home 済。commit `d19dec6` で section を移送した際 C# ラベルに **FOOTER-11／SCENARIO-13** を焼いたが、その番号は台本で実 venue Live HITL（`FooterModeE2ERunner.md` FOOTER-11）／theme 再描画 HITL（`ScenarioStartupE2ERunner.md` SCENARIO-13）に既割当だった（Action ID 衝突）→ 空き番号 **FOOTER-13／SCENARIO-15** へ統一（lead 裁定=案A）。assertion ロジックは不変（footer に transport 無し／startup tile に Run 無し・文字列のみ変更）。AFK GREEN（commit d19dec6；本ターンのラベル修正は文字列のみゆえ GREEN 維持・lead 再走で再確認）。**RED litmus**: `WorkspaceFooterView.Build` に `MakeButton(bar,"▶",…)` を足すと FOOTER-13 RED／`ScenarioStartupTile.Build` に `MakeButton(tile,"Run Replay",…)` を足すと SCENARIO-15 RED。
...
### 3e. 実装着地（landed・#95 Phase 6 全スライス・2026-06-21）

全スライス GREEN で着地（commit: Slice1 `b5cc80e` / Slice2+3 `418f8d9` / Slice4 `15f80f5` / Slice5+6 `de6c63c` / Slice7 AFK `25cfa7c` / S19 HITL split `c5fc1ec`）。AFK 実走 = **PASS 1件・FAIL 0件**（S16–S19 GREEN・S1–S15 回帰維持・`Found no leaked weakptrs` 後の bash `grep -a`＝flush-race でない）。#99（Hakoniwa docking）merge 後の drift を反映した実 seam:

**契約訂正（grill 時 handoff の誤りを code で是正・load-bearing）**:
- **C# は当初 mimetype/data/stale を受領できなかった**（handoff は「既に受領可」と誤記）。Phase 6 で C# struct を拡張: `HostNotebookCellExecutor.NotebookCellOutput` に `Mimetype`/`Data` 追加（`:47`）、`NotebookRunResult` に `Stale int[]` 追加（`NotebookRunLane.cs:62/72`）、`Parse`（`:90`）が mimetype/data/stale を読むよう拡張。これが Slice 2/5 の C# 本体（handoff は過小評価）。
- **restage の lane worker-thread 経路**（最大の設計点・handoff 見落とし）: `IncrementalNotebookSession` は最初に駆動した lane worker thread に thread-local 固定されるため、編集時 restage を Unity main の `onEndEdit` から host 直呼びすると second-thread 駆動で拒否される。よって `INotebookCellExecutor.Restage`（`NotebookRunLane.cs:86`）を追加し、lane の restage work-item として **run_cell と同一 worker thread 経由**で route（AFK synchronous mode では inline）。配線: `BackcastWorkspaceRoot` の cell input `onEndEdit`/blur（`:371`/`:535` の `EditCommitted`）→`NotebookRunController.Restage`（`:117`）→lane→`HostNotebookCellExecutor.Restage`（`:41`）→`WorkspaceEngineHost.InvokeNotebookRestage`（`:278`）→Python `notebook_restage`。

**実 seam（drift 後の file:line・post-#99 merge）**:
- Python 3層 RPC: `_backend_impl.notebook_restage`（`:1038`・`f"c{i}"` index mapping）→`backend_service.notebook_restage`（`:400`）→`inproc_server.notebook_restage`（`:218`）。`run_cell` 戻り `ran[]` に `mimetype`/`data`、`stale` 配列を完備（incremental `notebook_session.py`）。
- glyph 3状態: `StrategyEditorWindowFrame.SetRunButtonGlyph`（`:140`・running ■）＋ `SetRunButtonStale`（`:159`・amber ▶／green idle）を別 signal で駆動（running と stale は実運用排他）。route は `NotebookRunController.ApplyResult`（`:172`）が index→region map・`onStaleRegionsChanged`→`BackcastWorkspaceRoot.SetCellStaleRegions`（`:718`）。
- rich output: `StrategyEditorView.SetOutput(text, mimetype, data)`（`:117`）→per-mimetype 分岐（`:123-141`）／`TryDecodeImage`（`:169`・base64→`Texture2D.LoadImage`→RawImage）。RawImage 兄弟は `StrategyEditorContentBuilder`（`:116` の `view.Initialize(…, image)`）が配線。controller passthrough は `ApplyResult`→`SetOutput(co.Output, co.Mimetype, co.Data)`（`:198`）。
- document badge: `BackcastWorkspaceRoot.DocumentBadgeText`（`:1532-1545`・basename/`* `dirty/Untitled）→`MenuBarView._docBadge`（`:122`・左寄せ・`:172` refresh）＝venue/mode/message badge（`:119-123`）とは別レーン。

**S19 image/png HITL 降格の裁定**（§3b 脚注の確定）: `Texture2D.LoadImage` は headless batch（`-batchmode -nographics` でも `-batchmode` GPU 許可でも）graphics device 不在で PNG を decode/upload 不能＝**環境制約（routing バグでない）**。前段 4 mimetype（plain/markdown/html/unsupported）の AFK GREEN が routing 健全を裏付け、image leaf のみ decode 能力 probe（throwaway `Texture2D.LoadImage`）で分岐: decode 可（HITL/GPU）＝RawImage active assert／decode 不可（headless）＝mimetype 伝播のみ AFK（`[image/png]` ラベルで非 collapse 証明）＋RawImage active は HITL（S3 save-fail / S8 glyph-count と同型の降格）。production code は無改変。

**残課題（follow-up）**: なし。U4（footer transport-retired）→**FOOTER-13**（`FooterModeE2ERunner`）／ U5（startup-no-Run）→**SCENARIO-15**（`ScenarioStartupE2ERunner`）の cutover 負 invariant re-home は **done**（Slice 9・§3c 着地ノート・AFK GREEN・commit d19dec6＋ID 衝突修正）。Phase 6 残課題ゼロ。
