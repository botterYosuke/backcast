...
**残課題（follow-up）**: U4（footer transport-retired）/ U5（startup-no-Run）の cutover 負 invariant re-home は未了（§3c）＝U4→`FooterModeE2ERunner`・U5→`ScenarioStartupE2ERunner`（startup は #99 で floating window 化＝`_windows.RectOf("startup")`）への assertion 移送が要・他 runner にカバー無し。
