...
- **#115 readiness（2026-06-24・AFK de-risk）**: S7 HITL（owner 実機）に入る前の de-risk として、teardown 修正
  （`driver.stop` の `asyncio.wait({task}, timeout=…)`）＋ MOCK 縦串を再確認。`test_live_stop_wedged_consumer`
  / `test_cell_auto_bridge_roundtrip` / `test_v19_cell_auto_parity` の 3 本 GREEN、broad pytest **545 passed**
  （main 現状）。さらに §5.1 の軽微 edge 2 件は **#116 で AFK 化**（Section24 / STRATEGY-51,52・RED→GREEN 済）
  ＝HITL が手で踏む必要はもう無い。**残るは実 venue（立花 demo）の teardown レグのみ**＝場中に owner が
  launch→RUNNING→（発注/約定）→■ で ▶ 復帰を 1 度確認し、本 §5 と AC チェックを owner が埋める。
- **S7 HITL PASS（2026-06-24・owner 実機）**: 立花 demo 実機で owner が縦串を完走し **PASS** を確認
  （launch → live run RUNNING → ■ stop で ▶ へ確実復帰・teardown ハング再発なし）。これにより #112 の
  「1 cell を Replay/Auto 両駆動」縦串は実 venue で閉じた。**parity oracle（`v19_morning.py` /
  `test_v19_auto_live_afk.py`）の退役可否**: cell live 経路が AC#1–4 を MOCK で緑化済（S6）＋実機 HITL PASS
  のため退役の前提は満たすが、決定的な正解定義として **当面存置**（cell 経路の回帰が出たとき命令型 twin が
  原因切り分けの基準になる）。#115 クローズ。
...
- **既知の軽微 edge（S7 HITL で実機確認・#116 で修正済）**: (1) live start-in-flight の ~100ms 窓で ■ を押すと
  ...
  state machine が要り AFK 反復前提なので #116 へ切り出した（→ §5.3）。

### 5.3 #116 — live cell run lifecycle edge 2 件の堅牢化（2026-06-24・着地）

§5.1 の「既知の軽微 edge」2 件を `NotebookRunController`（plain C#・MonoBehaviour-free）で堅牢化。挙動の出所は
すべて C# control-logic なので gate は **Python-FREE AFK**（`StrategyEditorNotebookE2ERunner` Section24 /
**STRATEGY-51..52**）。REAL register→start／venue teardown は既存 pytest が正本
（`test_v19_cell_auto_parity` / `test_cell_auto_bridge_roundtrip` / `test_live_stop_wedged_consumer`）。

- **edge 1 — start-in-flight 窓の lost-stop（deferred-stop）**: per-cell RUN(Auto) の live launch 直後、
  `register→start` RPC が in-flight（`LiveAutoTransportViewModel.IsStartInFlight`・run_id 未確定）の窓で ■ を
  押すと、`onLiveStop` が `HasActiveRun==false` で no-op になり stop が捨てられていた。修正: controller に
  **confirmed-active シグナル** `Func<bool> liveRunConfirmed`（= `HasActiveRun`）を注入。従来の `liveRunActive`
  （`HasActiveRun ∨ IsStartInFlight`・■→▶ 復帰用）と分離し、in-flight 窓の ■ は `_pendingLiveStop` を立てて
  **遅延**、`SyncLiveRunButton` が run を confirmed と認識した瞬間に `_onLiveStop` を適用する。start 失敗
  （`!active`）時は pending を破棄して ▶ 復帰。`liveRunConfirmed==null` は legacy 即時 stop へフォールバック
  （既存 Section23 / Hakoniwa Journey runner を無改変で維持）。
- **edge 2 — live run 中の起動元 cell 削除/File→New で dangling**: `ListMutated→Invalidate` は Replay lane の
  generation しか reconcile せず `_liveRunRegion` が dangling（`IsLiveRunLaunched` true のまま新規 launch を
  ブロック・vanished region への ▶/■ 描画）だった。修正: launch 時に **起動元 cell の identity** `_liveRunCell`
  を捕捉し、`Invalidate` で `RegionOf(_liveRunCell)==null`（削除 **または** region 再利用）なら button tracking を
  best-effort で破棄。**live run（venue セッション）は止めない**（findings 0026）——dual-launch は `LaunchLive` の
  `_liveRunActive()` ガードが引き続き防ぐので、run は mode 離脱の teardown でのみ停止。
  - **code-review fix（52c）**: reconcile 時に **deferred stop が pending（in-flight 中に ■ を押していた）**
    だった場合は、その intent を黙って捨てず best-effort で honor（`if (_pendingLiveStop) _onLiveStop()`）。
    `_onLiveStop` は `HasActiveRun` で self-guard 済なので confirmed run は止まり unconfirmed は no-op（二重 stop
    無し）。pending が無い通常の cell 削除は従来どおり venue run を止めない（findings 0026）。delete-during-in-flight
    レースで #116 が直した lost-stop が再発しないよう Section24 STRATEGY-52c で RED→GREEN 固定。
  - **設計の refinement（grill）**: issue 本文は region ベース reconcile（`CellOf(_liveRunRegion)==null`）だったが、
    `SyncWindowsToNotebook` は **File→New で region_001 を新 cell 0 に再バインド**するため region ベースでは
    File→New を取りこぼす（issue が trigger に File→New を列挙しているので不足）。**cell identity ベース**
    （`RegionOf(_liveRunCell)`）が削除と File→New の両方を正しく捕捉する正解。
- **RED→GREEN（実機 AFK で確認）**:
  - edge 1: `StopRunning` の deferred-stop ゲートを `confirmed=true`（常時即時）に潰す → `S24/STRATEGY-51:
    deferred stop was not applied once the run was confirmed` で RED → 復元で GREEN。
  - edge 2: `Invalidate` を region-only（`CellOf(_liveRunRegion)==null`）に潰す → 52a（削除）は PASS のまま
    `S24/STRATEGY-52b: _liveRunRegion dangled after File→New reused region_001` で RED（＝cell-identity が
    load-bearing と実証）→ cell-identity 版へ復元で GREEN。
  - GREEN: `[E2E STRATEGY NOTEBOOK PASS]`・`error CS` 0・exit 0（既存 STRATEGY-01..50 全緑を含む）。
- **配線**: `BackcastWorkspaceRoot` の `NotebookRunController` 構築に `liveRunConfirmed: () => _footerAuto != null
  && _footerAuto.HasActiveRun` を追加（`onLiveStop` は既に `HasActiveRun` ガード済なので遅延適用が landing 点と一致）。
