...
> ⚠️ **STALE（一部）— ADR-0033 / findings 0114 が D3 の「DEV_* prefill 廃止」面を supersede（2026-06-25）**。
> demo/verify の認証情報 prefill が復活した（prod は手入力のまま）。本 findings の **PRODGATE-05/06**
> （「prefill しない＝空欄で開く」）は撤去され、新 **PREFILL-01..05**（test_login_prefill.py）が demo/verify
> prefill を assert する。**PRODGATE-08（login_flow は env を読まない）は維持**（env 読みは presenter に閉じる）。
> ADR-0027 の prod-allow ゲート廃止（D1/D2/D4）は不変。
