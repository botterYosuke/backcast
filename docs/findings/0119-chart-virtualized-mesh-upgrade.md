...
- **適用範囲**: `BackcastWorkspaceRoot` が poll 毎に `SetFitAllOnAutoScale(DockShape.ShouldFitChartToAll(mode))` を呼び、**Replay チャートのみ** true。Live は false のまま（`max_history_len=1000` リングを 6px で読みやすく保つ）。mode flip は idempotent に追随。mode→fit の判定は純関数 `DockShape.ShouldFitChartToAll(displayMode) = !IsLiveShape(displayMode)` に抽出し、`ChartFitAllE2ERunner` FITALL-WIRING-05 で全 mode を pin（predicate を反転すると RED＝inline `!IsLiveShape` 単独では拾えなかった litmus を回帰ガード化）。
- **gate**: `ChartFitAllE2ERunner`（FITALL-OFF-01 = opt-in 床/Live 非回帰・FITALL-FITS-02 = **200 本 gapped**（~280 暦日 span）を time-span fit で全描画＋最古 bar を左端に pin・FITALL-OVERFLOW-03 = 1000 本 MIN クランプ右端寄せ・FITALL-RESET-04 = reset で fit 復帰＋**contiguous 300 本で最古 bar pin の回帰ガード**・**FITALL-WIRING-05 = mode→fit policy（`DockShape.ShouldFitChartToAll`）を Replay→fit / Live→DEFAULT で pin**）。既存 RESET-INIT-01 / VIRTUALIZE-01/02 / RESET-01 は false 経路で不変緑。
- **fit-exact の translation は first に直接 pin（#156 follow-up・float round-trip 修正）**: 全 series が収まるとき（MIN ≤ fit ≤ MAX）は `translation_ms = first_bar_open_time_ms` を直接代入する。丸めた `visibleBars` から `latest-(visibleBars-1)*basis` で逆算すると float32 往復誤差で translation が `first` を数百 ms 上回り、`OnPopulateMesh` の厳密 `open_time_ms < winStart` 仮想化が**最古 bar を ~25% の span で脱落**させる（contiguous 300 本含む）。FITALL-FITS-02 / RESET-04 が exact 件数＋`translation == first` で回帰ガード。overflow（MIN）/ 短系列（MAX）枝は従来どおり右端寄せ。
