...
> **注**: 以下のコメントおよび §D-14 表の「旧 assertion」列は #161/#163 完了時点の migration narrative。`BestBid()` / `BestAsk()` / `LastRow()` / `Image Background` は撤廃済みの旧 Text API で、現行コードには存在しない。現行 API 一覧は ADR-0035 / `DepthLadderE2ERunner.md` 参照。
