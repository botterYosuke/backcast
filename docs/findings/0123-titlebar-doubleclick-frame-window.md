...
- `marginFraction` は NaN/Inf を 0 に倒し、`[0, 0.95]` に clamp（負値・1 超過は contain-fit を破壊する）。
- `size.x <= 0` や `size.y <= 0`（degenerate）は zoom 計算で 0 除算を避け、MAX_ZOOM へ倒す。
- `viewportSize.x|y <= 0` / NaN / Inf のときは **`fallbackView` 引数**（caller の現視点）をそのまま返す。これは S1 のときは `CanvasView.Identity()` で良かったが、S2 の glide seam では「identity への 200ms reset jolt」になるため `fallbackView` を渡せるシグネチャに改めた（`fallbackView == null` のときのみ Identity フォールバック）。`FloatingWindowTitleInput.OnPointerClick` / `BackcastWorkspaceRoot.FrameChartWindowForInstrument` は `_canvas.CaptureView()` を fallback に渡し、degenerate input に当たった glide が zero-distance（実質 no-op）になることを belt-and-braces で保証する。
...
- **Left ボタン限定**（owner 決定）: `IPointerClickHandler` は uGUI 仕様で全ボタンに発火するため、右クリックや中クリックのダブルでもタイトルバーから framing が走ってしまう。`eventData.button != Left` で fail-quiet（drag / focus / resize と同じ owner-locked gesture surface に揃える）。
- **mid-reflow / pre-init no-op**: `_viewport.rect.size.x|y <= 0` のときは early-return。CanvasViewMath.FrameWindow 側にも degenerate guard はあるが、ここで弾いておくと glide 開始すらしない（fallback view の zero-distance tween すら走らない）。
...
- API:
  ...
  - `IsAnimating` — 観測子。
- `Update()` / `Advance(dtMs)`:
  - `elapsedMs += clamp(dtMs, 0, MAX_DT_MS)`（time-scale 不変、unscaledDeltaTime ベース）。
  ...
  - `t < 1` のとき: `view_t = lerp(from, to, eased)` → `controller.ApplyView(view_t)` + `_lastApplied = view_t`。
  - `t >= 1` のとき: `controller.ApplyView(_to)` 直書き（lerp 結果ではなく `_to` をそのまま）+ クリア。lerp 値と `_to` が float ULP で微差を生むと、parallax layer offset 更新が同一 frame 内で 2 イベント発火するのを防ぐ（§3.3）。
- 定数:
  - `DURATION_MS = 200f`（findings 0088 §3 spring と整合）。
  - `EPS_INTERRUPT = 1e-3f`。
  - `MAX_DT_MS = DURATION_MS * 0.25f`（= 50ms、§3.3）。
...
### §3.3 first-frame dt cap と final-tick `_to` 直書き（review hardening）

- **`MAX_DT_MS = DURATION_MS * 0.25f`** — domain reload / scene load の直後は `Time.unscaledDeltaTime` が数百 ms に跳ね、それを raw に Advance に流すと初回 tick で `t>=1` に達して 200ms ease-out が即時 apply に潰れる。1 tick の dt を duration の 25% に cap し、worst-case 最初の数 frame でも `≥4 ticks` を保証する。
- **final tick で `_to` を直書き** — `t>=1` のときは `Lerp(_from, _to, 1)` の結果ではなく `_to` 自体を `ApplyView` する。lerp 結果と `_to` は浮動小数点 ULP で微差し得るため、parallax layer offset を更新する 2 つの ApplyView 呼出 (`Lerp(_,_,~1)` ＋ 最終 `_to`) が同一 frame 内で連続発火する経路を排除する（findings 0088 §14 の「fire-point は確定点のみ」規律）。
...
グライド中に `controller.PanByScreenDelta` / `controller.ApplyView` で外部書き戻し → 次の `Advance` で「`live := CaptureView()` と `_lastApplied` が `EPS_INTERRUPT=1e-3` 以上離れている」を検知して `IsAnimating == false`（kill）。kill 後の Content は外部が書いた値のまま（途中 `ApplyView` で潰さない）、`_lastApplied` も更新しない（次の `BeginGlide` が新 `_from` を取り直す）。
...
### §4-J `[E2E FRAMING-S3-INTEGRATION PASS]`

review finding M4: §4-G (`S3Sidebar` — hook が click で発火する) と §4-H (`S3Noop` — chart 窓が無いと no-op) は seam の両端だけで、production 経路（`UniverseSidebarController.FocusChartHook = BackcastWorkspaceRoot.FrameChartWindowForInstrument`）が本当に繋がっているかは保証していない。新 section は M3 抽出した `BackcastWorkspaceRoot.TryFrameChartWindowForInstrument` を `FocusChartHook` に直接 wire し、active chart 窓を `Adopt` した上で `SelectRow` を呼ぶ → `glide.IsAnimating == true` を assert（hook 未代入 / 引数順違反 / closure キャプチャミスはこのセクションだけが捕まえる）。同 id 再クリックで `glide.Stop()` 後に再 glide することも確認し、§4-G の「再クリックでも発火」契約が glide 層まで通ることを保証する。
...
| `FRAMING-S3-INTEGRATION` | S3 (#168) | sidebar → wired helper → active 窓で glide が開始する (review finding M4) |
