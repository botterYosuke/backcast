...
## 視覚リファインメント（owner・2026-06-27 追補 — grill-with-docs）

owner HITL 目視後の追補依頼「cube/sphere/capsule/cylinder の背景を透明にして左に寄せる。参考画像（ゲームのリソースバー＝
アイコン下げ数値・帯なしフラット）のように」。grill-with-docs（2026-06-27）で 4 つの下位決定を確定。**方針: [[ADR-0038]]**——
ADR §Decision 2 の「ガター予約なしで canvas に重なる」を**より忠実に**するリファインメント（透明＋クリック透過は「canvas に
重なる」と非矛盾）であり ADR の decision を覆さない。よって ADR 無改変・新規 ADR 不要、本 finding に下位事実として記録。

> 用語整理（grill で sharpen）: owner の言う「cube/sphere…の背景」＝**各アイコンの四角枠の背景**ではなく**バー全体の帯
> （strip）の背景**。各アイコン枠は既に透明（`AccountSummaryIconStage` の icon camera が alpha 0 clear＋`SetIconTexture`
> 後 `RawImage.color=white`）。実際に見えていた不透明背景は `_stripBg`（`panel_background`）のみ＝透明化対象はこれ。

- **D8 — strip 背景を透明＋クリック透過（owner 回答・sidebar をミラー）**: `_stripBg.color` を `panel_background`（alpha 1）→
  **alpha 0**、`_stripBg.raycastTarget` を **false** に。**正準ミラー = `UniverseSidebarView.cs:139-141`**（`bgColor.a=0f`／
  `bgImage.raycastTarget=false`「透明背景はクリックを遮らず後ろの canvas へ透過させる」）。owner note「sidebar も同様に背景
  透明でマウスのクリックも邪魔しない」。**上記「44px chrome をクリック吸収（意図）」を本決定が supersede**（帯はもう clicks を
  食わない）。`ApplyTheme` も `_stripBg.color` を alpha 0 維持で再適用すること（theme flip で alpha が戻らないよう）。
- **D9 — スロットを左詰め・固定幅（owner 回答）**: 現状 4 スロットは各 25%幅で全幅均等配置（`root.anchorMin.x=index*0.25`）。
  これを**左端から固定幅で連続配置**し右側は空ける（参考画像の通り）。固定幅化により各スロットの raycast 領域も小さく縮み、
  帯の空白部はクリック透過（D8）と整合＝**sidebar と同じ「コンテンツのある所だけ clicks を拾う」挙動が自然成立**。
- **D10 — 数値をアイコンの下に縦積み（owner 回答）**: 主数値を**アイコンの右**→**アイコンの下**へ。アイコン上・数値下の
  縦レイアウト。BAR_H は icon(30)＋数値(~15) が収まるよう拡げる（下位事実・実装で確定。透明帯ゆえ canvas を覆う面積が増えても
  reflow なし＝D4「ガター予約なし」）。
- **D11 — 主数値は短縮表記（owner 回答・新 formatter）**: 参考画像の `298k` / `3.17k` に倣い**金額を k/M 短縮**。**バー専用の
  新 compact formatter を追加**し、ホバー card は従来 `AccountSummaryFormat.Money`（フル桁 `#,0`）の **byte 一致再利用を維持**
  （ホバーでフル精度 `298,000` を出す＝精度は失わない）。count（positions/orders）は短縮せず素のまま。
  > 注意: `Money` を直接書き換えると D5 のホバー byte 一致が壊れる。**別関数**（例 `MoneyCompact`）を足し、バー主数値だけ
  > それを使う。slot ①②（equity/buying_power）が金額、③④（件数）は plain。

### D8–D11 の AFK ゲート（実装着手前に behavior-to-e2e formal invoke）
- 透過: `_stripBg.color.a==0` ＋ `_stripBg.raycastTarget==false`（sidebar litmus と同型・retheme 後も alpha 0 維持を ApplyTheme で）。
- 左詰め: slot の anchored 矩形が左端から固定幅で連続し最右 slot の right < strip width（右に空白が残る）を assert（25%均等の旧 geometry を反転）。
- 縦積み: 数値 Text の rect が icon rect の **下**（primary.y < icon.y）を assert。
- 短縮: 大きい金額（例 1_234_567）で主数値 == `"1.23M"`（or 採用記法）かつホバー card == フル桁 `"1,234,567"` を pin（litmus＝短縮とフル桁が別経路である証明）。
- 既存 ASB-01..10 のうち geometry / 主数値文字列を assert する leg は本リファインメントに整合させて再走。

## 日本語ラベル＆フォント（owner・2026-06-27 追補 — behavior-to-e2e）

owner HITL 目視で「ホバーポップアップに何も表示されない」報告（スクショ: Replay/Disconnected・リプレイ未実行・①card に裸の `—`）。
behavior-to-e2e で 2 つの原因に分解:

- **原因1（主）= フォントに日本語が無い**: ホバー card の `cardText` は legacy `UnityEngine.UI.Text`＋`_font`＝`LegacyRuntime.ttf`
  （Liberation Sans＝**CJK グリフ無し**）。プロジェクトは CJK フォントも fallback も同梱せず（TMP/legacy とも Latin-only）。当初
  「英語略号→日本語ラベル」化（`equity=`→`純資産:` 等）したが、このフォントでは日本語が描画されない。
- **原因2（副）= 空データ時に裸の `—`**: `PushAccountBarEmpty` が detail を `PLACEHOLDER`（`—`）のみに。リプレイ未実行（=スクショの状態）
  ではホバーしてもラベルすら出ず「空＝壊れている」に見える。

**修正（owner 選択: OS 動的フォント／ホバーカードのみ）**:
- `BackcastWorkspaceRoot.CreateCjkFont()`＝`Font.CreateDynamicFontFromOSFont(["Yu Gothic UI","Yu Gothic","Meiryo","MS Gothic","MS PGothic"],14)`
  を Awake（＋BuildWorkspace の null-guard で AFK runner 経路も）で生成し、`AccountSummaryBarView.Build(font, cjkFont, topOffset)` で
  **cardText だけ** `_cjkFont` に配線（主数値は数値ゆえ `_font`＝Latin 維持）。
- `AccountSummaryFormat.EmptyDetail(slot)` を新設し `PushAccountBarEmpty` が **ラベル付きプレースホルダ**（`純資産: — …`／`買付け余力: — …`／
  `(建玉なし)`／`(注文なし)`）を出す。

**★教訓1 = legacy dynamic Font は OS フォールバックする（HasCharacter は判別不能）**: 当初 ASB-15 を `Font.HasCharacter('純')` で
gate しようとしたが、**`LegacyRuntime.ttf`（dynamic font）の `HasCharacter('純')` が true** を返した（実機実測）。legacy `UnityEngine.UI.Text`＋
dynamic font は OS フォントへ on-demand フォールバックするため、`HasCharacter` は dynamic font に判別力が無い（＝豆腐だったのは
**TMP のコードエディタ**＝OS フォールバックしない別系統で、legacy card は元々 OS フォールバックで日本語が出ていた可能性が高い）。
明示 CJK 配線は「OS フォールバック頼みでなく既知フェイスに固定」する belt-and-suspenders として保持。**vacuity ガード（Latin が 純 を
map したら FAIL）が誤った proxy を即座に露出させた**＝proxy gate を疑え。

**★教訓2 = 実グリフ描画は HITL・配線は AFK で非空虚 gate**: 「日本語が描画されるか」は GPU 依存で headless 不可。代わりに ASB-15 は
**配線**（card font ≠ 数値 primary の Latin font）を gate。非空虚化のため**production と同じフェイス列で OS に日本語フェイスが在るか
プローブ**し、在るのに card==primary font なら FAIL（fix を `_font` に戻すと owner Windows で RED）。不在環境（CI）は SKIP。
実画素の日本語描画は HITL 行へ（「ホバー card に日本語が豆腐でなく描画される」）。
