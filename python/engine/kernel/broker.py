...
accepted during `on_bar(N)` fills **immediately at bar N's close** (the LAST price),
in full, with zero slippage and zero commission. `OrderFilled.ts_event = bar N.ts_event`.
...
class ReplayBroker:
    """Fills ACCEPTED MARKET orders against the current bar's close."""

    def __init__(self, order_engine: OrderEngine) -> None:
        self._engine = order_engine

    def fill_market(self, order: Order, bar: Bar) -> OrderFilled:
        """Fill an ACCEPTED MARKET order at `bar.close`, mutating the order to FILLED."""
        price = bar.close
        order.venue_order_id = self._engine.next_venue_order_id()
        order.status = OrderStatus.FILLED
        order.filled_qty = order.quantity
        order.avg_px = price
        order.ts_last_ns = bar.ts_event_ns
        return OrderFilled(
